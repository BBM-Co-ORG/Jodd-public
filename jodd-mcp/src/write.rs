...
pub fn do_list_accounts(accounts: &[Account], scope: &WriteScope) -> Vec<AccountEntry> {
    accounts
        ...
        .map(|a| AccountEntry {
            ...
            allowed_folders: scope.allowed_folders(&a.id).to_vec(),
        })
        .collect()
}
...
pub fn resolve_writable_account<'a>(
    ...
    if !jodd_lib::backend::Capabilities::for_backend(account.backend_kind).writes.allows(w) {
        let area = match w {
            ...
            // No MCP tool moves or deletes a note today — this arm exists so
            // a future one fails to compile without a message here, not so
            // it is reachable now.
            jodd_lib::backend::Write::Relocate => "moving or deleting notes",
...
pub fn do_create_note(
    ...
    let allowed = scope.allowed_folders(&account.id);
    ...
    // Mirrors extract_note's insert: a fresh uuid in the backend's own shape
    // (gotcha #18 — a CloudKit recordName is lowercase, Apple's email wire
    // format uppercase), Dirty, empty remote id until the worker pushes.
    let uuid = jodd_lib::backend::mint_uuid_for(account.backend_kind);
...
mod tests {
    ...
    /// Gotcha #18's mint half, through the MCP door: `do_create_note` writes
    /// straight to SQLite, so it must mint the per-backend shape itself. A
    /// CloudKit recordName is lowercase; Apple's email wire format is upper.
    #[test]
    fn create_note_mints_the_uuid_shape_its_backend_writes() {
        use jodd_lib::accounts::BackendKind;
        for (id, backend, kind) in [
            ("icloud:a@icloud.com", "icloud", BackendKind::ICloud),
            ("gmail:a@x.com", "gmail", BackendKind::Gmail),
        ] {
            let (_dir, db) = test_db();
            db.upsert_folder_from_remote(id, "Notes/__Claude__", "folder-record").unwrap();
            let uuid = do_create_note(
                &db, &[acct_with_backend(id, backend)], &scope_with(id, &["Notes/__Claude__"]),
                id, "Notes/__Claude__", "t", "body",
            ).unwrap();
            assert_eq!(
                jodd_lib::backend::canonical_uuid_for(kind, &uuid), uuid,
                "{backend}: minted {uuid} is not already in the backend's canonical shape"
            );
            match kind {
                BackendKind::ICloud => assert_eq!(uuid, uuid.to_lowercase(), "a CloudKit recordName is lowercase: {uuid}"),
                _ => assert_eq!(uuid, uuid.to_uppercase(), "Apple's email wire format is uppercase: {uuid}"),
            }
        }
    }
    ...
    /// Decision 12 of the URL ingest spec narrows cell `style` in the shared
    /// allowlist — so an agent's markdown table keeps its alignment and an
    /// inline-styled cell loses its CSS here too.
    #[test]
    fn create_note_keeps_table_alignment_and_drops_other_cell_css() {
        let (_dir, db) = test_db();
        let uuid = do_create_note(&db, &[acct("a@x.com")], &scope_with("a@x.com", &["Notes/__Claude__"]),
            "a@x.com", "Notes/__Claude__", "t",
            "| a | b |\n|:-:|---|\n| 1 | 2 |\n\n<table><tbody><tr><td style=\"position:fixed;inset:0\">overlay</td></tr></tbody></table>\n",
        ).unwrap();
        let body = db.get(&uuid, "a@x.com").unwrap().unwrap().body_html;
        assert!(body.contains(r#"<td style="text-align: center">1</td>"#), "{body}");
        assert!(!body.contains("position:fixed"), "{body}");
        assert!(body.contains(">overlay</td>"), "the cell's text survives without its style: {body}");
    }
