...
## [0.29.0] - 2026-09-20
### Added
- **Review meeting actions before saving.** Action Items now previews decisions, actions, owners, dates and missing information with links to the full source passages. Save a separate draft or explicitly confirm an append. If the target changed after preview, generate a new draft. Missing facts stay unspecified; quotation checks do not prove intent or ownership, so human review remains necessary.
- **Execution receipts make AI work inspectable.** Expand local metadata for stages, attempts, configured model and reported, estimated or unknown usage. App Settings includes retention, deletion and redacted export. Raw note text, prompts and answers are not stored in receipts by default.
- **Workflow-wide AI limits and optional enrichment.** Configure concurrency, attempts and planning/output allowances, including retries and follow-ups. Automatic folder/link suggestions are off by default. Planning units are not money; CLI usage and provider enforcement can remain unknown, and there is no automatic provider/model fallback.
- **A synthetic teaching lab and UI acceptance cases are available in the source checkout.** They replay exact-search, review, missing-information and refusal flows without real AI or private notes. They are development fixtures, not an installed-app mode or evidence of model quality.

### Changed
- **Account AI permission applies across Jodd-managed workflows.** Disabled accounts' data and metadata are excluded; Ask discloses destination/scope and restarts conversation eligibility when scope or policy changes. This does not replace external MCP permissions or recall data already sent.
- **Sync work is scheduled per account with bounded concurrency.** Two remote account rounds can progress independently, while LocalFS has its own slot. Same-account writes remain serialized. Two stalled remote rounds can still occupy both remote slots; this is not an unconditional latency guarantee.
- **Long source lists start collapsed.** Readable host/path labels, source counts, expansion, keyboard access and loading/error feedback keep evidence accessible. Browsing another folder now explains why the current editor remains open, with an explicit Close note action.

### Fixed
- **Enter on Cancel really cancels.** Confirmation dialogs use native button activation, safe initial focus, contained keyboard navigation and focus restoration.
- **Search results follow the latest query and scope.** Late replies no longer overwrite newer results; loading, empty, error and retry states are explicit. Exact-folder search remains exact.
- **Note identity and save feedback are account-qualified.** Duplicate UUIDs across accounts no longer collide in selection/mutations. The editor distinguishes unsaved, saved on this device, pending, blocked and synced states, including backend-assigned identities.
- **Newer unsaved drafts survive late saves and navigation.** An older save can advance the note's identity/version without replacing a newer closed draft if its queued save fails. Old sources also clear immediately when switching notes.
- **Recursive Ask/DB/MCP scope agrees on literal folder boundaries.** Case and literal wildcard characters no longer broaden recursive SQLite reads; exact navigation and MCP deny-by-default are preserved.

### Verification scope
- Host tests and synthetic Chrome fixtures cover these changes. They do not establish live provider quality/cost, human time savings, complete native platform smoke coverage or delayed Apple Notes reconciliation. Android remains a Developer Preview; release publication is gated by its on-device encryption test.
