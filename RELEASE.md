...
**The Android at-rest-encryption proof gates that publication.** `finalize` —
the job that publishes to the public repo — needs `android-encryption`, which
boots an emulator and runs the SQLCipher round-trip on-device
([android-encryption.yml](.github/workflows/android-encryption.yml), pulled in
via `workflow_call` so there is one definition, not a copy). If it fails, the
binaries still build and the private draft still exists, but **nothing reaches
users**. This was not true for v0.24.1, which published while that job was red
because it was absent from `finalize`'s `needs:`.

That workflow also runs on its own — automatically when `db_crypto.rs` or the
Cargo manifests change, and on demand:

```bash
gh workflow run android-encryption.yml
```

Prefer that over a full manual dispatch when you only need to exercise the
encryption proof: ~13 minutes and one Linux runner, versus a whole release
pipeline including macOS runners at 10× billing.
...
- **SQLite cache is encrypted at rest (SQLCipher)** as of v0.24.0, at `~/Library/Application Support/jodd/jodd.sqlite3` (macOS) / `%APPDATA%\jodd\` (Windows).
