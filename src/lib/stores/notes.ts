import { noteKey } from '../noteIdentity';
import { writable, derived, get } from 'svelte/store';
import type { Note, Folder, Account, MessageIndex, TrashedNote } from '../types';
import type { PushConfirmation } from '../editorRenderDecision';
...
// `Account.id` is now `{backend}:{email}`, so two accounts CAN share the same
// email on different backends (e.g. gmail:a@b.com and microsoft:a@b.com).
// The address alone no longer distinguishes them — this label is rendered
// unconditionally beside the address everywhere an account is named, not
// only when a collision is detected, per the account-identity design.
// `kind` is the serde form of BackendKind: 'gmail' | 'microsoft' | 'local_fs'
// | 'icloud'. Note that 'icloud' is NOT the snake_case rendering of `ICloud`
// (that would be 'i_cloud') — the Rust variant carries an explicit
// `#[serde(rename = "icloud")]` so accounts.json agrees with the `icloud:` id
// namespace. Matching 'i_cloud' here would silently fall through to the
// default and print the raw kind beside every iCloud account.
export function backendLabel(kind: string): string {
  switch (kind) {
    case 'gmail': return 'Gmail';
    case 'microsoft': return 'Outlook';
    case 'local_fs': return 'Local Folder';
...
// `folders` table row backs these (design spec decision 4). 'extracts' is
// derived from the Source block (extract filing, 2026-09-15).
export type SmartFolderKind = 'orphaned' | 'stale' | 'extracts';
export const selectedSmartFolder = writable<{ account: string; kind: SmartFolderKind } | null>(null);
...
// LLM folder proposals, keyed `folderSuggestionKey(account, uuid)`. Never
// persisted — a restart loses them by design and "Suggest folder" asks
// again. A proposal survives navigating away: the editor chip reappears
// when that note is reopened (see NoteEditor's chipSuggestion).
export type FolderSuggestion = { ai_result_id?: string; path: string; reason: string | null };
export const folderSuggestions = writable<Record<string, FolderSuggestion>>({});

export function folderSuggestionKey(accountId: string, uuid: string): string {
  return noteKey({ account_id: accountId, uuid });
}

export function setFolderSuggestion(accountId: string, uuid: string, s: FolderSuggestion) {
  folderSuggestions.update((m) => ({ ...m, [folderSuggestionKey(accountId, uuid)]: s }));
}

// Also removes every other entry of the same account holding the SAME object
// — the twin `recordFolderSuggestion` files under a pre-rekey uuid (gotcha
// #16). Identity, not equality: another note's equal-looking proposal stays.
export function clearFolderSuggestion(accountId: string, uuid: string) {
  folderSuggestions.update((m) => {
    const key = folderSuggestionKey(accountId, uuid);
    const gone = m[key];
    if (!gone) return m;
    const prefix = JSON.stringify([accountId]).slice(0, -1) + ',';
    return Object.fromEntries(
      Object.entries(m).filter(([k, v]) => k !== key && !(v === gone && k.startsWith(prefix))),
    );
  });
}
...
/**
 * Drop any selection pointing at an account that is no longer active.
 *
 * An account leaves the active set in two ways, and only one of them is a
 * click: the user deactivates it, or the sync worker finishes draining it and
 * flips it to Inactive on its own (gotcha #6). Either way `selectedNote` and
 * `selectedFolder` keep pointing at the account that just went away — the
 * editor goes on rendering a note nothing can load, then errors, and the
 * folder header keeps a path that only ever existed in the old account, so the
 * NEXT account gets "Folder not found: Notes/…" for a folder that was never
 * theirs.
 *
 * Written as one subscription over the account list rather than as cleanup
 * inside the deactivate button, because the button is not the only path in and
 * the worker's path has no click to hang cleanup off.
 */
activeAccounts.subscribe(($active) => {
  // An EMPTY list means "not loaded yet", not "everything went away". The
  // store starts empty and stays empty until the first `list_accounts`
  // resolves, so acting on it would clear a selection made during startup —
  // which is what `newNoteFn`'s test caught, and what a user would have seen
  // as their folder silently jumping to the root on every launch.
  if ($active.length === 0) return;
  const live = new Set($active.map((a) => a.id));

  const note = get(selectedNote);
  if (note?.account_id && !live.has(note.account_id)) selectedNote.set(null);

  const trashed = get(selectedTrashedNote);
  if (trashed && !live.has(get(currentAccount) ?? '')) selectedTrashedNote.set(null);

  // The folder is a bare path with no account on it, so it is stale exactly
  // when the account it was chosen in has gone. Back to the root rather than
  // to nothing: every account has one, and an empty selection would leave the
  // note list with nothing to render.
  const cur = get(currentAccount);
  if (cur && !live.has(cur)) {
...
// `relocate` split out from `notes` for iCloud (M2 relocation live pass,
// 2026-08-24): move/trash/restore never touch a note's content and are
// measured safe there, while content editing stays refused (per-character
// CRDT identity on every note). Every other backend has `notes === relocate`
// today, but the two are independent bits on the Rust side
// (`backend::Writes`) and must not be collapsed back into one here.
export type WriteKind = 'notes' | 'relocate' | 'folders' | 'sidecars';
export type Writes = { notes: boolean; relocate: boolean; folders: boolean; sidecars: boolean };
...
// Multi-select set in the note list. Stores account-qualified note keys (not Notes) so it stays
...
// Invalidate local search/tag snapshots after mutations; keys never cross accounts.
export const noteMutationRevision = writable(0);
export const pendingDeletedNoteKeys = writable<Set<string>>(new Set());
...
// ─── Push confirmations ──────────────────────────────────────────────────────
// The newest receipt from the sync worker naming the bytes a backend actually
// accepted for a note (Rust: `note-pushed`). Distinct from a save: `save_note`
// is local-first — it commits to SQLite and returns, and the worker pushes on
// a later tick — so this is the only signal that says what the REMOTE holds.
// NoteEditor anchors its `isEcho` check to it; nothing else should need it.
export const lastPushConfirmation = writable<PushConfirmation | null>(null);
