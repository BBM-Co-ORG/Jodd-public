...
describe('per-area write gate (canWrite)', () => {
  const allWritable = { has_trash: true, writes: { notes: true, relocate: true, folders: true, sidecars: true } };
  const allRefused = {
    has_trash: false,
    writes: { notes: false, relocate: false, folders: false, sidecars: false },
  };
  ...
  const notesOnly = {
    has_trash: false,
    writes: { notes: true, relocate: true, folders: false, sidecars: false },
  };
  ...
  it('keeps folders hidden on Microsoft while notes and pin stay open', () => {
    const ms = { has_trash: false, writes: { notes: true, relocate: true, folders: false, sidecars: true } };
    expect(canWrite(ms, 'notes')).toBe(true);
    expect(canWrite(ms, 'folders')).toBe(false);
    expect(canWrite(ms, 'sidecars')).toBe(true);
  });

  // The actual iCloud shape (M2 relocation live pass, 2026-08-24): move,
  // trash and restore never send the note's document, and are measured safe
  // on three independent Apple surfaces. Content editing stays refused
  // (per-character CRDT identity on every note). This is the first backend
  // where `notes` and `relocate` genuinely disagree — every other backend
  // above has them equal, which is what made a single `notes` bit look
  // sufficient until this one.
  it('lets iCloud relocate notes it cannot edit', () => {
    const icloud = {
      has_trash: true,
      writes: { notes: false, relocate: true, folders: true, sidecars: false },
    };
    expect(canWrite(icloud, 'notes')).toBe(false);
    expect(canWrite(icloud, 'relocate')).toBe(true);
    expect(canWrite(icloud, 'folders')).toBe(true);
    expect(canWrite(icloud, 'sidecars')).toBe(false);
  });
...
describe('canWriteAccount (whole-account gate for not-yet-split components)', () => {
  it('mirrors canWrite for notes, the area it proxies', () => {
    expect(canWriteAccount({ has_trash: false, writes: { notes: false, relocate: true, folders: true, sidecars: true } })).toBe(false);
    expect(canWriteAccount({ has_trash: true, writes: { notes: true, relocate: true, folders: false, sidecars: false } })).toBe(true);
  });
