...
export type FolderSuggestionOutcome =
  | { kind: 'suggested'; ai_result_id?: string; uuid: string; path: string; reason: string | null }
  | { kind: 'no_candidates' }
  | { kind: 'none_fits' }
  | { kind: 'note_not_found' };
...
export function requestFolderSuggestion(accountId: string, uuid: string, parentRequestId?: string): Promise<FolderSuggestionOutcome> {
...
export function recordFolderSuggestion(accountId: string, uuid: string, outcome: FolderSuggestionOutcome) {
  if (outcome.kind === 'suggested') {
    const s = { path: outcome.path, reason: outcome.reason, ...(outcome.ai_result_id ? { ai_result_id: outcome.ai_result_id } : {}) };
...
export function explicitErrorMessage(e: unknown): string | null {
  ...
  if (msg.startsWith('provider not configured: AI data access')) return msg.replace('provider not configured: ', '');
