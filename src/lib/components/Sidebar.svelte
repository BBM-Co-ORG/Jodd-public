<script lang="ts">
  ...
  type PendingPushes = {
    ...
    pins: number;
    folders: number;
    // Notes holding unsent content that the worker has STOPPED trying to
    // send (the backend refused permanently). Not queued work — see the two
    // helpers below for why that distinction has to be kept.
    blocked: number;
  };
  // What the worker still owes the backend. Drives "finishing sync — N left",
  // and must exclude `blocked` or that number would never reach zero: nothing
  // is going to send those, so the account would sit in Draining forever
  // showing a count that never moves.
  const totalPending = (c: PendingPushes): number =>
    c.notes + c.deletes + c.pins + c.folders;
  // What the user LOSES if this account is deleted right now. Includes
  // `blocked`, because a note the worker gave up on is still the user's
  // content and `db.delete_account` wipes it with everything else. Leaving it
  // out of the removal warning would hide exactly the notes most likely to be
  // unsent — the ones that could not be sent.
  const totalUnsent = (c: PendingPushes): number => totalPending(c) + c.blocked;
  ...
  //
  // Retired workflow folders: Jodd no longer writes to them, so they are the
  // user's to rename or delete. `__Extracts__` retired with extract filing
  // (2026-09-15) — extracts now land in Notes/Inbox and the "Extracts" view
  // is derived from the Source block. `__Claude__` is NOT retired: jodd-mcp
  // uses it as its write allowlist, and renaming it breaks MCP writes.
  const RETIRED_WORKFLOW_FOLDERS: readonly string[] = ['__Extracts__'];
  ...
  function isProtectedWorkflowFolder(
    ...
    if (!(leaf.startsWith('__') && leaf.endsWith('__') && leaf.length > 4)) return false;
    return !RETIRED_WORKFLOW_FOLDERS.includes(leaf);
  }
  ...
  // Smart Folders ("Orphaned"/"Stale"/"Extracts") are fully virtual — no
  // folders-table row, no rename/move/delete. Selecting one leaves
  // selectedFolder's VALUE untouched (App.svelte's folder-reactive watcher is
  // keyed off changes to that store and must not re-fire); NoteList checks
  // selectedSmartFolder FIRST, before falling through to the folder/tag/search
  // logic, so the stale selectedFolder value is simply never consulted while
  // a Smart Folder is active.
  function selectSmartFolder(accountId: string, kind: SmartFolderKind) {
    if ($currentAccount !== accountId) {
      currentAccount.set(accountId);
    }
    clearSelectedTags();
  ...
  //
  // A `draining` account no longer gets that treatment at all: the backend
  // (roadmap #0d) queues the request instead of destroying anything, and
  // only performs the real removal once its queue is confirmed empty — so
  // there is nothing here for the "will be deleted" warning to apply to.
  // `remove_account`'s return tells us which of the two actually happened
  // (`RemoveAccountOutcome`, serialized as "removed" | "queued") rather than
  // inferring it from `a.status`, which can race the worker's own
  // Draining -> Inactive flip between this click and the command's reply.
  async function removeAccount(a: Account) {
    const accountId = a.id;
    let confirmMessage: string;
    if (a.status === 'draining') {
      confirmMessage = `Remove ${accountId} from Jodd? It's still finishing a sync — removal will happen automatically once that's done. Anything still syncing will be sent first, not lost.`;
    } else {
      let n: number | undefined;
      try {
        // totalUnsent, not totalPending: removal destroys blocked notes too,
        // and those are the ones the user is least likely to have another copy
        // of — nothing ever sent them anywhere.
        n = totalUnsent(await invoke<PendingPushes>('count_pending_pushes', { accountId }));
      } catch (e) {
      ...
      const warning =
        n === undefined
          ...
          : n > 0
            ...
            : '';
      confirmMessage = `Remove ${accountId} from Jodd?${warning}`;
    }
    const ok = await askConfirm('Remove account?', confirmMessage);
    if (!ok) return;
    try {
      const outcome = await invoke<'removed' | 'queued'>('remove_account', { accountId });
      if (outcome === 'queued') {
        // Still here, still draining server-side — just record the request
        // so the row shows "removal queued" instead of a re-clickable
        // button. list_accounts' periodic poll (refreshPending, while the
        // Inactive group is open) picks up the real disappearance once
        // sync_worker_tick finishes it.
        accounts.update((list) =>
          list.map((x) => (x.id === accountId ? { ...x, pending_removal: true } : x))
        );
      } else {
        const remaining = $accounts.filter((x: Account) => x.id !== accountId);
   ...
   *
   * Not an OAuth flow: `icloud_sign_in` opens Apple's own pages in a webview
   * and does not resolve until the session is complete, the user closes the
   * window, or it times out. On success it emits `oauth-success`, so the
   * listener below does the same post-add refresh every other backend gets —
   * no second refresh path to keep in step.
   *
   * A rejection is a real outcome worth showing, not a lost callback: a
   * cancelled window, or an account with Advanced Data Protection on, where
   * the backend refuses before creating anything and the message names ADP and
   * what to do about it.
   */
  // The post-add refresh is NOT optional here, and its absence was invisible
  // because everything else worked. `icloud_sign_in` creates the account,
  // indexes it and returns — and this function used to drop that on the floor,
  // so the sidebar kept rendering the account list it had loaded at startup.
  // The account existed in accounts.json, its notes were in SQLite, and the
  // screen showed neither until the app was restarted. Clicking the menu item
  // again then produced "one iCloud account at a time" about an account the
  // user could not see, which is how this was found.
  //
  // **Gotcha #6, for the fourth time**: state the backend changes needs a
  // route back to the frontend, and reciting the rule is not applying it.
  // `addLocalFolder` carries the same three lines with a comment saying it
  // mirrors the oauth-success flow; this one never got them. When a second
  // caller re-implements a shared boundary, diff the pair.
  async function addICloud() {
    accountPanelOpen = false;
    try {
      console.log('[jodd] icloud_sign_in: invoking');
      await invoke<string>('icloud_sign_in');
      console.log('[jodd] icloud_sign_in: reply received');
      // Mirror the oauth-success post-add flow: refetch accounts, switch to
      // the new one, then refresh notes + folders.
      const list = await invoke<Account[]>('list_accounts');
      accounts.set(list);
      // By id rather than by diffing the list, because sign-in is idempotent:
      // it succeeds without adding a row when the account is already there
      // (a resumed sign-in), and a diff finds nothing to select in that case.
      // `icloud_sign_in` resolves to the Apple ID, and `account_id_for` builds
      // the id as `{backend}:{email}` — see accounts.rs.
      const icloud = list.find((a) => a.backend_kind === 'icloud');
      if (icloud) currentAccount.set(icloud.id);
      $refreshNotes();
      refreshFolders();
    } catch (e) {
      alert(`iCloud sign-in did not complete: ${e}`);
    }
  }
  ...
  onMount(() => {
    const unlisten = (async () => {
      ...
      return await listen('oauth-success', async () => {
        // Logged rather than assumed: this listener firing is the difference
        // between "the event never arrived" and "it arrived and the refresh
        // failed", and on a release build there is no devtools to ask.
        console.log('[jodd] oauth-success received — refetching accounts');
        try {
          const list = await invoke<Account[]>('list_accounts');
          accounts.set(list);
...
<aside class="sidebar" style="width: {width}px; min-width: {width}px;">
  ...
  <nav class="folder-list">
    {#each $activeAccounts as acct (acct.id)}
      <!-- data-account-id is the only thing that tells two rows apart now that
           two accounts can share an email (gmail:a@b.com / microsoft:a@b.com):
           the address renders identically on both, so a test asserting on the
           sidebar's text alone cannot tell which row carries which backend
           label. See Sidebar.test.ts. -->
      <div class="account-section" data-account-id={acct.id}>
        <div class="account-header" title={accountDisplay(acct)}>
          ...
          {#if acct.backend_kind}
            <span class="account-backend-tag">{backendLabel(acct.backend_kind)}</span>
          {/if}
          <button
            type="button"
        ...
        <!-- Views — the read-only virtual entries, grouped under one header so
             ...
             rename/move/delete. Smart Folders (Orphaned, Stale, Extracts) are
             fully virtual with no folders-table row (design spec decision 3/4);
             Extracts lists every note carrying Extract's Source block,
             wherever it's filed (list_extract_notes). Recently Deleted reads
             Gmail Trash. They used to be styled exactly like real folders, and
             Recently Deleted sat outside any group entirely. -->
        <div class="folder-group">
          ...
          <div
            class="folder-item"
            class:active={$selectedSmartFolder?.account === acct.id && $selectedSmartFolder?.kind === 'extracts'}
            style="padding-left: 16px"
            role="button"
            tabindex="0"
            data-smart-folder="extracts"
            title="Notes made by Extract, wherever they are filed"
            onclick={() => selectSmartFolder(acct.id, 'extracts')}
            onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selectSmartFolder(acct.id, 'extracts'); } }}
          >
            <span class="folder-icon"><Icon name="bulb" /></span>
            <span class="folder-name">Extracts</span>
          </div>
  ...
  <div class="sidebar-footer">
    ...
    >
      ...
      {#if $activeAccounts.find((a) => a.id === $currentAccount)?.backend_kind}
        <span class="account-chip-backend">{backendLabel($activeAccounts.find((a) => a.id === $currentAccount)?.backend_kind ?? '')}</span>
      {/if}
    ...
    {#if accountPanelOpen}
      <div class="account-panel" role="menu">
        ...
        {#each $activeAccounts as a (a.id)}
          <div class="account-row" class:active={a.id === $currentAccount}>
            ...
            >
              ...
              {#if a.backend_kind}
                <span class="account-row-backend">{backendLabel(a.backend_kind)}</span>
              {/if}
            </button>
            <button
          ...
          {#if a.blocked_reason}
            <!-- Its own row under the account, not a tooltip or a badge: the
                 reason names a setting in Apple's own UI, and a message the
                 user has to hover to find is a message they will not read.
                 Backend-neutral by construction — the text is whatever the
                 backend put in `blocked_reason`. -->
            <p class="account-row-blocked" role="status">{a.blocked_reason}</p>
          {/if}
        {/each}
            ...
            {#each dismissedAccounts as a (a.id)}
              <div class="account-row dimmed">
                ...
                {#if a.status === 'draining'}
                  ...
                  {#if a.pending_removal}
                    <span class="account-row-note">removal queued</span>
                  {:else}
                    <button type="button" class="account-row-remove" onclick={() => removeAccount(a)}>Remove</button>
                  {/if}
                {:else}
        ...
        <!-- Every platform, Android included: Microsoft redirects through the
             same App Links URL Gmail uses (`auth_ms::redirect_uri`), so the
             loopback listener that used to keep it off Android is no longer
        ...
        {#if $supportsIcloud}
          <!-- Every desktop platform, not just macOS: `data_store_identifier`
               is Apple's isolation lever and `data_directory` is everyone
               else's (icloud_auth's `isolated()`), measured identical on both
               engines 2026-09-09. Android is the exclusion.

               Its own function, not `addAccount('icloud')`: there is no OAuth
               here at all — no url to open, no browser, no redirect — so it
               shares none of that path beyond the name. -->
          <button class="account-panel-action" onclick={addICloud}>
            <span class="icon">＋</span>
            <span class="label">Add iCloud account</span>
          </button>
        {/if}
        {#if !$isAndroid}
...
{#if menuPath !== null && menuAccountId !== null}
  ...
  >
    ...
    {:else if menuPath !== 'Notes'}
      <!-- Active workflow folders (matching __name__ pattern, e.g. __Claude__):
           ...
           __Extracts__ retired with extract filing (2026-09-15) and is
           EXCLUDED from this branch by RETIRED_WORKFLOW_FOLDERS — it falls
           through to the normal rename/move/delete branch above, same as the
           legacy folders below. __Claude__ stays here: jodd-mcp uses it as
           its write allowlist, so renaming it would silently break MCP
           writes.
...
<style>
  ...
  /* Named unconditionally beside the address, not only on collision —
     Account.id is now `{backend}:{email}`, so two accounts CAN share an
     address on different backends and the address alone can't tell them
     apart. Muted styling matches .dup-pill.result: informational, not
     interactive. */
  .account-backend-tag,
  .account-chip-backend,
  .account-row-backend {
    font-size: var(--size-xs);
    color: var(--text-muted);
  ...
  .account-row-settings {
    ...
    font-size: var(--size);
    cursor: pointer;
    /* Visible by default, and hidden only below where a pointer can reveal
       them again. The hiding used to live here, unconditionally, and a touch
       screen has no hover to undo it — so on Android both controls were
       invisible for good. `opacity: 0` hides paint and not hit-testing, which
       made the worse half: a live Remove-account button nobody could see, in a
       fixed spot at the end of every row.

       Stated in this direction on purpose. If `hover: hover` is ever
       unsupported or mis-evaluated, the fallback is a visible button rather
       than an invisible destructive one — the failure that costs a user an
       account is the one worth designing against. */
    opacity: 1;
  ...
  /* Pointer devices keep the reveal-on-hover affordance: the row stays quiet
     until it is pointed at. `hover: hover` asks the device what it can do,
     rather than inferring it from viewport width — a touch laptop and a phone
     both answer honestly, and a narrow desktop window is not mistaken for a
     phone. */
  @media (hover: hover) {
    .account-row-remove,
    .account-row-settings {
  ...
  /* Wraps, unlike `.account-row-note`: this one is a sentence naming a
     setting in Apple's own UI, and truncating it to one line would cut off
