<script lang="ts">
  ...
  // Per-account collapse of the Tags section (space-saving — the tag pills can
  // get long). Collapsed account ids live in this set.
    ...
    {#each $accounts as acct (acct.id)}
      <div class="account-section">
        ...
        >
          ...
          <span class="folder-name">All {accountDisplay(acct)}</span>
        ...
        {#snippet folderRow(row: FolderRow, allRows: FolderRow[], isWorkflow: boolean)}
          {#if isRowVisible(row.path, expandedPaths)}
            ...
            >
              ...
              {/if}
              <span class="folder-icon">{isWorkflow ? '💡' : '📁'}</span>
              <span class="folder-name">{stripWorkflowMarkers(row.name)}</span>
        ...
        {#if splitRowsByAccount[acct.id]}
          ...
          {/if}
        {/if}
        <!-- Recently Deleted — reads from Gmail Trash (Apple's Recently Deleted
             over this backend). selectedFolder = "__TRASH__". From origin/main;
             rendered after both folder groups so it lives below the Workflows
             section. -->
        <div
          class="folder-item"
  ...
  <div class="sidebar-footer">
    ...
    >
      ...
      <span class="account-chip-email">
        {accountDisplay($accounts.find((a) => a.id === $currentAccount), $currentAccount ?? 'No account')}
      </span>
        ...
        {#each $accounts as a (a.id)}
          <div class="account-row" class:active={a.id === $currentAccount}>
            ...
            >
              ...
              {#if a.backend_kind === 'local_fs'}<span class="account-row-kind-icon">📁</span>{/if}
              <span class="account-row-email">{accountDisplay(a)}</span>
            </button>
            <button
              class="account-row-settings"
              onclick={() => { currentAccount.set(a.id); extractModalOpen.set(true); accountPanelOpen = false; }}
              title="Extract lessons from a source"
              aria-label="Extract lessons"
            >💡</button>
            <button
              class="account-row-settings"
...
{#if menuPath !== null && menuAccountId !== null}
  ...
  >
    ...
    {#if menuPath !== 'Notes' && !isProtectedWorkflowFolder(folderKindsByAccount[menuAccountId!] ?? {}, menuPath!)}
...
<style>
  ...
  .tags-header:hover { color: #8a8270; }
  .tags-caret { font-size: 8px; width: 8px; }
  .tags-num {
    font-size: 9px;
    color: #b0a99a;
    background: rgba(0, 0, 0, 0.06);
  ...
  .tag-pill:hover { background: rgba(0, 0, 0, 0.1); }
  .tag-pill.active { background: #f3e4c0; color: #3a2f12; }
  .pill-name {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  ...
  .settings-gear:hover { color: #5a5a52; }
  .version-label {
    display: block;
    text-align: center;
    padding: 4px 0;
