<script lang="ts">
  ...
  import { notes, selectedFolder, selectedNote, isAuthenticated, accounts, activeAccounts, currentAccount, refreshNotes, noteIndex, hydratedFolders, indexRewriteOnFolderRename, selectedTags, tagMatchMode, toggleSelectedTag, clearSelectedTags, tagsByAccount, renameTagInStore, deleteTagFromStore, setAccountNoteTags, searchQuery, accountDisplay, selectedSmartFolder } from '../stores/notes';
  ...
  // and both must sum the SAME five fields (a field missed in one place would
  // read as "nothing queued" exactly where that is most dangerous).
  type PendingPushes = { notes: number; deletes: number; pins: number; tags: number; folders: number };
  const totalPending = (c: PendingPushes): number =>
    c.notes + c.deletes + c.pins + c.tags + c.folders;
  ...
  $: if ($activeAccounts.length > 0 && !expandedPaths['Notes']) {
