...
describe('⌘N on a backend that cannot write notes (Microsoft, still all-false in M2)', () => {
  // App.svelte's global ⌘N handler calls $newNoteFn() with no capability
  // check of its own — it trusts newNote() (NoteList.svelte) to refuse.
  // Before this fix that trust was misplaced: newNote() always inserted a
  // `tmp:` row into the `notes` store, so a note-write-refused account got a
  // note that renders in the list, can never be pushed (refuse_write in
  // lib.rs rejects it on the next sync tick), and never goes away — a ghost
  // row that reads as data loss. This pins newNote() refusing locally,
  // before any IPC, mirroring canWriteAccount.test.ts's cases.
  it('refuses to create a note when the account cannot write', () => {
    currentAccount.set('ms:readonly@example.com');
    capabilitiesByAccount.set({
      'ms:readonly@example.com': {
        has_trash: false,
  ...
  it('still creates a note when the account can write', () => {
    currentAccount.set('localfs:testvault');
    capabilitiesByAccount.set({
      'localfs:testvault': {
        has_trash: true,
        writes: { notes: true, relocate: true, folders: true, sidecars: true },
      },
    });
    mountList();
    get(newNoteFn)();
  ...
  // Optimistic-while-loading: capabilities for this account have not arrived
  // yet (no entry in the map at all). Must not lock the user out — same
  // no-flicker contract as canWriteAccount(undefined) === true.
  it('still creates a note while capabilities are undefined', () => {
    currentAccount.set('localfs:testvault');
    capabilitiesByAccount.set({});
    mountList();
    get(newNoteFn)();
