<script lang="ts">
  ...
  import { accounts, notes, selectedNote, selectedFolder, refreshNotes, currentAccount, indexRemoveOnDelete, indexUpsertOnSave, noteTagsByAccount, getNoteTags, setNoteTags, accountDisplay } from '../stores/notes';
  ...
  async function reExtractLessons() {
    ...
    try {
      ...
      await invoke<string>('re_extract_lessons', {
        accountId,
        uuid: note.uuid,
        requestId,
      });
        ...
        <div class="submenu-anchor" role="none" onmouseenter={fitSubmenu}>
          <button
            ...
            title={!acctEnabled
              ...
              : accountDisplay($accounts.find((a) => a.id === acct), acct)}
            type="button"
          >
            <span class="icon">👤</span>
            <span class="label">{accountDisplay($accounts.find((a) => a.id === acct), acct)}</span>
            <span class="chevron">▸</span>
          </button>
          {#if acctEnabled}
