<script lang="ts">
  import { deleteNoteOptimistic, beginNoteDeletion, deletingNotes } from '../deleteNote';
  import { noteKey, sameNote } from '../noteIdentity';
  import { invoke } from '@tauri-apps/api/core';
  import { pendingDeletedNoteKeys, noteMutationRevision, error, activeAccounts, notes, selectedNote, selectedFolder, refreshNotes, currentAccount, indexRemoveOnDelete, indexUpsertOnSave, noteTagsByAccount, getNoteTags, setNoteTags, setAccountNoteTags, accountDisplay, backendLabel, capabilitiesByAccount, canWrite, needsPermanentDeleteConfirm } from '../stores/notes';
  ...
  async function refetchFromRemote() {
    ...
    try {
      ...
      notes.update((ns) => {
        const idx = ns.findIndex((n) => sameNote(n, fresh));
        if (idx >= 0) ns[idx] = fresh;
        return ns;
      });
      if (sameNote($selectedNote, fresh)) selectedNote.set(fresh);
    } catch (e) {
  ...
  async function togglePinOne() {
    ...
    notes.update((ns) =>
      ns.map((n) => (sameNote(n, { account_id: accountId, uuid: targetUuid }) ? { ...n, pinned: nextPinned } : n)),
    );
    selectedNote.update((cur) =>
      cur && sameNote(cur, { account_id: accountId, uuid: targetUuid }) ? { ...cur, pinned: nextPinned } : cur,
    );
    ...
    try {
      ...
      rollbackFields(prevNotes, prevSelectedNote, ['pinned'], n => sameNote(n, { account_id: accountId, uuid: targetUuid }));
      error.set(`Failed to ${nextPinned ? 'pin' : 'unpin'} note: ${e}`);
    }
  }

  function rollbackFields(before: Note[], selected: Note | null, fields: ('pinned' | 'label')[], matches: (n: Note) => boolean) {
    const restore = (n: Note) => {
      const old = before.find(x => sameNote(x, n)) ?? (sameNote(selected, n) ? selected : null);
      if (!old || !matches(n)) return n;
      return { ...n, ...Object.fromEntries(fields.map(f => [f, old[f]])) };
    };
  ...
  async function setPinBatch(nextPinned: boolean) {
    ...
    notes.update((ns) =>
      ns.map((n) => (n.account_id === accountId && uuidSet.has(n.uuid) ? { ...n, pinned: nextPinned } : n)),
    );
    selectedNote.update((cur) =>
      cur && cur.account_id === accountId && uuidSet.has(cur.uuid) ? { ...cur, pinned: nextPinned } : cur,
    );
    ...
    try {
      ...
      rollbackFields(prevNotes, prevSelectedNote, ['pinned'], n => n.account_id === accountId && uuidSet.has(n.uuid));
      error.set(`Failed to ${nextPinned ? 'pin' : 'unpin'} ${uuids.length} note(s): ${e}`);
    }
  }
  ...
  async function deleteNote() {
    ...
    try {
      ...
      const pending = deleteNoteOptimistic({ ...note }, accountId);
      onClose();
      await pending;
      return;
    } catch (e) {
  ...
  async function deleteBatch() {
    ...
    batch.forEach(beginNoteDeletion);
    pendingDeletedNoteKeys.update(s => new Set([...s, ...batch.map(noteKey)]));
    notes.update((ns) => ns.filter((x) => !(x.account_id === accountId && uuids.includes(x.uuid))));
    if ($selectedNote && $selectedNote.account_id === accountId && uuids.includes($selectedNote.uuid)) {
    ...
    const liveById = new Map(prevNotes.map((n) => [noteKey(n), n.id] as const));
    for (const n of batch) indexRemoveOnDelete(accountId, liveById.get(noteKey(n)) || n.id);
    ...
    try {
      ...
      notes.update(ns => [...ns, ...prevNotes.filter(n => n.account_id === accountId && uuids.includes(n.uuid) && !ns.some(x => sameNote(x, n)))]);
      if (!get(selectedNote) && prevSelectedNote?.account_id === accountId && uuids.includes(prevSelectedNote.uuid)) selectedNote.set(prevSelectedNote);
      for (const [u, tags] of tagSnap) setNoteTags(accountId, u, tags);
      error.set(`Failed to delete ${batch.length} note(s): ${e}`);
    } finally {
      queueMicrotask(() => batch.forEach(n => deletingNotes.delete(noteKey(n))));
      noteMutationRevision.update(n => n + 1);
      pendingDeletedNoteKeys.update(s => new Set([...s].filter(k => !batch.some(n => noteKey(n) === k))));
    }
  }
  ...
  async function moveBatchTo(target: string) {
    ...
    notes.update((ns) =>
      ns.map((n) =>
        n.account_id === accountId && movingUuids.includes(n.uuid) ? { ...n, label: target } : n,
      ),
    );
    selectedNote.update((cur) =>
      cur && cur.account_id === accountId && movingUuids.includes(cur.uuid) ? { ...cur, label: target } : cur,
    );
    ...
    try {
      ...
      for (const n of batch) {
        if (n.account_id === accountId && movingUuids.includes(n.uuid)) {
          indexUpsertOnSave(accountId, n.id || null, n.id || '', target);
        }
      }
    } catch (e) {
      console.error('batch move failed', e);
      rollbackFields(prevNotes, prevSelectedNote, ['label'], n => n.account_id === accountId && movingUuids.includes(n.uuid));
      error.set(`Failed to move ${batch.length} note(s): ${e}`);
    }
  }
