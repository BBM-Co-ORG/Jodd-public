<script lang="ts">
  ...
  // (`save_note`/`re_extract_note`/`apply_wiki_link_appends`); delete and
  // move are `Write::Relocate` (`delete_note`/`delete_notes_batch`/
  // `move_notes_batch`) — split out from `notes` because iCloud can do these
  // without being able to edit content (M2 relocation live pass,
  // 2026-08-24: move/trash/restore never send the document, confirmed on
  // three independent Apple surfaces). Re-extract additionally needs
  // `Write::Folders` (`ensure_workflow_folder`), which is refused on
  // Microsoft (folders never reach Apple Notes, measured 2026-08-14/15) —
  // so re-extract is unavailable there even though plain note writes are
  // not. Using one coarse gate here would show Pin as available on
  ...
  async function moveTo(target: string) {
    ...
    // Snapshot before onClose() — Svelte 5's prop reactivity resolves
    // `note` to null once this menu unmounts.
    ...
    try {
      ...
      // The remote id doesn't change here — a move is a pure label rewrite
      // in SQLite, same reasoning as moveBatchTo's own comment below.
      indexUpsertOnSave(accountId, prevId || null, prevId || '', target);
    } catch (e) {
