<script lang="ts">
  ...
  function onEditorKeydown(e: KeyboardEvent) {
    ...
    if (mod && !e.altKey) {
      // Same root cause as the Cmd+B/I/U note below: no native Edit menu means
      // no key-equivalent routing, so WebKit's built-in undo/redo chord never
      // fires on its own — must call execCommand directly. Cmd/Ctrl+Shift+Z is
      // the universal redo chord; Ctrl+Y is also accepted since it's the
      // Windows-native redo convention (Jodd's primary target platform).
      ...
      if (!e.shiftKey && e.code === 'KeyY') { e.preventDefault(); formatRedo(); onInput(); return; }
  ...
  $: if ($selectedNote) {
    ...
    externalChangePending = nextExternalChangeBanner({
      ...
      bodyChanged,
      isEcho: $selectedNote.body_html === lastRemoteBody,
    });
  ...
  <div class="editor-pane">
    <div class="editor-toolbar">
      ...
      <div class="toolbar-actions">
        ...
        <button class="fmt-btn" onmousedown={(e) => e.preventDefault()} onclick={formatLink} title="Link (Cmd/Ctrl+K)" aria-label="Link (Cmd/Ctrl+K)"><Icon name="link" size={14} /></button>
        <button class="fmt-btn" onmousedown={(e) => e.preventDefault()} onclick={formatHeading} title="Heading 2 (Cmd/Ctrl+Alt+2)">H</button>
        <button class="fmt-btn" onmousedown={(e) => e.preventDefault()} onclick={formatBlockquote} title="Blockquote (Cmd/Ctrl+Shift+.)" aria-label="Blockquote (Cmd/Ctrl+Shift+.)"><Icon name="quote" size={14} /></button>
        ...
        <button class="fmt-btn" onmousedown={(e) => e.preventDefault()} onclick={formatTask} title="Task / Checklist (Cmd/Ctrl+Shift+9)" aria-label="Task / Checklist (Cmd/Ctrl+Shift+9)"><Icon name="checkbox" size={14} /></button>
        ...
        <div class="graph-modal cheat-modal" onclick={(e) => e.stopPropagation()}>
          ...
          <table class="cheat-table">
            <tbody>
              ...
              <tr><td>Undo</td><td><kbd>Cmd/Ctrl</kbd>+<kbd>Z</kbd></td></tr>
              <tr><td>Redo</td><td><kbd>Cmd/Ctrl</kbd>+<kbd>Shift</kbd>+<kbd>Z</kbd> or <kbd>Ctrl</kbd>+<kbd>Y</kbd></td></tr>
