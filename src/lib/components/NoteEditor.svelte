<script lang="ts">
  ...
  async function refreshCitations() {
    ...
    try {
      ...
      if (seq === citeSeq) citations = res;
    } catch (e) {
      if (seq === citeSeq) citations = [];
    }
  }
  function openConnection(n: Note) {
    ...
    notes.update((ns) => {
      const i = ns.findIndex((x) => x.uuid === n.uuid);
      if (i >= 0) ns[i] = n;
      else ns.push(n);
      return ns;
    });
  ...
  async function autoSave() {
    ...
    const ok = await saveNote($selectedNote, title, bodyHtml);
    ...
    try {
      ...
      notes.update(ns => {
        ...
        const isThisNote = (n: Note) =>
          n.uuid === saved.uuid || n.uuid === note.uuid || (tmpUuidToReplace !== null && n.uuid === tmpUuidToReplace);
        const idx = ns.findIndex(isThisNote);
        if (idx < 0) return [updatedNote, ...ns];
        ...
        collapsed[collapsed.findIndex(isThisNote)] = updatedNote;
        return collapsed;
      });
  ...
  async function retrySync() {
    ...
    selectedNote.update((n) => (n ? { ...n, push_blocked_reason: null } : n));
    try {
  ...
  async function deleteNote() {
    ...
    if (!target.uuid || target.uuid.startsWith('tmp:')) {
...
{#if $selectedNote}
  <div class="editor-pane">
    ...
    {#if connections.outgoing.length || connections.backlinks.length || citations.length}
      <div class="connections">
        ...
        {#if citations.length}
          <div class="conn-group">
            <span class="conn-label"><Icon name="paperclip" size={12} /> Sources</span>
            {#each citations as url (url)}
              <a class="conn-link" href={url} title={url} target="_blank" rel="noopener">{url}</a>
            {/each}
          </div>
        {/if}
      </div>
