<script lang="ts">
  ...
  import { notes, selectedFolder, selectedNote, isLoading, refreshNotes, currentAccount, accounts, selectedUuids, clearSelectedUuids, selectedTags, tagMatchMode, toggleSelectedTag, noteTagsByAccount, getNoteTags, searchQuery, accountDisplay } from '../stores/notes';
  ...
  $: currentAccountDisplay = accountDisplay($accounts.find((a) => a.id === $currentAccount), $currentAccount ?? '');
...
<div class="note-list" style="width: {width}px; min-width: {width}px;">
  ...
  {#if $searchQuery}
    <div class="search-scope">
      <span class="scope-label">ค้นใน:</span>
      <select class="scope-select" bind:value={searchScope}>
        <option value="folder">โฟลเดอร์นี้</option>
        <option value="account">บัญชีนี้</option>
        <option value="all">ทุกบัญชี</option>
      </select>
    </div>
  {:else if $selectedTags.size > 0}
    <div class="search-scope">
      <span class="scope-label">🏷️ {[...$selectedTags].map((t) => '#' + t).join(' ')} — ค้นใน:</span>
