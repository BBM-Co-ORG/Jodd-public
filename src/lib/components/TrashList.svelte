<script lang="ts">
  ...
  // days invites them to treat a permanent file as self-cleaning. The row
  // only renders when `has_trash` is true (Sidebar's capability check), which
  // already excludes Microsoft's hard delete — but that leaves two backends
  // reaching this component, not one, and it named only the first: every
  // account, iCloud included, was told "Gmail clears these", which is simply
  // false on an account that has never touched Gmail.
  $: backendKind = $activeAccounts.find((a) => a.id === $currentAccount)?.backend_kind ?? 'gmail';
...
<div class="note-list" style="width: {width}px; min-width: {width}px;">
  ...
  <p class="hint">
    ...
    {:else if backendKind === 'icloud'}
      Apple clears these after about 30 days.
    {:else}
...
{#if menuOpen && menuItem}
  ...
  <div class="context-menu" style="left: {menuX}px; top: {menuY}px" role="menu" onclick={(e) => e.stopPropagation()}>
    <!-- Plain "Restore" means "put it back where it was", so it only exists
         when the backend actually knows where that was. On iCloud it does not:
         a trashed record's folder reference has been replaced by the Trash's.
         Offering the button anyway would file every restored note in the root
         and call that a restore. See TrashedNote.original_known. -->
    {#if menuItem.original_known}
    ...
    {#if folders.length > 0}
      <div class="submenu-anchor">
        <button class="item has-submenu" type="button">
          <span class="icon"><Icon name="folder" size={14} /></span><span class="label">{menuItem.original_known ? 'Restore to…' : 'Restore into…'}</span><span class="chevron"><Icon name="chevron-right" size={12} /></span>
        </button>
        <div class="submenu">
          {#each folders as f (f)}
