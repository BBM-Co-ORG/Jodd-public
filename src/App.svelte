<script lang="ts">
  import { createRefreshQueue } from "./lib/controllers/refreshQueue";
  import { noteKey, sameNote } from './lib/noteIdentity';
  import { refreshNotePersistence } from './lib/notePersistence';
  ...
  import SignInBlocked from './lib/components/SignInBlocked.svelte';
  ...
  import type { PushConfirmation } from './lib/editorRenderDecision';
  import { isAuthenticated, notes, isLoading, isSaving, error, refreshNotes, accounts, activeAccounts, currentAccount, selectedNote, selectedFolder, recentlySavedUuids, recentSaveTimestamp, noteIndex, requestFolderRefresh, hydratedFolders, markFolderHydrated, selectedTags, setAccountNoteTags, selectedSmartFolder, smartFolderNotes, newNoteFn, lastPushConfirmation, type SmartFolderKind } from './lib/stores/notes';
  import { smartFolderCommand } from './lib/smartFolders';
  import { extractModalOpen, whatsNewOpen, whatsNewVersions, appSettingsOpen, askModalOpen, signInBlocked } from './lib/stores/ui';
  import { routeOauthError, type OauthErrorPayload } from './lib/signInBlocked';
  ...
  const refreshQueue = createRefreshQueue(() => $isSaving, e => error.set(String(e)));
  ...
  function scheduleRefresh(fn: () => Promise<void>) {
    ...
    refreshQueue.schedule(fn);
  }

  function drainQueue() { refreshQueue.drain(); }
  ...
  $: if ($selectedFolder !== lastFolderSeen) {
    ...
    if ($isAuthenticated) {
      ...
      if (f !== '__TRASH__') {
        if (acctNow && f === '__ALL__') {
          // The "All <account>" row filters $notes by account rather than by
          // label — so what it shows is whatever the last folder happened to
          // load, and clicking it appeared to do nothing at all. It needs the
          // account painted, not a folder.
          paintAccountFromCache(acctNow);
        } else if (acctNow && f) {
          // Immediate cache paint — does not touch the remote.
  ...
  // navigation exists. The one path it cannot see is a tap on the note that
  // is ALREADY selected (no change to observe), so NoteList's plain click
  // also calls navigateToPane('note'). Clearing the selection (uuid → null, e.g. a note
  ...
  $: {
    const uuid = $selectedNote ? noteKey($selectedNote) : null;
  ...
  async function loadSmartFolderNotes(accountId: string, kind: SmartFolderKind) {
    try {
      const command = smartFolderCommand(kind);
  ...
  // The "All <account>" counterpart of paintFolderFromCache. Same doctrine —
  // one local SQLite read, no remote — but scoped to the account rather than
  // to a label, because the All row has no label to scope by.
  //
  // Until this existed, selecting All left $notes holding only the folder the
  // user came from, so the list did not change and the header count disagreed
  // with the sidebar. The sidebar was right: its number comes from the index,
  // which knows the whole account.
  async function paintAccountFromCache(accountId: string) {
    try {
      const cached = await invoke<any[]>('list_cached_notes', { accountId });
      const cachedUuids = new Set(cached.map((n: any) => n.uuid));
      const recents = get(recentlySavedUuids);
      const cutoff = Date.now() - 30_000;
      notes.update((ns) => {
        // Everything belonging to OTHER accounts is untouched — the All view
        // is per-account, and other accounts' notes are not ours to drop.
        const others = ns.filter((n) => n.account_id !== accountId);
        const mine = ns.filter((n) => n.account_id === accountId);
        const survivors = mine.filter(
          (n) =>
            n.uuid &&
        ...
        return [...others, ...survivors, ...cached];
      });
      reconcileSelection(get(notes));
    } catch (e) {
      // Same posture as paintFolderFromCache: a failed local read is
      // exceptional and the next navigation retries. Never an error banner
      // for something the cache recovers from on its own.
      console.error('paintAccountFromCache failed', e);
    }
  }
  ...
  function requestRefresh(source: 'focus' | 'poll' | 'manual' | 'folder' | 'remote') {
  ...
  async function startFocusPolling() {
    ...
    // The sync worker's change detector found a real remote change and dropped
    // the cached read (see `detect_remote_changes` / `frontend_wakeup_needed`).
    // Dropping a cache is invisible from here: nothing re-reads through it
    // until the 10-minute poll comes round, so an edit made in Apple Notes sat
    // unseen for up to ten minutes with the backend having known about it the
    // whole time.
    //
    // The payload is an account id and nothing else. It is a nudge, not a
    // delivery — the authoritative read this schedules is the same one the
    // poll performs, which is what keeps this from becoming a second, thinner
    // copy of the pull path. Routed through requestRefresh rather than
    // scheduleRefresh so the throttle covers several accounts detecting in the
    // same tick.
    const unlisten4 = await listen<string>('remote-changed', (e) => {
      console.log(`[jodd] remote change on ${e.payload} — refreshing`);
      requestRefresh('remote');
    });

    // A receipt, not a nudge — so unlike `remote-changed` it DOES carry
    // content. It names the bytes the backend accepted, which is the one
    // thing the editor cannot re-read for itself: the cache row holds
    // whatever the note looks like NOW, and a local edit landing mid-push has
    // already moved past what actually went out. Deliberately does not
    // schedule a refresh; nothing on screen is stale because of a push.
    const unlistenPersistence = await listen<{ accountId: string; uuid: string }>('note-persistence-changed', (e) => {
      void refreshNotePersistence({ account_id: e.payload.accountId, uuid: e.payload.uuid });
    });
    const unlisten5 = await listen<PushConfirmation>('note-pushed', (e) => {
      lastPushConfirmation.set(e.payload);
    });
  ...
  onMount(async () => {
    ...
    // Payload gained a second field on 2026-08-17 (see `OauthError` in
    // src-tauri/src/lib.rs): a Microsoft refusal may carry the URL an IT
    // administrator needs, which a bare string could not.
    await listen<OauthErrorPayload>('oauth-error', (event) => {
      const routed = routeOauthError(event.payload);
      if ('panel' in routed) signInBlocked.set(routed.panel);
      else error.set(routed.errorText);
    });
  ...
  function onVisibility() {
    ...
    // Becoming visible again is a repair point, not just a flush point.
    //
    // On Android the iCloud sign-in page does not sit *over* Jodd — wry
    // attaches it with `activity.setContentView`, which detaches Jodd's own
    // webview from the view hierarchy (gotcha #31). A detached WebView is
    // hidden as far as the page is concerned, and 0.28.2 shipped with the
    // symptom that proves something is lost across that gap: a signed-in
    // iCloud account was written to accounts.json, indexed, and announced with
    // `oauth-success`, and the sidebar still showed one account until the app
    // was restarted.
    //
    // Whatever was missed — the awaited reply, the event, or both — the cure
    // is the same and is cheap: re-read the authoritative list from SQLite the
    // moment the page is back on screen. `refreshAccounts` is a local command,
    // so this costs no network and runs on every platform, where it is simply
    // a no-op that re-confirms what is already displayed.
    console.log('[jodd] page visible again — re-reading accounts');
    refreshAccounts().catch((e) => console.warn('visibility refresh failed', e));
  }
  ...
  async function selectNoteByUuid(accountId: string, uuid: string) {
    ...
    try {
      await refreshNotePersistence({ account_id: accountId, uuid });
      const cached = await invoke<Note[]>('list_cached_notes', {
        accountId,
      });
      const found = cached.find((n) => sameNote(n, { account_id: accountId, uuid }));
      if (!found) {
  ...
  async function sweepTick() {
    ...
    if (refreshQueue.running) return;    // full refresh has higher priority
  ...
  //     the note again.
  async function reconcileSelection(fetched: Note[]) {
    const cur = get(selectedNote);
    if (!cur) return;
    if (!cur.id) return; // new unsaved — preserve user's in-memory work
    const updated = fetched.find((n) => sameNote(n, cur));
    if (!updated) {
      // A worker rekey may precede its event. Resolve against SQLite before
      // interpreting a missing old UUID as deletion. Unknown/failed reads
      // preserve the editor; only authoritative absence clears selection.
      const status = await refreshNotePersistence(cur);
      const current = get(selectedNote);
      if (status === null && sameNote(current, cur) && current?.local_version === cur.local_version && !get(isSaving)) selectedNote.set(null);
      return;
    }
