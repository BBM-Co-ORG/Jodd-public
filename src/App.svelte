<script lang="ts">
  ...
  import { extractModalOpen, whatsNewOpen, appSettingsOpen } from './lib/stores/ui';
  ...
  onMount(async () => {
    ...
    // app version to the last value recorded in localStorage; surface every
  ...
  // Global hotkey: Cmd/Ctrl+Shift+L opens the lesson-extraction modal.
  // No other component listens for this combo (NoteEditor uses Cmd+F,
  // AccountSettings uses Cmd+Enter, NoteList uses Cmd+A) so we can
  // claim it at window scope without conflicts. Modal own onkeydown
