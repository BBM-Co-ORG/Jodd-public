  ...
  $: if ($isAuthenticated && !lastAuthed) {
    ...
    (async () => {
      ...
      try {
        ...
        // Re-paint from the cache so pinned notes show in the right order
        // immediately. loadCachedNotes is cheap (pure SQLite) and is the
        // single source of "what's in $notes."
        await loadCachedNotes();
      } catch (e) {
        console.warn('cold-start pin sync failed:', e);
      }
