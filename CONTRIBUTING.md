...
- **Platform support fixes** — macOS, Windows, or Android bugs and
  accessibility improvements.
- **Backend work behind the existing `Vertical` trait** — changes that
  preserve the shared transport/encoding/identity boundary are easier to
  review.

## What is harder to land without a discussion first

- **New backends** (Outlook/Graph, Yahoo, custom IMAP). New work must
  implement the existing `Vertical` trait without coupling provider logic to
  the sync worker or UI. Coordinate before starting.
...
- **Platform** (macOS / Windows / Android + version and device where relevant).
