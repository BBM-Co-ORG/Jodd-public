...
> **Superseded by the live site, and materially out of date. Read
> [jodd.bbmedia.co.th/privacy.html](https://jodd.bbmedia.co.th/privacy.html)
> instead.** That page is the policy; this file is a frozen 2026-08-13
> snapshot kept only as an in-repo reference, and it is now missing **two
> whole backends** that shipped after it — the Microsoft/Exchange backend
> (0.24.1) and the iCloud/CloudKit backend (0.25.0), each with its own
> access model and its own disclosures. Do not cite this file for what Jodd
> accesses today.
>
> Edit the policy in the website source
> ([BBM-Co-ORG/Jodd-Homepage](https://github.com/BBM-Co-ORG/Jodd-Homepage)),
> never here. The at-rest-encryption disclosure this banner used to list as
> outstanding on the live site has since been applied there.
...
**At-rest encryption.** For Gmail-synced accounts, notes stored in Jodd's
local database are encrypted at rest (AES-256, SQLCipher). The encryption
key is generated on your device and stored in your operating system's
secure credential store (macOS Keychain / Windows Credential Manager /
Android Keystore) — Jodd never transmits this key anywhere. This does not
apply to Local Folder vaults: those store notes as plain files in the
folder you choose, and protecting that folder (e.g. via full-disk
encryption) is your responsibility. Jodd discloses this plainly at the
point you choose to add a Local Folder vault, not only here.
