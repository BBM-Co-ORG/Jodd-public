...
> **Superseded in part (2026-09-15).** Decision 1 is **reversed** and
> Decision 4 is **superseded** by
> [URL ingest](2026-09-15-url-ingest-design.md), Decision 1: Jodd fetches web
> pages and YouTube transcripts itself, deterministically, and no provider is
> ever granted a tool. This spec's `UrlOnly` classification became
> `ingest::urls` + `is_mostly_urls`, and its disabled `[🔒 Let AI fetch it]`
> chip became the Extract modal's "Sources from links" section. Decisions 2,
> 3, 5, 6 and 7 are unaffected.
