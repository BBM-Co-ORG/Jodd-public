...
Jodd uses [`https://www.googleapis.com/auth/gmail.modify`](https://developers.google.com/workspace/gmail/api/auth/scopes)
which Google classifies as **"restricted"** (confirmed 2026-07-30 against
Google's live scope table — this doc previously said "sensitive," which was
wrong). It permits:
...
Restricted scopes require an annual CASA security assessment, but only for
apps that can access the data **"from or through a third-party server"**
(per Google's restricted-scope-verification docs). Jodd is local-first with
no backend server, so it may qualify for that exemption — but this is
unconfirmed until the app is actually submitted for verification, not
something the scope choice guarantees on its own. `gmail.modify` was still
the right choice over the broader `https://mail.google.com/` scope: it
excludes permanent delete and full-mailbox access, a narrower justification
to defend during verification regardless of which CASA tier Google
ultimately assigns. See [DISTRIBUTION.md](DISTRIBUTION.md) for the full
picture.
...
~/Library/Application Support/jodd/accounts.json
  ├── work@example.com      (added 2026-06-04T...)
  └── personal@example.com  (added 2026-06-04T...)

OS Keychain:
  ├── jodd / rt::work@example.com      → <refresh token>
  └── jodd / rt::personal@example.com  → <refresh token>
