...
## What has to exist in Azure (Microsoft accounts)

Microsoft uses the **same** redirect URL on Android as Gmail —
`https://jodd.bbmedia.co.th/oauth2redirect` — and the same page below. The app
tells the two flows apart by the backend it recorded when the flow started
(`pending_backend`, persisted beside the PKCE pair), not by the URL, so one
host, one path and one `intent-filter` serve both.

Unlike Google, no second client is needed: a Microsoft app registration
carries redirect URIs per *platform*, not per client type, so the one
`MS_CLIENT_ID` serves desktop and Android alike. Register the URL under:

**Microsoft Entra ID → App registrations → (the app) → Manage →
Authentication → Platform configurations → Mobile and desktop applications →
Add URI.**

Two rules, each of which fails in a way that does not name itself:

- **It must go under "Mobile and desktop applications", the block that already
  holds `http://localhost:8080` — not under a new "Web" platform.** Microsoft
  decides whether a token request comes from a public or a confidential
  client by which platform the redirect URI was registered on. A URI under
  "Web" makes the code exchange demand a `client_secret`
  (`AADSTS7000218`), and this is a public client with none.
- **Exact match, no trailing slash.** Microsoft compares the string as sent,
  and `auth_ms::redirect_uri` sends `ANDROID_REDIRECT_URI` verbatim.

Sign-in cancelled on the phone reaches the app the same way it does for Gmail
today, which is to say not at all: Microsoft redirects with
`?error=access_denied` and no `code`, the page below shows its plain "go back
to the app" state, and the app's own two-minute poll times out. Same
behaviour, same limit, deliberately not widened here.
