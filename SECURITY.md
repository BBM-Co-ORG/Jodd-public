...
- The Jodd macOS, Windows, and Android binaries and their source code in this
  repository.
...
- Issues that require physical access to an unlocked, signed-in
  device. The local SQLite cache **is** encrypted at rest (SQLCipher,
  AES-256), but the key lives in your operating system's credential
  store, which hands it to a process in your unlocked session exactly
  as it hands it to Jodd. At-rest encryption protects a database file
  copied off the device, not a session someone already has; full-disk
  encryption is the control for that. See
