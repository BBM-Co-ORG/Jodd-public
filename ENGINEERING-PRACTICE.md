...
`src-tauri/src/backend/microsoft/`. Builds 0.24.1 and earlier carry no
Microsoft OAuth client — `MS_CLIENT_ID` was read from the environment at
runtime and nothing embedded one at build time — so trying it on one of those
means supplying your own registered application's client id via the
environment (or `.env`) and restarting Jodd, not rebuilding it. Later builds
embed one and the variable still overrides it. See
[Status](README.md#status).)*

The same discipline applied to the question of whether to embed that client id
at all, and the answer came from a measurement rather than from reasoning about
it. The temptation was to treat "release builds can't sign in to Microsoft" as a
plumbing oversight and just wire it up. The thing worth knowing first was
whether the accounts that would then be able to try it can actually consent —
so we asked a non-admin user in an unrelated Microsoft 365 tenant to open the
consent URL, and Microsoft refused them: *"Need admin approval."* Two earlier
sign-in tests had succeeded and both turned out to have been admins, which is
why they proved less than they appeared to.

That negative did not reverse the decision — it clarified what the decision was
about. Consent is evaluated per organisation against the app's identity, so it
is indifferent to whether the client id was compiled in or read from the
environment; withholding it helps no blocked user and breaks every user who
*could* have consented. But it did change what we tell people, which is why the
limitation is written into the README next to the two Graph ones instead of
being left for someone to hit on their first launch.
