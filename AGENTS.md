...
A Tauri 2 desktop app (Svelte 5 + TypeScript frontend, Rust backend) that
...
network at all. Windows is the primary target, macOS for dev/test.

### Current state (as of v0.17.6)

- Apple Notes ↔ Gmail bidirectional sync via OAuth2 PKCE, **BYO credentials**
  (users supply their own Google OAuth client via App Settings or `.env`)
...
Three files must stay in sync when bumping: `package.json`,
`src-tauri/Cargo.toml`, `src-tauri/tauri.conf.json`. See `RELEASE.md`.
