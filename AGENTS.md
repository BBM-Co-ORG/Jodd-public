...
- Backend-agnostic trait surface (`Transport`/`NoteStore`/`Identity`/`Deriver`/
...
| `src-tauri/src/backend/mod.rs` | Backend-agnostic trait surface (`Transport`, `NoteStore`, `Identity`, `Deriver`, `MetadataSidecar`, `Vertical`) |
...
`CLAUDE.md` is the index. As of 2026-09-11 the detail it used to carry lives in
companion files, quoted verbatim, and the index links to each of them:
`docs/GOTCHAS.md` (the 31 live constraints — read the full entry, not just the
one-line trigger), `docs/BACKEND-ICLOUD.md`, `docs/BACKEND-MICROSOFT.md`,
`docs/ARCHITECTURE.md`, `docs/ROADMAP.md`, `docs/GRAPHIFY.md`.

- **What's done / not done** → `CLAUDE.md` roadmap index → `docs/ROADMAP.md`
- **Architecture & doctrine** → `CLAUDE.md` body, then `docs/ARCHITECTURE.md`
- **Constraints that will bite you** → `docs/GOTCHAS.md`
