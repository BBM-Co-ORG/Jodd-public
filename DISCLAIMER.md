...
production use. On a Gmail account, the synchronization model edits real
messages in your mailbox. On a Microsoft account, it edits real Exchange
note items in place. Neither is a copy Jodd owns — both are your own
account's live data. A Local Folder vault is different: there's no
external account to edit, only the files in the folder you chose.

**Before using Jodd against an account that contains data you care
about, back it up.**

- **Gmail:** select all messages under the `Notes` label, choose "Forward
  as attachment" to an archive address, or export via
  [Google Takeout](https://takeout.google.com/).
- **Microsoft:** export or copy your Notes folder through
  [outlook.live.com](https://outlook.live.com/mail/notes) before making
  bulk changes with Jodd.
- **Local Folder:** back up the folder itself — it's plain files, so any
  ordinary file backup (Time Machine, a copy to another drive, etc.)
  covers it completely.

On the Apple side, your iPhone/Mac maintains its own copy in iCloud —
but only for as long as the account stays signed in there, and this
doesn't apply to a Local Folder vault, which was never connected to
Apple Notes in the first place.
...
documented APIs** — the Gmail REST API and the Microsoft Graph API —
and the publicly observable storage format used by Apple Notes to
synchronize notes with these non-iCloud accounts. No proprietary
...
Jodd obtains OAuth authorization directly from Google when you sign in.
Official Developer Preview builds may embed BBMedia's OAuth client
configuration; source builds can use a developer's own client configuration.
In either case, access and refresh tokens are stored **only on your device**
— refresh tokens in the platform credential store and access tokens in
process memory. **There is no Jodd note-storage or note-sync server.** On
Android, `jodd.bbmedia.co.th` only hands the OAuth callback back to the app;
it does not receive or store your notes or tokens.

Notes themselves are cached in a **local SQLite database** in your
operating system's user data directory:

- **macOS**: `~/Library/Application Support/jodd/jodd.sqlite3`
- **Windows**: `%APPDATA%\jodd\jodd.sqlite3`
- **Linux source builds**: `~/.local/share/jodd/jodd.sqlite3`
- **Android**: the app-private data sandbox managed by Android

The local database is **encrypted at rest** (SQLCipher, AES-256)
as of v0.24.1. The key is machine-generated, never derived from a
passphrase, and stored in your operating system's credential store
(macOS Keychain, Windows Credential Manager, the Android keystore) —
not in the database and not in any file beside it.

**What that does and does not protect against.** It protects a
database file copied off the device — a stolen disk, a backup, a
misplaced drive. It does **not** protect against someone using your
already-unlocked user account, because the credential store will hand
that session the key exactly as it hands it to Jodd. For a shared or
unattended device, full-disk encryption (FileVault, BitLocker, LUKS)
is still the control that matters.

**Two things beside the database stay plaintext, both deliberately.**
`accounts.json` holds account metadata (address, backend kind) in the
clear. And a **Local Folder vault is plaintext by design** — at-rest
encryption covers the cache of a remote source of truth, whereas a
directly-readable folder of `.eml` files on a disk you chose is the
entire point of that backend; encrypting it would remove the property
...
1. **Direct observation of message format.** The Apple Notes ↔ Gmail
   ...
   engineering of a proprietary protocol. The same applies to the
   Microsoft Graph vertical: its behavior was likewise established by
   reading and writing a live, authenticated account's own Exchange
   notes and observing the result in Apple's own apps.
