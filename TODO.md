...
> Current shipped version: **v0.17.6**. For full architectural state and
...
- [ ] **Android version** — Tauri 2 supports it; needs OAuth redirect
  rework (no localhost callback on mobile).
...
- v0.16.x — backend trait extraction (`Transport`/`AtRest`/`Identity`/
  `Deriver`/`MetadataSidecar`/`Vertical`) out of the old monolithic
  `gmail.rs`; rename-safe `[[slug-uuid8]]` wikilinks
