...
//
// Run from src-tauri/ (lives in examples/ — see Cargo.toml comment for why
// this MUST NOT live in src/bin/):
//   cargo run --example test_checkbox             # uses first account in accounts.json
//   cargo run --example test_checkbox -- 1        # uses account at index 1
//
...
use jodd_lib::backend::gmail::wire as gmail;
...
"#;
    ...
    let saved = gmail::save_note(
        ...
        &account.email,
        &label_map,
        &[],                       // attachments (none for this test)
    )
    .await?;
