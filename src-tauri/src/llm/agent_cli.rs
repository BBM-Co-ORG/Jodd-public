...
impl LlmProvider for AgentCliProvider {
    async fn extract(
        ...
        self.run_json(SYSTEM_PROMPT, source, Some(ExtractEnvelope::JSON_SCHEMA), cancel)
            .await
    }
...
fn dig_unwrap(text: &str, field: &str) -> Result<String, String> {
    ...
    match found {
        ...
        // The field exists but is not a string. Whether `claude
        // --json-schema` returns `result` as a JSON string or a parsed
        // object/array is unmeasured — this machine's claude session cannot
        // authenticate, so nobody has exercised the effect of the schema
        // flag on the output shape. `claude` is Jodd's DEFAULT preset and is
