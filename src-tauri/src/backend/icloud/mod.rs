...
impl NoteStore for ICloudVertical {
    ...
    async fn save_note_full(
        ...
        let (write, previously_pinned) = match op.existing_remote_id.filter(|id| !id.is_empty()) {
            // ── update ──────────────────────────────────────────────────
            Some(id) => {
                ...
                // **Resolved only when the label actually changed.** A note
                // whose `Folder` names a record the walk never saw is filed
                // under the root for display (`decode_note`, counted as
                // `orphaned`), so re-resolving its label on every save would
                // write `DefaultFolder-CloudKit` onto it and MOVE it — a
                // silent reorganisation of the user's account performed by an
                // edit that changed a word. Measured `orphaned = 0` on the
                // live account, which is a reason to keep it that way, not a
                ...
                let base = base.clone();
                let doc =
                    compose::writability_with_refs(&base.document, &base.title_field, &scan.inline_refs)
                        .map_err(refused)?;
                ...
                // The title the body was CUT by, re-derived from the remote's
                // own text — not `TitleEncrypted`, which is a lossy derivation
                // of it (gotcha #21, and `doc::note_title`).
                let old_title = doc::note_title(doc.text(), &base.title_field);
                // One parse yields the plain text AND the formatting model,
                // so the two can never disagree (M3 F2).
                ...
                // **The dispatch the CarriesCrdtIdentity relaxation (M2.5)
                // needs and `with_text` alone cannot give it.** `writability`
                // now returns `Ok` for a document `doc.crdt: Some(_)` too —
                // `with_text` never learned about that (it only re-lengths
                // `attribute_run`, leaving `substring`/`timestamp` exactly as
                // they were), so calling it here on such a document would
                // silently produce a structurally invalid CRDT write (its own
                // `debug_assert!` catches this in a debug build rather than
                // let it reach the wire). `with_text_crdt` is the sibling
                // that actually understands the identity: same edit, but
                // spliced through `crdt::apply_text_edit`.
                ...
                // Formatting rides the same save (M3): rewrite the changed
                // paragraphs' runs, downgrade to text-only on refusal.
                Self::reconcile_formatting(&mut edited, &parsed_body, self.replica_id);
                // Title/snippet derive from the text with objects rendered
                // as their display text — a raw U+FFFC in TitleEncrypted is
                // a replacement glyph in Apple's list view (gotcha #21).
                let display_text = doc::text_with_objects_rendered(
                    &text,
