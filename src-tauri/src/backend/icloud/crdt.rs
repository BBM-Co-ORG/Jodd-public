...
mod tests {
    ...
    /// Reported live 2026-09-09: a note created from Jodd reached Apple Notes
    /// with a stray U+0E37 (Thai SARA UEE) in front of its title. The user had
    /// mistyped it, autosave pushed, they pressed backspace, and the second
    /// push never removed it. The character appears nowhere in the note's
    /// body, so a deleted typo surviving the splice is the only account of it.
    ///
    /// Thai only made it visible: a lone Thai vowel renders as a dotted circle
    /// and is impossible to miss, where a stray ASCII letter would have gone
    /// unnoticed. The mechanism under test is a **deletion at offset 0**, so
    /// these cases are ASCII except the one that reproduces the report.
    #[test]
    fn deleting_the_very_first_character_actually_deletes_it() {
        for (before, after) in [
            ("Xnote", "note"),
            ("\u{0e37}note in F1 from Jodd", "note in F1 from Jodd"),
            ("ab", "b"),
        ] {
            let mut doc = simple_document(before);
            apply_text_edit(&mut doc, after, REPLICA_B).unwrap();
            assert_eq!(doc.text, after, "doc.text after deleting the head of {before:?}");
            assert_eq!(
                visible_text(&doc),
                after,
                "VISIBLE text after deleting the head of {before:?}: a document whose own \
                 `text` is right while its runs still expose the old character is exactly \
                 what reached Apple"
            );
            validate_document_invariants(&doc).unwrap();
        }
    ...
    /// The same edit one character in, as a control: if this passes while the
    /// offset-0 case fails, the defect is specific to the head of the document
    /// rather than to deletion in general.
    #[test]
    fn deleting_a_character_after_the_first_also_deletes_it() {
        let mut doc = simple_document("aXnote");
        apply_text_edit(&mut doc, "anote", REPLICA_B).unwrap();
        assert_eq!(doc.text, "anote");
        assert_eq!(visible_text(&doc), "anote");
        validate_document_invariants(&doc).unwrap();
    }
