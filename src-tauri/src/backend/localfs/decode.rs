...
mod tests {
    ...
    // The body handed to `build_note_mime` is EDITOR-VIEW — no title row.
    // `inject_title_into_body` adds exactly one on the way to the wire and
    // `decode_eml` takes exactly that one back off; see the invariant on
    // `mime822::inject_title_into_body`. This fixture used to include the
    // title row itself, which only worked while inject short-circuited on it.
    #[test]
    fn build_then_decode_roundtrips_envelope() {
        let raw = crate::mime822::build_note_mime(
            "My Title",
            "<html><body><div>hello #t [[L-abcd1234]]</div></body></html>",
            "AAAA-BBBB",
    ...
    fn thai_title_and_body_roundtrip() {
        ...
        // Editor-view: no title row. `build_note_mime` injects one.
        let body_html = format!("<html><body><div>{}</div></body></html>", thai_body_text);
        let raw = crate::mime822::build_note_mime(
            thai_title,
            &body_html,
            "THAI-UUID-1234",
