...
impl NoteStore for MicrosoftVertical {
    ...
    async fn save_note_full(&self, op: &SaveOp<'_>, attachments: &[Attachment]) -> Result<SavedNote, TransportError> {
        ...
        let saved = match op.existing_remote_id {
            ...
            None => {
                // A cached id when there is one, the well-known name `notes`
                // when there is not and the destination is the root — see
                // `wire::WELL_KNOWN_NOTES_FOLDER` for the measurement, and
                // `wire::create_destination` for why a subfolder gets no such
                // fallback.
                //
                // Before this, a miss was an unconditional permanent refusal,
                // which deadlocked every mailbox that had never been used with
                // Apple Notes: gotcha #12 makes a note's `parentFolderId` the
                // only route to a folder id, so a mailbox with zero notes had
                // no id to file the first note with, and no way to acquire one
                // without filing it. Measured live 2026-08-17
                // (`TheArchitect@renny.co.th`): two notes, 5,816 refusals, zero
                // pushes, and an empty `Notes` folder sitting right there in
                // Outlook web the whole time.
                let dest = wire::create_destination(&self.folder_ids, op.label)
                    .ok_or_else(|| TransportError::Permanent {
                        source: anyhow::anyhow!(
                            "no Exchange folder id for '{}', and Graph offers a well-known \
                             name for the Notes root only — this folder cannot be written \
                             to until some note is filed into it (see gotcha #12)",
                            op.label
                        ),
                    })?;
                wire::create_note(&self.token, &dest, op.title, op.body_html).await?
            }
        };
...
mod vertical_tests {
    ...
    /// returns from the `create_destination` miss before any dispatch (`wire::
    /// create_note` is never reached), unlike every other test in this
    /// module that exercises `save_note_full`.
    ///
    /// **Scoped to a SUBFOLDER since 2026-08-17.** The root case this
    /// originally covered is no longer an error at all: it is written by
    /// well-known name (see the next test), which is the escape from the
    /// bootstrap deadlock. A subfolder still has no address.
    #[tokio::test]
    async fn creating_into_an_unknown_subfolder_names_the_label_rather_than_failing_generically() {
        // Empty folder_ids: no label has a cached Exchange id.
        let v = MicrosoftVertical::new("tok".into(), "acct".into(), Default::default());
        let op = SaveOp { title: "T", body_html: "<div>b</div>",
            existing_remote_id: None, existing_uuid: Some("temp"),
            existing_created_date: None, label: "Notes/Projects" };
        ...
        assert!(err.to_string().contains("Notes/Projects"), "must name the offending label; got: {err}");
    }

    /// The bootstrap deadlock, at the vertical's own boundary: a mailbox that
    /// has never held a note has no folder id to file the first one with, and
    /// no way to learn one without filing it (gotcha #12). Before this the
    /// create refused permanently and the worker retried it every 5 s forever
    /// — measured live 2026-08-17 on `TheArchitect@renny.co.th`: two notes,
    /// 5,816 refusals, zero successful pushes, Notes folder visibly empty in
    /// Outlook web.
    #[tokio::test]
    async fn a_first_note_reaches_an_empty_notes_folder_that_has_no_discoverable_id() {
        let (addr, seen) = server_returning(
            r#"{"id":"NEW","subject":"T","internetMessageId":"<n@x>","parentFolderId":"REAL-ID",
                "createdDateTime":"2026-08-17T10:00:00Z","lastModifiedDateTime":"2026-08-17T10:00:00Z",
                "body":{"contentType":"html","content":"<html><body>T</body></html>"}}"#,
        ).await;
        wire::set_graph_base_for_test(&format!("http://{addr}"));
        ...
        let saved = v.save_note_full(&op, &[]).await
            .expect("the root must be writable by name when no id was ever discovered");
        assert_eq!(saved.uuid, "n@x");

        let line = seen.lock().unwrap().first().cloned().expect("a request");
        assert!(
            line.starts_with("POST /me/mailFolders/notes/messages"),
            "must address the root by well-known name, not by an id it does not have; got: {line}"
        );
    }
}
