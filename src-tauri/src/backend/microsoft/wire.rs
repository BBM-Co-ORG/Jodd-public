...
pub fn strip_leading_title_ms(html: &str, subject: &str) -> String {
    ...
    // GROUND TRUTH FIRST. `subject` carries the title as plain text, so when
    // the body opens with exactly that text there is nothing to infer: cut
    // precisely there. Every branch below is a heuristic for the cases this
    // one cannot answer (a formatted title is markup in the body but plain in
    // `subject`, so it will not prefix-match).
    //
    // This is also the RECOVERY path for notes Jodd itself damaged before
    // gotcha #17 was fixed. Those were written with title and body fused into
    // one text run and no boundary between them — unrecoverable by any
    // boundary-scanning rule, because there is no boundary. Their titles are
    // still in `subject`, so re-reading them now returns the body instead of
    // nothing.
    //
    // Getting this wrong in the other direction would be worse than the
    // original bug: a note whose cached body is empty pushes that emptiness
    // back to Exchange on the next edit, destroying text that had survived on
    // the server.
    if let Some(cut) = subject_prefix_cut(inner, subject) {
    ...
    let cut = match first_line_boundary(lower_inner) {
        ...
        // A `<br>` is the SEPARATOR between the title line and the body, not
        // the body's first content — so the cut goes PAST it. Every other
        // boundary tag opens real content and the cut stops before it.
        //
        // This is what makes the round trip exact for a body with no leading
        // block element: `inject_title_into_body_ms` emits `title<br>body`
        // for those, and cutting at the `<br` rather than past it would hand
        // the editor a body with a spurious blank first line that grows a
        // little further from the truth on every cycle.
        //
        // `i > 0` matters: a body that OPENS with `<br>` has an empty first
        // line, which is a blank line the user typed, not a title to strip.
        // That falls through to the arm below and strips nothing.
        Some((i, "<br")) if i > 0 => match lower_inner[i..].find('>') {
            Some(j) => i + j + 1,
            // Malformed, unterminated `<br` — cut before it rather than
...
/// Byte offset just past the title when the body's first line IS the subject,
/// or `None` when it is not (so the caller falls back to its heuristics).
///
/// Compares against the ESCAPED subject, because that is what the injector
/// wrote: `escape_html_text` turns `&` into `&amp;`, so a raw comparison would
/// miss every title containing `& < >` — the exact titles where a wrong cut
/// does the most damage.
///
/// A `<br>` immediately after the title is the separator the injector adds
/// (gotcha #17), not content, so it is consumed with the title. Any other
/// following markup is content and is left alone.
///
/// Returns `None` for an empty/whitespace subject: that would match at every
/// position and strip nothing meaningfully, and the injector's blank-title
/// branch writes `<div></div>` rather than empty text anyway.
fn subject_prefix_cut(inner: &str, subject: &str) -> Option<usize> {
    if subject.trim().is_empty() {
        return None;
    }
    let escaped = escape_html_text(subject);
    let lead_ws = inner.len() - inner.trim_start().len();
    let rest = &inner[lead_ws..];
    if !rest.starts_with(&escaped) {
        return None;
    }
    let mut cut = lead_ws + escaped.len();

    // Consume one immediately-following `<br …>` — the injected separator.
    let after = &inner[cut..];
    let lower_after = after.to_ascii_lowercase();
    if lower_after.starts_with("<br") {
        if let Some(j) = lower_after.find('>') {
            cut += j + 1;
        }
    }
    Some(cut)
...
pub fn inject_title_into_body_ms(title: &str, body_html: &str) -> String {
    ...
    let title_is_blank = title.trim().is_empty();
    let escaped = if title_is_blank {
        "<div></div>".to_string()
    } else {
    ...
    let (prefix, rest, suffix) = match at {
        Some(at) => (&body_html[..at], &body_html[at..], ""),
        None => ("<html><body>", body_html, "</body></html>"),
    };

    // The separator is the whole point, not a cosmetic touch.
    //
    // The title is the first *LINE* of the body (gotcha #11) — and a line
    // needs something to end it. When the body opens with a block element
    // that element IS the boundary, which is why this was invisible for so
    // long: every shape anyone tested with (`<div>one</div>…`) supplied one
    // for free. A body that opens with plain text or inline markup supplies
    // nothing, so title and body fuse into a single text run, and
    // `strip_leading_title_ms` — correctly, given what it was handed — reads
    // that whole run as the title and returns an empty body.
    //
    // Measured 2026-08-18 on kaiwan.h@live.com: three notes whose text was
    // intact on Exchange and in Apple Notes rendered blank in Jodd, their
    // cached bodies `<body …></body>` with nothing between the tags.
    //
    // No separator when the title is blank: that branch already emits
    // `<div></div>`, which is itself a boundary the stripper consumes whole.
    let needs_break = !title_is_blank && !opens_with_a_line_boundary(rest);
    let sep = if needs_break { "<br>" } else { "" };

    format!("{prefix}{escaped}{sep}{rest}{suffix}")
}

/// Does this body content already begin with something that ends the title's
/// line? Whitespace-tolerant, and an empty body (or one that goes straight to
/// `</body>`) counts as "yes" — a title-only note needs no separator, and
/// adding one would put a stray blank line into every such note.
fn opens_with_a_line_boundary(rest: &str) -> bool {
    ...
    LINE_BOUNDARIES.iter().any(|t| lower.starts_with(t))
}
...
mod inject_title_tests {
    ...
    fn round_trips_through_the_stripper() {
        let bare = [
            ...
            ("empty body", ""),
            // The shape that actually shipped broken. Every other body in
            // this fixture opens with a block element, which SUPPLIES the
            // line boundary the stripper looks for — so the fixture could
            // not distinguish "the injector emits a boundary" from "every
            // test body happened to start with one".
            //
            // The editor emits exactly this for a single unformatted line,
            // and it is what the user typed on 2026-08-18: title and body
            // fused into one text run with nothing between them, so the
            // stripper saw one line, called all of it the title, and
            // returned an empty body. Measured on kaiwan.h@live.com — all
            // three cached rows held `<body …></body>` with nothing inside.
            ("plain title", "one plain line of body text"),
            ("thai plain", "มีข้อมูล 1 บรรทัด"),
            // Inline markup is not a line boundary either: a first body line
            // the user bolded has no block wrapper.
            ("inline first line", "<b>bold first line</b>"),
        ...
        //
        // Run the whole property TWICE, against both strippers:
        //
        //   `title` — production. `subject` carries the title, so the cut is
        //             made on ground truth (`subject_prefix_cut`).
        //   `""`    — the heuristic fallback, for a body whose first line is
        //             NOT a plain copy of the subject (a formatted title) and
        //             for anything that reaches the stripper with no subject.
        //
        // Pinning only the first would let the boundary-scanning half rot
        // unnoticed until a formatted title hit it in production; pinning
        // only the second is what this fixture did before, and it missed the
        // fused-line defect entirely.
        for subject_mode in ["ground-truth", "heuristic"] {
            for (title, body) in bare {
                let subject = if subject_mode == "ground-truth" { title } else { "" };

                let injected = inject_title_into_body_ms(title, body);
                assert_eq!(
                    strip_leading_title_ms(&injected, subject),
                    format!("<html><body>{body}</body></html>"),
                    "bare input must normalise to the wrapper shape, losing only the title: \
                     {title:?} ({subject_mode})"
                );

                // A WRAPPED body is the production case — every pulled note's
                // cached body_html is wrapped — and here the round trip IS
                // strict equality.
                let wrapped = format!("<html><body>{body}</body></html>");
                let injected = inject_title_into_body_ms(title, &wrapped);
                assert_eq!(
                    strip_leading_title_ms(&injected, subject), wrapped,
                    "wrapped round trip failed for title {title:?} ({subject_mode})"
                );
            }
        }
    }

    /// Recovery for notes Jodd already damaged.
    ///
    /// Before gotcha #17 was fixed, Jodd wrote title and body fused into one
    /// text run with no boundary between them. No boundary-scanning rule can
    /// undo that — there is no boundary. `subject` is the way back in, and it
    /// is always present.
    ///
    /// This is not cosmetic. With the cached body empty, the user's next edit
    /// pushes that emptiness to Exchange and destroys text that had survived
    /// on the server. Verbatim shape from `kaiwan.h@live.com`.
    #[test]
    fn a_note_damaged_by_the_old_fused_write_is_recovered_on_re_read() {
        let damaged = "<html><head>\r\n<meta http-equiv=\"Content-Type\" content=\"text/html; \
                       charset=utf-8\"></head><body style=\"line-break:after-white-space\">\
                       Note 1 A ก่อนลบมีข้อมูล 1 บรรทัด</body></html>";

        // What shipped: no boundary to find, so the whole run reads as title.
        assert!(
            strip_leading_title_ms(damaged, "").contains("</body>")
                && !strip_leading_title_ms(damaged, "").contains("มีข้อมูล"),
            "precondition: the heuristic alone cannot recover this"
        );

        let recovered = strip_leading_title_ms(damaged, "Note 1 A ก่อนลบ");
        assert!(recovered.contains("มีข้อมูล 1 บรรทัด"), "the body must come back: {recovered}");
        assert!(!recovered.contains("Note 1 A ก่อนลบ"), "the title must still be stripped: {recovered}");
    }

    /// A title with `&`/`<`/`>` is stored ESCAPED in the body, so ground-truth
    /// matching has to compare against the escaped form. A raw comparison
    /// silently falls through to the heuristic — on exactly the titles where
    /// a wrong cut does the most damage.
    #[test]
    fn ground_truth_matching_handles_an_escaped_title() {
        let injected = inject_title_into_body_ms("a & b <c>", "plain body");
        assert_eq!(
            strip_leading_title_ms(&injected, "a & b <c>"),
            "<html><body>plain body</body></html>"
        );
    }

    /// A formatted title is markup in the body but plain text in `subject`,
    /// so it cannot prefix-match — and must fall through to the heuristic
    /// rather than cutting nothing or cutting wrongly.
    #[test]
    fn a_formatted_title_falls_through_to_the_heuristic() {
        let html = "<html><body><b><i>styled title</i></b><div>real line</div></body></html>";
        let out = strip_leading_title_ms(html, "styled title");
        assert!(!out.contains("styled title"), "the formatted title must still be stripped: {out}");
        assert!(out.contains("real line"), "content must survive: {out}");
    }

    /// The exact note measured on kaiwan.h@live.com, 2026-08-18.
    ///
    /// The user typed a title and one unformatted line. What reached
    /// Exchange was the two fused into a single text run —
    /// `…เปลี่ยน Titleมีข้อมูล 1 บรรทัด`, with no separator — so Outlook and
    /// Apple Notes showed one run-on line, and Jodd, re-reading its own
    /// write, stripped the entire run as "the title" and rendered nothing.
    #[test]
    fn the_title_and_a_plain_first_line_do_not_fuse_into_one_line() {
        let injected = inject_title_into_body_ms("Note 1 A ก่อนลบ", "มีข้อมูล 1 บรรทัด");

        assert!(
            !injected.contains("Note 1 A ก่อนลบมีข้อมูล 1 บรรทัด"),
            "title and body must not fuse — that is what Outlook and Apple Notes displayed \
             as one run-on line: {injected}"
        );
        assert_eq!(
            strip_leading_title_ms(&injected, "Note 1 A ก่อนลบ"),
            "<html><body>มีข้อมูล 1 บรรทัด</body></html>",
            "reading back Jodd's own write must return the body, not an empty one"
        );
    }

    /// The separator is added only where it is missing. A body that already
    /// opens with a block element supplies its own boundary, and injecting a
    /// second one would put a blank line at the top of the note — on every
    /// edit, of most notes, since a pulled body is `<div>`-shaped.
    #[test]
    fn a_body_that_already_starts_with_a_block_element_gets_no_extra_break() {
        let injected = inject_title_into_body_ms("T", "<div>one</div>");
        assert_eq!(injected, "<html><body>T<div>one</div></body></html>");
    }

    /// A title-only note must not grow a trailing blank line.
    #[test]
    fn a_title_only_note_gets_no_separator() {
        let injected = inject_title_into_body_ms("T", "");
        assert_eq!(injected, "<html><body>T</body></html>");
        assert_eq!(strip_leading_title_ms(&injected, "T"), "<html><body></body></html>");
    }

    /// Repeated save→pull→save cycles must converge, not accumulate.
    /// Whatever the injector adds, the stripper must take back out — or the
    /// note grows a blank line per edit for as long as the user keeps typing.
    #[test]
    fn the_round_trip_is_idempotent_across_repeated_edits() {
        let mut body = "one plain line".to_string();
        for _ in 0..5 {
            body = strip_leading_title_ms(&inject_title_into_body_ms("Title", &body), "Title");
        }
        assert_eq!(body, "<html><body>one plain line</body></html>",
            "five save/pull cycles must leave the body exactly where it started");
    }
...
struct RawMessage {
    ...
    #[serde(default)]
    subject: String,
    /// Nullable in Graph — see [`null_as_empty_string`]. Empty is the value
    /// [`raw_to_graph_message`] already reads as "no identity", so a null
    /// takes the existing, tested route (dropped and counted in
    /// `skipped_without_identity`) instead of failing the whole page.
...
/// Decode a Graph string property that may arrive as an explicit JSON `null`
/// into the empty string, which every reader here already treats as "absent".
///
/// **`#[serde(default)]` alone does NOT cover this**, and the difference cost
/// a production outage: `default` supplies a value when the KEY IS MISSING,
/// but an explicit `"field": null` still reaches the `String` visitor and
/// fails the whole page with `invalid type: null, expected a string`. Graph
/// types most message properties as nullable and really does send `null`
/// rather than omitting the key — observed live 2026-08-17, where it broke
/// [`list_pinned`] on every sync tick with a `permanent:` classification, so
/// the affected push retried forever and never succeeded.
///
/// **Null-tolerant, not type-tolerant.** This routes through
/// `Option<String>`, so `null` becomes `""` while a number/object/array in
/// the same position still fails to deserialize. That distinction is the
/// point: `null` is Graph saying "this property has no value", which the
/// readers below already have correct behavior for (drop the row, fall back
/// to a placeholder name, read as not-pinned); a wrong TYPE is Graph — or a
/// proxy in front of it — returning a shape this module does not understand,
/// and silently defaulting that would turn a real protocol change into
/// quietly-missing notes and pins instead of a loud error.
fn null_as_empty_string<'de, D>(de: D) -> Result<String, D::Error>
where
    D: serde::Deserializer<'de>,
{
    Ok(<Option<String> as serde::Deserialize>::deserialize(de)?.unwrap_or_default())
}

#[derive(serde::Deserialize)]
struct RawExtProp {
    id: String,
    /// Nullable in Graph: a `singleValueExtendedProperty` that exists on a
    /// message but holds nothing comes back as `"value": null`. Empty is
    /// already what all three readers expect for "no such property" —
    /// [`find_ext_prop`]'s callers each `unwrap_or_default()` — so
    /// collapsing null into it changes no behavior, it only stops the null
    /// from failing the page. See [`null_as_empty_string`].
    #[serde(default, deserialize_with = "null_as_empty_string")]
    value: String,
}
...
fn raw_to_graph_message(m: RawMessage) -> Option<GraphMessage> {
    ...
    // `subject` is the title as plain text — pass it so the stripper can cut
    // on ground truth instead of inferring a boundary. See
    // `subject_prefix_cut`. Bound before the struct literal because the
    // struct also takes ownership of `m.subject`.
    let body_html = strip_leading_title_ms(&body, &m.subject);
...
mod decode_tests {
    ...
    /// Graph sends `internetMessageId: null` rather than omitting the key on
    /// a message that was never submitted. `#[serde(default)]` does not cover
    /// an explicit null, so before `null_as_empty_string` this failed the
    /// whole page instead of taking the skip-and-count route the empty-string
    /// case above already had. Same defect that broke `list_pinned` live on
    /// 2026-08-17; this path is shielded by `sticky_note_filter` today, which
    /// is why it never fired here — not a reason to leave it decoding a
    /// nullable Graph property as a non-nullable `String`.
    #[test]
    fn a_null_internet_message_id_is_skipped_like_an_empty_one_not_a_page_error() {
        let json = r#"{"value":[
            {"id":"x","subject":"s","internetMessageId":null,
             "parentFolderId":"p","createdDateTime":"2026-01-01T00:00:00Z",
             "lastModifiedDateTime":"2026-01-01T00:00:00Z",
             "body":{"contentType":"html","content":"<html><body>s</body></html>"}},
            {"id":"y","subject":"real","internetMessageId":"<a@b>",
             "parentFolderId":"p","createdDateTime":"2026-01-01T00:00:00Z",
             "lastModifiedDateTime":"2026-01-01T00:00:00Z",
             "body":{"contentType":"html","content":"<html><body>real</body></html>"}}
        ]}"#;
        let page = decode_page(json).expect("a null internetMessageId must not fail the page");
        assert_eq!(page.skipped_without_identity, 1, "null is 'no identity', same as empty");
        assert_eq!(page.messages.len(), 1, "the message that does have an identity still decodes");
        assert_eq!(page.messages[0].internet_message_id, "a@b");
    }
...
mod read_path_tests {
    ...
    // ── The bootstrap deadlock: writing into a folder with no reachable id ──

    /// The whole point. A mailbox never used with Apple Notes holds zero
    /// notes, so no `parentFolderId` exists to learn the Notes id from — and
    /// before this, the create refused permanently and the note retried every
    /// 5 s forever (measured 2026-08-17: 5,816 failures in one session, zero
    /// pushes). The well-known name needs no id, so the first note can land.
    #[test]
    fn the_notes_root_is_writable_by_name_when_no_id_has_ever_been_discovered() {
        assert_eq!(
            create_destination(&HashMap::new(), "Notes").as_deref(),
            Some(WELL_KNOWN_NOTES_FOLDER)
        );
        // Same for a row that exists but carries no usable id.
        assert_eq!(
            create_destination(&ids(&[("Notes", "  ")]), "Notes").as_deref(),
            Some(WELL_KNOWN_NOTES_FOLDER)
        );
    }

    /// A real id always wins. The name is the fallback, not the default:
    /// once a scan has learned the id, keep using it, so nothing about
    /// steady-state behaviour changes.
    #[test]
    fn a_known_id_beats_the_well_known_name() {
        assert_eq!(
            create_destination(&ids(&[("Notes", "AAMkAGI2")]), "Notes").as_deref(),
            Some("AAMkAGI2")
        );
    }

    /// Graph offers a well-known name for the root and for nothing beneath
    /// it, so a subfolder with no cached id has no address at all. That must
    /// stay a refusal — inventing one would file the note in the wrong place.
    #[test]
    fn a_subfolder_with_no_id_has_no_address_and_must_not_fall_back_to_the_root() {
        assert_eq!(create_destination(&ids(&[("Notes", "x")]), "Notes/Projects"), None);
        assert_eq!(create_destination(&HashMap::new(), "Notes/Projects"), None);
    }
...
/// The Notes root addressed by well-known NAME instead of by id.
///
/// Graph will not let you *read* this folder — `GET /me/mailFolders/notes`
/// answers `404 ErrorItemNotFound`, exactly as it does for every genuine
/// Notes-tree member (gotcha #12) — but it does resolve the segment for
/// navigation and for writes. Measured 2026-08-17 on
/// `TheArchitect@renny.co.th`:
///
/// | request | result |
/// | --- | --- |
/// | `GET /me/mailFolders/notes` | **404** `ErrorItemNotFound` |
...
/// | `POST /me/mailFolders/notes/messages` | **201**, `parentFolderId` = the real Notes id |
///
/// The 400 on a name Graph does not know is what makes the 200 meaningful:
/// `notes` is a *recognised* well-known name, not a string being tried as a
/// malformed id. The created item came straight back from the whole-mailbox
/// `IPM.StickyNote` scan, so it is a real note in the real container, not a
/// draft parked somewhere.
///
/// This is the ONLY escape from the bootstrap deadlock, and it is the
/// difference between "cannot be discovered" and "cannot be written":
/// gotcha #12 establishes that a folder holding zero notes has no reachable
/// id, which used to mean a mailbox never used with Apple Notes could never
/// receive its first note — finding the folder needed a note, creating the
/// note needed the folder. Addressing the destination by name sidesteps
/// discovery entirely. **Gotcha #12 is unchanged**: the folder still cannot
/// be enumerated, read, or walked. Only the root, and only for a write.
pub const WELL_KNOWN_NOTES_FOLDER: &str = "notes";

/// The Jodd path that means "the Notes root" on this backend.
///
/// Hardcoded, as the five folder-side sites gotcha #9 already lists are —
/// this is the sixth. `MicrosoftVertical` is constructed with a token, an
/// account id and a path→id map (`vertical_for`, lib.rs), never with the
/// `Account`, so `effective_notes_label()` is not reachable from here.
/// Whoever finishes roadmap 0c must fix this site along with those five.
const NOTES_ROOT_PATH: &str = "Notes";

/// How to address the destination folder of a create.
///
/// `Some(id)` — a real Exchange folder id from the local `folders` table.
/// `Some(WELL_KNOWN_NOTES_FOLDER)` — the root, which needs no id at all.
/// `None` — a SUBFOLDER Jodd has no id for, which genuinely cannot be
/// written to: a well-known name exists for the root and for nothing below
/// it, and the id is unreachable until some note is filed there
/// (gotcha #12). The caller turns that into a permanent, *surfaced* failure
/// rather than a retry loop.
///
/// An empty cached id counts as a miss, matching [`folder_read_plan`] —
/// `/me/mailFolders//messages` is not a folder.
pub fn create_destination(folder_ids: &HashMap<String, String>, label: &str) -> Option<String> {
    match folder_ids.get(label) {
        Some(id) if !id.trim().is_empty() => Some(id.clone()),
        _ if label == NOTES_ROOT_PATH => Some(WELL_KNOWN_NOTES_FOLDER.to_string()),
        _ => None,
    }
}
...
///
/// `folder_id` is either a real Exchange id or the well-known name
/// [`WELL_KNOWN_NOTES_FOLDER`] — the URL is identical in shape either way
/// (percent-encoding a bare `notes` leaves it unchanged), which is why this
/// function needs no branch for it. See [`create_destination`].
///
...
///
/// **Being the module's only unfiltered read has a second consequence that
/// cost an outage.** Every other scan here is narrowed to notes server-side
/// ([`messages_url`] carries [`sticky_note_filter`]; [`folder_notes_url`] is
/// scoped to a folder that holds nothing but notes), so this is the one
/// decoder that meets ORDINARY MAIL — and ordinary mail includes shapes a
/// note never has, starting with a draft, which carries no
/// `internetMessageId`. Graph sends `null` there rather than omitting the
/// key, which `#[serde(default)]` does not cover; see
/// [`null_as_empty_string`]. Anything added to this scan's `$select` must be
/// assumed nullable for the same reason.
...
fn decode_pin_page(json: &str) -> Result<(Vec<PinScanRow>, Option<String>), String> {
    ...
    #[serde(rename_all = "camelCase")]
    struct RawRow {
        /// Nullable in Graph, and this is the decoder that actually meets a
        /// null: [`pin_scan_url`] is the only unfiltered whole-mailbox read
        /// in this module (every other one sits behind [`sticky_note_filter`]
        /// or a notes-folder scope), so it is the only one that sees ordinary
        /// mail — including a draft, which carries no `internetMessageId` at
        /// all. See [`null_as_empty_string`] for what broke and why null is
        /// tolerated but a wrong type still is not.
        #[serde(default, deserialize_with = "null_as_empty_string")]
        internet_message_id: String,
        #[serde(default)]
        single_value_extended_properties: Vec<RawExtProp>,
...
mod pin_sidecar_tests {
    ...
    /// The production failure this null-tolerance exists for (observed
    /// 2026-08-17, `jodd.log`): `permanent: failed to decode Graph
    /// list_pinned: invalid type: null, expected a string`, on every tick,
    /// forever. [`pin_scan_url`] is the module's ONLY unfiltered
    /// whole-mailbox read — every other decoder sits behind
    /// [`sticky_note_filter`] or a notes-folder scope — so it is the one
    /// place an ordinary draft reaches, and Graph returns
    /// `internetMessageId: null` on a message that was never submitted.
    ///
    /// The row must be DROPPED, exactly as the empty-string row above is: an
    /// explicit `null` and an absent key are the same statement in OData
    /// ("this message has no internet message id"), and neither yields a
    /// uuid a `SidecarRecord` could be keyed by.
    #[test]
    fn a_null_internet_message_id_drops_the_row_instead_of_failing_the_whole_page() {
        let with_draft = r#"{"value":[
            {"id":"m1","internetMessageId":"<pinned@x>",
             "singleValueExtendedProperties":[
               {"id":"String {98d27db2-72ad-4511-a3b5-b73c7c42694b} Name JoddPin","value":"{\"pinned\":true}"}
             ]},
            {"id":"draft","internetMessageId":null,"singleValueExtendedProperties":[]}
        ]}"#;
        let (rows, _) = decode_pin_page(with_draft).expect("a null internetMessageId must not fail the page");
        assert_eq!(rows.len(), 1, "the draft is dropped; the real note still decodes");
        assert_eq!(rows[0].uuid, "pinned@x");
        assert!(rows[0].pinned);
    }

    /// The other nullable string in this decoder. Graph types a
    /// `singleValueExtendedProperty`'s `value` as nullable, so a property
    /// that exists on a message but holds nothing arrives as `null` rather
    /// than being omitted.
    ///
    /// Reads as NOT pinned — the same verdict [`decode_pin_value`] already
    /// gives unparseable garbage, and for the same reason spelled out in its
    /// doc comment: a value `list_pinned` cannot make sense of must never
    /// resurrect a pin the user removed.
    #[test]
    fn a_null_extended_property_value_decodes_as_not_pinned() {
        let null_value = r#"{"value":[
            {"id":"m1","internetMessageId":"<empty-prop@x>",
             "singleValueExtendedProperties":[
               {"id":"String {98d27db2-72ad-4511-a3b5-b73c7c42694b} Name JoddPin","value":null}
             ]}
        ]}"#;
        let (rows, _) = decode_pin_page(null_value).expect("a null property value must not fail the page");
        assert_eq!(rows.len(), 1, "the row still has a uuid, so it survives decode");
        assert!(!rows[0].pinned, "a property with no value is not a pin");
    }

    /// Null-tolerance must not become type-tolerance. `null` is Graph saying
    /// "absent"; a number where a string belongs is Graph (or a proxy)
    /// returning something this decoder does not understand, and swallowing
    /// that would turn a real protocol change into silently-missing pins.
    #[test]
    fn a_wrong_typed_field_is_still_a_decode_error_not_a_silent_default() {
        let numeric_id = r#"{"value":[{"id":"m1","internetMessageId":7,"singleValueExtendedProperties":[]}]}"#;
        assert!(
            decode_pin_page(numeric_id).is_err(),
            "a non-string internetMessageId is malformed, not absent"
        );

        let numeric_value = r#"{"value":[
            {"id":"m1","internetMessageId":"<x@y>",
             "singleValueExtendedProperties":[
               {"id":"String {98d27db2-72ad-4511-a3b5-b73c7c42694b} Name JoddPin","value":7}
             ]}
        ]}"#;
        assert!(
            decode_pin_page(numeric_value).is_err(),
            "a non-string property value is malformed, not absent"
        );
    }
}
...
mod write_path_tests {
    ...
    /// The bootstrap escape, at the URL level. The well-known name goes into
    /// the same `/me/mailFolders/{}/messages` URL an id does — percent-encoding
    /// a bare `notes` leaves it untouched, which is why `create_note` needs no
    /// branch for it. Pinned because a future encoder change (or a caller
    /// "helpfully" quoting the segment) turns the one measured escape from the
    /// bootstrap deadlock back into a `400 ErrorInvalidIdMalformed`, silently.
    #[tokio::test]
    async fn a_create_addressed_by_well_known_name_hits_the_folder_named_notes() {
        let (addr, seen) = scripted_server(vec![message_with_identity("NEW-ID")]).await;
        set_graph_base_for_test(&format!("http://{addr}"));

        create_note("tok", WELL_KNOWN_NOTES_FOLDER, "s", "")
            .await
            .expect("the name-addressed create is the bootstrap escape");

        let (line, _) = seen.lock().unwrap().first().cloned().expect("a request");
        assert!(
            line.starts_with("POST /me/mailFolders/notes/messages"),
            "the well-known name must reach Graph unmangled — got: {line}"
        );
    }
