...
///
/// **A request that never got an answer is `Transient`, and is checked first.**
/// Every `wire.rs` send site does `.send().await.map_err(|e| e.to_string())`,
/// and reqwest's `Display` for a connect / DNS / TLS / timeout failure is
/// `"error sending request for url (…)"` — no status, and no source chain (so
/// "operation timed out" never reaches this string). Falling through to
/// `Permanent` made `push_one_dirty` block the note (gotcha #14) and
/// `push_folder` drop a new folder's row on a network blip (observed on
/// Android 2026-09-16). Checked before the status markers because such an
/// error can never carry a status, only a URL.
///
/// `"error decoding response body"` deliberately stays `Permanent`: reqwest
/// uses it both for a truncated read and for a JSON shape mismatch, and on
/// `save_note` it follows a 200 insert, where a retry would duplicate the note.
pub(crate) fn classify_str(err: &str) -> TransportError {
    let has = |code: &str| err.contains(code);
    if has("error sending request") {
        TransportError::Transient { source: anyhow::anyhow!("{}", err) }
    } else if has(" 401") || has("UNAUTHENTICATED") || has("Invalid Credentials") || has(" 403") {
        TransportError::Auth
    } else if has(" 429") {
...
mod classify_tests {
    ...
    fn classify_str_maps_known_codes() {
        ...
        // Observed verbatim on Android 2026-09-16 (`list_all_message_ids`);
        // later sweeps of the same label succeeded.
        assert!(matches!(
            classify_str("error sending request for url (https://gmail.googleapis.com/gmail/v1/users/me/messages?labelIds=Label_85&maxResults=500)"),
            TransportError::Transient { .. }
        ));
        // A genuine refusal must stay Permanent — it is what gotcha #14's
        // push block exists for.
        assert!(matches!(
            classify_str("Save failed 400 Bad Request: {\"error\":{\"code\":400}}"),
            TransportError::Permanent { .. }
        ));
    }

    /// Not a hand-written string: a real reqwest failure, stringified exactly
    /// the way every `wire.rs` send site does (`.map_err(|e| e.to_string())`).
    /// Pins the classifier to reqwest's actual `Display`, so a reqwest upgrade
    /// that rewords it fails here rather than silently blocking pushes again.
    #[tokio::test]
    async fn a_real_connection_failure_is_transient_not_permanent() {
        // Bind for a free port, then drop the listener so nothing listens.
        let dead_port = {
            let l = std::net::TcpListener::bind("127.0.0.1:0").expect("bind");
            l.local_addr().expect("addr").port()
        };
        let err = reqwest::Client::new()
            .get(format!("http://127.0.0.1:{dead_port}/gmail/v1/users/me/messages"))
            .send()
            .await
            .expect_err("a closed port must fail")
            .to_string();
        assert!(
            matches!(classify_str(&err), TransportError::Transient { .. }),
            "a dropped connection must retry, not block the push; got {:?} for {err:?}",
            classify_str(&err),
        );
    }
}
...
impl Transport for GmailVertical {
    ...
    async fn move_note(&self, remote_id: &str, add: &[String], remove: &[String]) -> Result<Option<crate::backend::RemoteNoteVersion>, TransportError> {
        crate::backend::gmail::wire::modify_message_labels(&self.token, remote_id, add, remove)
            .await.map_err(|e| classify_str(&e))?;
        // A labels.modify call never re-mints the message id, and `id` IS
        // this backend's version (see `Note::version`'s doc comment) — so
        // nothing about the note's version changed.
        Ok(None)
    }
}
...
impl MetadataSidecar for GmailVertical {
    ...
    async fn put_sidecar(&self, note_uuid: &str, kind: SidecarKind, body: Option<&[u8]>, replace: Option<&str>) -> Result<(String, Option<crate::backend::RemoteNoteVersion>), TransportError> {
        let SidecarKind::Pin = kind;
        ...
        let id = crate::backend::gmail::wire::save_meta_sidecar(
            &self.token, note_uuid, &payload, &meta_id, replace, &self.user_email,
        ).await.map_err(|e| classify_str(&e))?;
        // The sidecar is a wholly separate Gmail message — this write never
        // touches the note's own object, so its remote_version is unchanged.
        Ok((id, None))
    }
