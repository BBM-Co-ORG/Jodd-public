...
const ORPHAN_RECENT_WINDOW_MS: i64 = 24 * 60 * 60 * 1000;
...
async fn safe_cleanup_orphans_for_account(
    ...
    let cutoff = db::now_ms() - ORPHAN_RECENT_WINDOW_MS;
    let candidates: Vec<db::CachedNote> = state.db
        .list_notes(account_id)
        .map_err(|e| e.to_string())?
        .into_iter()
        .filter(|n| {
            matches!(n.sync_state, db::SyncState::Clean)
                && n.last_local_modified_at > cutoff
                && !n.id.is_empty()
        })
        .collect();

    log!(
        "cleanup_orphans: scanning {} recent clean note(s) for account {}",
...
async fn cleanup_orphans(
    ...
    // After a successful cleanup, decrement the observed count by the
    // number we actually trashed. The next list_notes call will recompute
    // from scratch; this just keeps the pill honest in the interim.
    if trashed > 0 {
        let mut stats = state.dup_stats.lock().unwrap();
        if let Some(s) = stats.get_mut(&account_id) {
            s.collapsed = s.collapsed.saturating_sub(trashed);
            // Don't try to be clever about uuids_affected — let the next
            // list_notes correct it.
        }
    }
    Ok(trashed)
}
...
async fn trash_specific_messages(
    ...
    // Decrement dup_stats by what we actually trashed so the pill stays honest.
    if trashed > 0 {
        let mut stats = state.dup_stats.lock().unwrap();
        if let Some(s) = stats.get_mut(&account_id) {
            s.collapsed = s.collapsed.saturating_sub(trashed);
...
async fn push_one_deletion(
    ...
    // Nothing on the server to trash.
    if n.id.is_empty() {
