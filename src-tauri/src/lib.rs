...
async fn vertical_for(state: &State<'_, AppState>, account_id: &str) -> Result<Box<dyn Vertical>, String> {
    ...
    match kind {
        ...
        accounts::BackendKind::ICloud => {
            #[cfg(not(desktop))]
            {
                // The credential is a live WKWebView cookie jar, and mobile
                // sign-in is its own constraint chain (M3). gotcha #8 is the
...
async fn icloud_vertical_for(
    ...
    let cookies = std::sync::Arc::new(icloud_auth::LiveSession {
...
async fn icloud_sign_in(state: State<'_, AppState>) -> Result<String, String> {
    #[cfg(not(desktop))]
    {
        let _ = &state;
        return Err(
            "iCloud sign-in needs a desktop webview to hold the session. Add the account \
             on macOS."
                .to_string(),
        );
    }
    #[cfg(desktop)]
    {
        ...
        let cookies = std::sync::Arc::new(icloud_auth::LiveSession { app: app.clone() });
