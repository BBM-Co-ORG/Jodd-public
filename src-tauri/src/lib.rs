...
fn push_needs_explicit_move_dispatch(
...
enum DirtyPushKind { Content, MoveOnly, Unsupported }

fn dirty_push_kind(writes: backend::Writes, was_create: bool) -> DirtyPushKind {
    if writes.notes {
...
fn push_confirms_content(kind: DirtyPushKind) -> bool {
    match kind {
...
async fn sync_worker_tick(app: &AppHandle) {
    ...
    let due: Vec<String> = {
        ...
        let candidates: Vec<String> = state
            ...
            .unwrap()
            .iter()
            .filter(|a| a.status == accounts::AccountStatus::Active)
...
const PULL_INTERVAL: std::time::Duration = std::time::Duration::from_secs(60);
...
/// cheap.
fn incremental_pull_supported(kind: accounts::BackendKind) -> bool {
    match kind {
...
fn pull_outcome(stored_cursor: Option<&str>, changed: usize) -> PullOutcome {
...
fn cursor_write_needed(outcome: PullOutcome) -> bool {
    match outcome {
...
fn frontend_wakeup_needed(outcome: PullOutcome) -> bool {
    match outcome {
...
fn store_sync_cursor(state: &State<'_, AppState>, account_id: &str, cursor: Option<String>) {
    let mut list = state.accounts.lock().unwrap();
...
fn ensure_icloud_replica_id(state: &State<'_, AppState>, account_id: &str) -> Option<[u8; 16]> {
    let mut list = state.accounts.lock().unwrap();
...
async fn detect_remote_changes(state: &State<'_, AppState>, account_id: &str) -> bool {
    let stored = {
        let list = state.accounts.lock().unwrap();
...
async fn suggest_note_folder(
    ...
    request_id: String,
    state: State<'_, AppState>,
) -> Result<crate::llm::filing::FolderSuggestionOutcome, String> {
    ...
    let result: Result<crate::llm::filing::FolderSuggestionOutcome, String> = async {
        ...
        let provider = crate::llm::resolve::resolve_provider_for_account(&account).map_err(|e| e.to_string())?;
        crate::llm::filing::suggest_folder(provider.as_ref(), &state.db, &account_id, &uuid, cancel)
            .await
            .map_err(|e| match e {
    ...
    if let Ok(outcome) = &result {
        log!("suggest_note_folder: {outcome:?}");
    }
    result
}
...
async fn test_llm_provider(
    ...
    let (provider, preset_id) = match account_id {
        ...
        Some(id) => {
            ...
            let provider = crate::llm::resolve::resolve_provider_for_account(&account)
                .map_err(|e| e.to_string())?;
...
mod extract_mint_tests {
    ...
    fn extract_create_sites_mint_through_the_backend_policy() {
        let src = include_str!("lib.rs");
        for signature in ["async fn extract_note_into(", "fn create_fallback_source_note("] {
...
mod capabilities_tests {
    ...
    fn every_guarded_command_declares_the_right_write_kind() {
        ...
        assert_eq!(mapping.len(), 20, "20 commands, each single-kind");
