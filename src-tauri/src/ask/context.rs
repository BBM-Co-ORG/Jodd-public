...
pub fn build_answer_context(
    ...
    for (account_id, uuid) in selected {
        ...
        // Redacted BEFORE truncation, so a cut can never leave half a URL's
        // query string behind. A URL-ingest note's stored Source block keeps
        // each full link (`URL:` lines); the provider must not see its
        // signed-URL tokens (URL ingest spec, "URLs that carry secrets").
        let full = crate::ingest::urls::redact_url_secrets(&crate::db::strip_html_to_text(&n.body_html));
...
mod tests {
    ...
    #[test]
    fn an_ingest_notes_stored_url_reaches_the_provider_without_its_query_string() {
        // A URL-ingest note stores each source's full URL (spec: "The note
        // stores the full URL, which the user already had") — but the answer
        // context goes to the account's LLM provider, and the spec also says
        // signed-URL tokens never reach a provider. The stored text itself
        // must still arrive: Ask Jodd answers from it.
        let db = temp_db();
        let a = "acct@x";
        let body = "<h2>Sources</h2>\n<ul>\n<li><a href=\"https://e.example/p?token=SECRET&amp;x=1\">Page</a> — ok</li>\n</ul>\n\
                    <hr>\n<details>\n<summary>Source (verbatim)</summary>\n<pre>=== Jodd source 1 of 1 ===\n\
                    URL: https://e.example/p?token=SECRET&amp;x=1\nTitle: Page\nStatus: ok\n\nfetched words about sync</pre>\n</details>\n";
        note(a, "ingested").title("Ingest").body(body).insert(&db);
        let selected = vec![(a.to_string(), "ingested".to_string())];

        let ctx = build_answer_context(&db, &selected, &HashSet::new()).unwrap();
        let rendered = render_context(&ctx);

        assert!(!rendered.contains("SECRET"), "a signed-URL token reached the provider context: {rendered}");
        assert!(rendered.contains("https://e.example/p"), "the source stays identifiable: {rendered}");
        assert!(rendered.contains("fetched words about sync"), "the stored source text must still reach Ask Jodd: {rendered}");
    }
