...
pub fn load_key_hex() -> Result<Option<String>, String> {
    // One real credential-store access per call — see the note in
    // `accounts::load_refresh_token`. This is also the read jodd-mcp performs
    // on startup, where it lands on stderr rather than jodd.log because that
    // binary never calls `applog::init()`.
    log!("keychain READ   {}/{}", KC_SERVICE, kc_key_name());
...
pub fn save_key_hex(hex: &str) -> Result<(), String> {
    log!("keychain WRITE  {}/{}", KC_SERVICE, kc_key_name());
    let entry = keyring_core::Entry::new(KC_SERVICE, &kc_key_name())
        .map_err(|e| format!("keychain open: {e}"))?;
    entry
        .set_password(hex)
...
pub enum DbOpenError {
    ...
    /// Migration #19 (`account_id` -> `{backend}:{email}`) refused to run:
    /// the cache holds notes or folders under an id that matches no account,
    /// and guessing a backend would misfile them.
    ///
    /// Its own variant because the caller must NOT treat it the way it treats
    /// a generic failure. `.setup()`'s catch-all opens a fresh temp-dir cache,
    /// which for this error means the user's intact database is simply not
    /// shown — zero notes, zero folders, every launch, one line in the log.
    /// A refusal to migrate has to surface as a refusal.
    AccountIdMigration(String),
...
impl std::fmt::Display for DbOpenError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            DbOpenError::AccountIdMigration(e) => {
                write!(f, "account-id migration refused: {e}")
            }
