...
// Inject the title as the first <div> of the body content, matching Apple's
// convention that the body's first element is the displayed title.
//
// ALWAYS injects, exactly once — this function owns the inject/strip
// invariant, and the two are exact inverses over editor-view bodies:
//
//     strip_leading_title(inject_title_into_body(x, t), t) == x
//
// pinned by `round_trips` in the tests below.
//
// **Why there is no idempotence check.** `SavedNote.body_html` (backend/mod.rs)
// is EDITOR-VIEW: the pull path strips the title row before the cache ever
// sees it, and the push path injects on the way out. So an editor body never
// carries the title, and a first line that happens to EQUAL the title is the
// user's own content — a note titled "Shopping" whose body opens with the word
// "Shopping", a shape Apple Notes stores and displays without complaint.
//
// This used to short-circuit on two conditions — a literal leading
// `<div>{title}</div>`, and a tag-stripped first line equal to the title —
// and both were wrong for exactly that body: inject read the user's line as
// "the title is already here" and emitted nothing, so the wire body became
// byte-identical to one that had never been injected. The next pull's
// `strip_leading_title` then cut that line, correctly given what it was
// handed, and `upsert_from_remote` wrote the truncation into the cache. One
// line of the user's note, deleted silently, with the editor's save status
// still reading "Saved". Same failure the Microsoft injector's doc comment
// describes; `inject_title_into_body_ms` resolved it the same way, by always
// injecting.
//
// The cost of always injecting is bounded and visible: a legacy cache row that
// still holds a title row (only reachable from a Jodd predating the
// styled-title strip fix, aa9a041, June 2026) grows a second one, which the
// single strip pass leaves showing in the editor for the user to delete. A
// duplicated line the user can see beats a real one deleted where they cannot.
...
//
// **Cuts AT MOST ONE row.** This used to loop until a pass changed nothing, to
// repair wire messages carrying both `<div>{title}</div>` (a pre-fix ANB4W
// injection) AND `<span>{title}</span>` (Apple's original) back to back — a
// shape whose producer was fixed in the same commit that added the loop
// (aa9a041, June 2026), so nothing has emitted one since.
//
// The loop is not merely stale, it is unsound. On the wire,
// `<div>T</div><div>T</div><div>rest</div>` has exactly one meaning: title T,
// body `T` + `rest` — what Apple Notes itself stores for a note whose first
// body line repeats its title. A second pass reduces that to `rest`, deleting
// a line the user typed, and `upsert_from_remote` then writes the truncation
// into the cache. No amount of care in `inject_title_into_body` can restore
// the round trip while this iterates, because the shapes it cuts are the same
// shapes a correct injection produces. Single-pass is what makes the pair
// exact inverses — see `inject_title_into_body`, which owns that invariant,
// and `round_trips` in the tests below.
//
// The Microsoft pair is single-pass for the same reason
...
mod title_tests {
    ...
    // Deliberate reversal of an older contract, recorded rather than deleted.
    // This used to assert that inject NO-OPS on a body whose first line already
    // carries the (styled) title. That is a WIRE body — `strip_leading_title`
    // has already removed the row by the time anything reaches the cache — and
    // treating it as inject's input is what broke the round trip: a real
    // editor body whose first line equals the title got no injection, so the
    // next pull's strip deleted the user's line.
    //
    // Injecting is now unconditional. The pair still converges — this is the
    // property that matters, and it is what the old assertion was reaching for.
    #[test]
    fn inject_always_adds_a_row_even_when_the_first_line_matches() {
        let title = "new from iphone เพิ่มภาษาไทย(bold italic)";
        let b = body("new from iphone <b><i>เพิ่มภาษาไทย(bold italic)</i></b><div>x</div>");
        let injected = inject_title_into_body(&b, title);
        assert_eq!(
            injected,
            body("<div>new from iphone เพิ่มภาษาไทย(bold italic)</div>\
new from iphone <b><i>เพิ่มภาษาไทย(bold italic)</i></b><div>x</div>")
        );
        // …and strip takes back exactly what inject added, no more.
        assert_eq!(strip_leading_title(&injected, title), b);
    }
    ...
    // Same reversal, fragment shape. A leading `<div>Fresh</div>` in an
    // EDITOR body is a line the user typed, not a title already injected —
    // so it gets one of its own, and strip removes only that one.
    #[test]
    fn inject_adds_a_row_to_a_fragment_whose_first_line_already_matches() {
        let title = "Fresh";
        let editor = "<div>Fresh</div><p>just a paragraph</p>";
        let injected = inject_title_into_body(editor, title);
        assert_eq!(injected, "<div>Fresh</div><div>Fresh</div><p>just a paragraph</p>");
        assert_eq!(strip_leading_title(&injected, title), editor);
    }
    ...
    //
    // `inject` and `strip` are an INVERSE PAIR over EDITOR-VIEW bodies, and
    // that direction is the one the cache depends on: `SavedNote.body_html`
    // (backend/mod.rs) stores what the user sees, pre-inject, while the wire
    // stores the injected form. So every push→pull cycle runs
    // `strip(inject(x))` and must land back on `x`.
    //
    // An editor-view body NEVER carries the title row — the pull path strips
    // it before the cache ever sees it. Therefore a first line that happens to
    // equal the title is the user's own content (a note titled "Shopping"
    // whose body opens with the word "Shopping" — a shape Apple Notes stores
    // and displays fine), not a title to re-detect.
    fn round_trips(x: &str, title: &str) {
        let injected = inject_title_into_body(x, title);
        assert_eq!(
            strip_leading_title(&injected, title),
            x,
            "strip(inject(x)) must equal x\n  x        = {x}\n  injected = {injected}"
        );
    }

    #[test]
    fn round_trip_holds_when_the_body_opens_with_the_title_text() {
        // The failing case: `<div>{title}</div>` leading the EDITOR body is a
        // line the user typed. inject used to read it as "already injected"
        // and no-op; strip then removed it — one line gone, silently.
        round_trips("<div>Shopping</div><div>eggs</div>", "Shopping");
    }

    #[test]
    fn round_trip_holds_for_a_styled_first_line_matching_the_title() {
        // Same drift with styling: `strip_html_tags` makes the comparison
        // succeed, so inject skipped and strip cut.
        round_trips("<div><b>Shopping</b></div><div>eggs</div>", "Shopping");
    }

    #[test]
    fn round_trip_holds_for_a_bare_text_first_line_matching_the_title() {
        round_trips("Shopping<div>eggs</div>", "Shopping");
    }

    #[test]
    fn round_trip_holds_for_ordinary_and_awkward_bodies() {
        round_trips("<div>just body</div>", "Fresh");
        round_trips("<p>a paragraph</p>", "Fresh");
        round_trips("", "Fresh");
        round_trips("<div>#tag</div><div>x</div>", "Fresh");
        round_trips("\n<div>x</div>", "Fresh");
        round_trips("<div>x</div>", "a & b <h3>"); // angle brackets in the title
        round_trips(&body("<div>just body</div>"), "Fresh"); // full document
        round_trips(&body("<div>Shopping</div><div>eggs</div>"), "Shopping");
    }

    /// Repeated save→pull→save cycles must converge, not shed a line each
    /// time. Copied in shape from the Microsoft side's
    /// `the_round_trip_is_idempotent_across_repeated_edits`.
    #[test]
    fn the_round_trip_is_idempotent_across_repeated_edits() {
        let title = "Shopping";
        let start = "<div>Shopping</div><div>eggs</div>";
        let mut b = start.to_string();
        for _ in 0..5 {
            b = strip_leading_title(&inject_title_into_body(&b, title), title);
        }
        assert_eq!(b, start, "five save/pull cycles must leave the body where it started");
    }
