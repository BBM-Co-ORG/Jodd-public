...
// Inject the title as the first <div> inside <body>. Idempotent: if the body's
// first line already IS the title (any styling), we don't double-inject.
...
pub fn inject_title_into_body(body_html: &str, title: &str) -> String {
    ...
    if let Some(start) = body_html.find("<body") {
        ...
        // First line already carries the title (bare, wrapped, or styled)? Skip.
...
pub fn strip_leading_title_once(body_html: &str, title: &str) -> String {
    ...
    let (head, inner) = match body_html.find("<body") {
        Some(start) => {
            let after_open = match body_html[start..].find('>') {
                Some(o) => start + o + 1,
                None => return body_html.to_string(),
