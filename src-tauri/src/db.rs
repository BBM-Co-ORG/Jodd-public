...
impl Db {
    ...
    pub fn move_notes_batch(
        ...
        // A move is a write, so it follows forwarding addresses: a caller
        // holding a pre-rekey uuid (gotcha #16) would otherwise match no row
        // and be told nothing. Resolved before taking the lock, which
        // `resolve_note_uuid` takes itself. A live uuid resolves to itself.
        let resolved = uuids
            .iter()
            .map(|u| self.resolve_note_uuid(u, account_id))
