...
impl Db {
    ...
    fn migrate(&self) -> SqlResult<()> {
        ...
        let migrations: &[(i64, &str)] = &[
            ...
            (
                ...
                "ALTER TABLE notes ADD COLUMN remote_label TEXT;",
            ),
        ];
    ...
    const NOTE_COLS: &'static str = "n.uuid, n.account_id, n.id, n.title, n.body_html, \
        ...
        n.pinned, n.meta_msg_id, n.pin_dirty";
    ...
    pub fn upsert_from_remote(&self, n: &CachedNote) -> SqlResult<()> {
        ...
        tx.execute(
            ...
            // `pinned` deliberately omitted from ON CONFLICT SET: all
            // Jodd-local, never authoritative from remote. On a fresh INSERT
            // (new uuid) it comes from the CachedNote (from_remote stamps
            // pinned=false), so new-to-us notes arrive unpinned.
            // remote_label bound to the SAME ?8 as label: on a fresh pull the
            // label we're about to store IS what the remote currently holds
            // — this is the ground-truth source `push_one_dirty`'s
            // move-dispatch check (Microsoft M2 fix 2) relies on to self-heal
            // past a stale/NULL value. See migration #16's comment.
...
mod cross_process_safety_tests {
    ...
    fn upsert_from_remote_stamps_remote_label_from_the_incoming_label() {
        ...
        db.upsert_from_remote(&n).unwrap();
        assert_eq!(
        ...
        db.upsert_from_remote(&moved).unwrap();
        assert_eq!(
            db.get_remote_label("u7", "acc").unwrap(),
