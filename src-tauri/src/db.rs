...
//
// remote_version is the last-known value of the vertical's opaque change
// token (`backend::Note::version` — Gmail: message id; Microsoft:
// lastModifiedDateTime; LocalFs: Date header), NOT necessarily a message id.
// Gmail's insert-then-trash pattern happens to make its id a sufficient
// version token, which is why this column carried "the id" for so long, but
// that is a Gmail property, not a property of remote_version itself — see
// `backend::Note::version` and `reconcile_one`'s `remote_changed` helper.
//
// `mark_pushed` stamps this column from `SavedNote::version` (a distinct
// value from the `id` it also writes) for exactly this reason: the push side
// has to agree with the pull side (`CachedNote::from_remote`, above) about
// what "the version" is, or the very next reconcile compares two different
// kinds of value and reads a no-op push as a remote change. See
// `SavedNote::version` and `Db::mark_pushed`.
//
...
pub struct CachedNote {
    ...
    pub remote_version: Option<String>,     // vertical's Note::version at last known sync point (see module doc)
    ...
    /// Why the last push of this note was refused **permanently** — i.e. why
    /// retrying it unchanged can never succeed. `None` for every note that is
    /// syncing normally, which is nearly all of them.
    ///
    /// Only meaningful alongside `sync_state = Dirty`: the row still holds
    /// unsent local content (that is what makes it dirty), it just has no
    /// route to the remote. Set by `push_one_dirty` (lib.rs) off
    /// `TransportError::Permanent`, cleared by any subsequent local edit and
    /// by the explicit `retry_blocked_pushes` command. See migration #17 for
    /// why this is a column rather than a sixth `SyncState`.
    pub push_blocked_reason: Option<String>,

    // Tags have no equivalent sidecar state here — they live purely in the
    // `note_tags` join table, re-derived from the note body's inline
    // #hashtags on every write/pull (see `tags_from_body` /
    // `reconcile_tags_from_body_conn`). The `tags_meta_msg_id`/`tags_dirty`
    // columns from the old tag-sidecar mechanism still exist in the SQLite
    // schema (migrations #7/#8) but are no longer read or written by any
    // Rust code — left in place rather than dropped, since this codebase
    // has no DROP COLUMN precedent.
}
...
pub struct PendingPushes {
    ...
    pub folders: usize,
    /// Notes holding unsent local content that the worker has **stopped**
    /// trying to send, because the backend refused permanently.
    ///
    /// Deliberately NOT part of `notes`, and deliberately not counted by
    /// `has_pending_pushes`: these are not queued work the worker will get to,
    /// so folding them into "N left" would show a number that never goes down
    /// — the exact "finishing sync — 0 left" trap gotcha #6 records, inverted.
    /// They are still the user's unsynced data, so they need their own number
    /// rather than silence.
    pub blocked: usize,
}
...
/// One migration's body.
///
/// `Sql` is every migration through #18. `Rust` exists because migration #19
/// must read `accounts.json` to learn each account's backend kind, and that
/// lives outside the database entirely — it cannot be expressed as a SQL
/// string. A separate migration system running outside `migrate()` was
/// rejected: the `migrations` table would stop being the whole answer to
/// "what has been applied".
enum Step {
    Sql(&'static str),
    Rust(fn(&rusqlite::Connection) -> SqlResult<()>),
}

/// Every table whose PRIMARY KEY contains `account_id`.
///
/// `notes_fts` is deliberately absent even though it is an EIGHTH carrier
/// (`account_id UNINDEXED`, migration #10): its content is *derived* from
/// `notes`, so migration #19 re-derives it rather than UPDATEing it
/// (gotcha #4). A rewrite that stopped at these seven would leave every
/// search result pointing at an account id that no longer exists.
const ACCOUNT_ID_TABLES: &[&str] = &[
    "notes",
    "folders",
    "note_tags",
    "tag_tombstones",
...
/// Prefix on migration #19's fail-closed abort message.
///
/// `Step::Rust` can only fail as a `rusqlite::Error`, and every one of those
/// reaches `Db::open` as `DbOpenError::Sqlite` — which `.setup()` handles by
/// opening a fresh temp-dir cache, so a refusal to migrate would present to
/// the user as "all your notes are gone", once per launch, with one line in
/// the log. This marker is what lets [`classify_migrate_error`] hand the
/// caller a distinguishable error instead.
const ACCOUNT_ID_MIGRATION_ABORT: &str = "migration 19 aborted";

/// Map a migration failure onto the error the caller can act on.
///
/// Only migration #19's fail-closed abort is singled out; everything else is
/// an ordinary SQLite failure and keeps its existing meaning.
fn classify_migrate_error(e: rusqlite::Error) -> crate::db_crypto::DbOpenError {
    if let rusqlite::Error::InvalidParameterName(msg) = &e {
        if msg.starts_with(ACCOUNT_ID_MIGRATION_ABORT) {
            return crate::db_crypto::DbOpenError::AccountIdMigration(msg.clone());
        }
    }
    crate::db_crypto::DbOpenError::Sqlite(e)
}

/// Whether this connection is the user's REAL cache rather than a test or
/// temp-dir database.
///
/// This gates one thing only: whether migration #19 is allowed to rewrite
/// `accounts.json`. Without it, every `cargo test` run would — `Db::open_*`
/// runs the migration, `accounts::save_accounts()` resolves through
/// `paths::config_base()`, and `paths.rs`'s `OnceLock` "cannot be reset
/// between tests", so the file it would rewrite is the developer's own
/// `~/Library/Application Support/jodd/accounts.json`. `#[cfg(test)]` is not
/// enough: `jodd-mcp`'s tests link this crate compiled WITHOUT `cfg(test)`
/// and open temp databases through the same path.
///
/// Both real consumers of the live cache pass this — the app
/// (`paths::data_base()/jodd`, `lib.rs`'s `.setup()`) and `jodd-mcp`, whose
/// default `resolve_db_path()` is the same directory. The desktop-only
/// last-resort temp-dir cache in `.setup()` does not, which is correct: that
/// cache is volatile, and account ids should not be rewritten on the strength
/// of one.
///
/// **`jodd-mcp` passing is intended, not an accident to be narrowed later.**
/// It opens the *same* real database through the same `Db::open`, so it runs
/// migration #19 for real and must migrate the account list along with it — a
/// jodd-mcp run that qualified the tables and left `accounts.json` bare would
/// reproduce exactly the split state this migration exists to close. Two
/// processes racing is safe: the migration is idempotent, resolves by `email`
/// (which neither process changes), and `save_accounts` renames atomically, so
/// whichever arrives first wins and the other finds the work already done and
/// no-ops.
///
/// Canonicalised on both sides so a symlinked home or `/var` -> `/private/var`
/// cannot make the live cache look like a stranger — a false negative here is
/// the expensive direction (the database would be rewritten and the account
/// list left behind).
///
/// **`live_base` is a parameter, not a call to `paths::data_base()`, purely so
/// the `true` branch is reachable from a test.** With the lookup inlined, the
/// only way to make this function say `true` was to make the connection the
/// developer's actual cache — so the branch that does the dangerous thing had
/// nothing watching it, and an earlier attempt to test it by forcing `true`
/// rewrote a real `accounts.json`. The single production call site passes
/// `crate::paths::data_base()`; tests pass a tempdir and exercise both
/// directions with zero exposure to the real data directory.
fn connection_is_the_live_user_cache(
    conn: &rusqlite::Connection,
    live_base: Option<PathBuf>,
) -> bool {
    let Some(path) = conn.path() else { return false };
    if path.is_empty() {
        return false; // in-memory or temporary database
    }
    let Some(live_dir) = live_base.map(|d| d.join("jodd")) else {
        return false;
    };
    let Some(parent) = std::path::Path::new(path).parent() else {
        return false;
    };
    match (parent.canonicalize(), live_dir.canonicalize()) {
        (Ok(a), Ok(b)) => a == b,
        _ => parent == live_dir,
    }
}

/// Migration #19 — rewrite every `account_id` to `{backend}:{email}`.
///
/// This is the registered `Step::Rust`. It is a thin shell around
/// [`migrate_account_ids_with`], which holds all of the database work and
/// touches no filesystem — the split is what makes the migration testable,
/// because `accounts::load_accounts()` / `save_accounts()` resolve through
/// `paths::config_base()`, a process-global `OnceLock` that cannot be pointed
/// at a tempdir from a unit test (`paths.rs`).
///
/// **`accounts.json` is written LAST, after `tx.commit()`.** Two properties
/// make that the safe order, and neither is a preference:
///
/// 1. **A failed `save_accounts` is retried in full, and retries cheaply.**
///    The error propagates out of this function, so `Db::migrate`'s loop never
///    reaches its `INSERT INTO migrations` and the whole migration re-runs on
///    the next launch. The already-qualified rows are then skipped by
///    `accounts::is_qualified`, so the retry is a no-op on the tables and only
///    the file write is actually re-attempted. (An earlier version of this
///    comment argued for save-first on the grounds that save-last would strand
///    a qualified database beside a bare account list "permanently, with
///    version 19 recorded". That was true only of a draft that swallowed the
///    error with `let _ =`. This code propagates it, so the premise is false.)
/// 2. **A fail-closed abort must not touch the file at all.**
///    `migrate_account_ids_with` refuses to guess a backend for an orphan id
///    and returns `Err` with the tables untouched (gotcha #2's orphan rows are
///    a real state). Under save-first, `accounts.json` had *already* been
///    rewritten by then — leaving a qualified account list beside a bare
///    database, and a migration that aborts identically on every subsequent
///    launch. Under save-last the abort happens before the file is opened.
///
/// The remaining crash window — process death between the commit and the
/// rename — is harmless in both directions, because resolution is by `email`,
/// which this migration never changes: the next run finds the tables already
/// qualified and rewrites only the list. `accounts::save_accounts` is itself
/// atomic (tmp file + `rename`), so the file is never left half-written.
///
/// **There is no rollback path.** A downgrade past this migration is
/// unsupported: nothing records what the previous ids were.
fn migrate_account_ids(conn: &rusqlite::Connection) -> SqlResult<()> {
    let accounts = crate::accounts::load_accounts();
    let updated: Vec<crate::accounts::Account> = accounts
        .iter()
        .cloned()
        .map(|mut a| {
            if !crate::accounts::is_qualified(&a.id) {
                a.id = crate::accounts::account_id_for(a.backend_kind, &a.email);
            }
            a
        })
        .collect();

    // Tables first. `updated` rather than `accounts` so the list this resolves
    // against matches what is about to be written to disk. Which one is passed
    // cannot change the outcome: resolution is by email, and email is the
    // field this migration never touches.
    migrate_account_ids_with(conn, &updated)?;

    // The COUNT THAT CHANGED, not the length of the list. This is the line a
    // maintainer reads while watching a live upgrade, so "qualifying 3 account
    // id(s)" on a profile where two were already qualified is not log noise —
    // it is a wrong answer to the question the log line exists to answer.
    let changed_count = updated
        .iter()
        .zip(accounts.iter())
        .filter(|(u, a)| u.id != a.id)
        .count();
    if changed_count > 0 {
        if connection_is_the_live_user_cache(conn, crate::paths::data_base()) {
            crate::log!("migration 19: qualifying {} account id(s)", changed_count);
            crate::accounts::save_accounts(&updated).map_err(|e| {
                rusqlite::Error::InvalidParameterName(format!(
                    "migration 19: tables are qualified but accounts.json could not be \
                     rewritten ({e}) — version 19 stays unrecorded, so the next launch \
                     retries (a no-op on the tables, which is_qualified now skips)"
                ))
            })?;
        } else {
            crate::log!(
                "migration 19: not the live user cache ({:?}) — rewriting the tables only",
                conn.path().unwrap_or("<none>")
            );
        }
    }
    Ok(())
...
/// The database half of migration #19. No filesystem access, by design.
///
/// Opens its own transaction. `Db::migrate` applies each migration in
/// autocommit and records the version in a *separate* `INSERT` afterwards, so
/// a crash between the two hands this function a database it has already
/// converted. Idempotency is therefore load-bearing rather than defensive, and
/// it comes from `accounts::is_qualified`: an id that already carries a backend
/// prefix is skipped, which is also what makes every `localfs:{uuid}` row a
/// no-op on the first run.
///
/// Resolution is **by email, not only by the old id**, because `accounts.json`
/// cannot join this transaction. A bare `account_id` equals the `email` of
/// exactly one account, and email never changes, so either half-finished state
/// (list rewritten, tables not — or the reverse) completes here.
///
/// **Fails closed — but only where failing closed protects something.** An
/// unresolvable `account_id` is not one situation, it is three, and the first
/// version of this function treated them alike, which turned an ordinary
/// orphan row into an app that opens an empty temp-dir cache on every launch:
///
/// 1. **The id carries user data** (`notes` or `folders`). Guessing a backend
///    would silently misfile someone's notes under the wrong account, so this
///    aborts the transaction — and, via [`classify_migrate_error`], surfaces
///    as `DbOpenError::AccountIdMigration` so the caller can say so instead of
///    substituting a fresh database.
/// 2. **The id appears only in satellite tables** (`note_tags`,
///    `tag_tombstones`, `attachments`, `edges`, `note_uuid_aliases`). That is
///    a provably-gone account: `remove_account` saves the shortened
///    `accounts.json` *before* wiping the database, and `delete_account`'s
///    satellite deletes are best-effort `let _ =` — and only grew to cover
///    `attachments`, `edges` and `note_uuid_aliases` in 2026-08, so existing
///    installs carry these today. There is no note to misfile, so the rows are
///    deleted rather than allowed to wedge the open.
/// 3. **The account list is EMPTY beside a non-empty database.** That
///    combination means *"the list could not be read"*, not *"every account
///    was removed"* — `accounts::load_accounts` swallows every error and
///    returns an empty `Vec`, so an unreadable or truncated `accounts.json`
///    looks exactly like a user with no accounts. Deleting or aborting on the
///    strength of it would destroy, or lock out, an intact cache. The
///    migration does nothing at all and returns `Ok`.
///
/// Case 3 returns `Ok`, so `Db::migrate` records version 19 and the tables
/// stay bare for good. That is deliberate: acting on an empty list is the only
/// option that can destroy an intact cache, so doing nothing is the safe
/// direction even though it means this profile never migrates.
///
/// Bare tables beside a bare `accounts.json` are **almost** self-consistent —
/// resolution is by id, the credential fallback already handles a bare id, and
/// every id minted afterwards is qualified without colliding with the bare
/// ones. **The exception is re-authentication, and it is the one thing this
/// state cannot do cleanly.** `complete_oauth` dedupes on the *qualified* id
/// (lib.rs), so a surviving bare account that later signs in again is not
/// recognised: the user gets a SECOND account row for the same mailbox, and
/// its `index_account` re-imports the whole mailbox beside the bare copy.
/// Nothing on this branch can reach that except case 3.
///
/// Converging them later would mean re-running this on every open, and the
/// cost is a `DISTINCT` over seven tables per launch on affected profiles.
/// That is the trade, stated plainly — an earlier version of this comment
/// argued the alternative would "make case 1's abort fire forever instead of
/// once", which was never true once a case-1 abort began refusing the launch
/// outright: it already fires every launch until the user repairs
/// `accounts.json`.
fn migrate_account_ids_with(
    conn: &rusqlite::Connection,
    accounts: &[crate::accounts::Account],
) -> SqlResult<()> {
    let tx = conn.unchecked_transaction()?;

    // Every table, not just notes+folders: an id surviving in, say, note_tags
    // alone would be orphaned by a rewrite that never looked there.
    let union = ACCOUNT_ID_TABLES
        .iter()
        .map(|t| format!("SELECT account_id FROM {t}"))
        .collect::<Vec<_>>()
        .join(" UNION ");
    // ORDER BY is not cosmetic: it makes the order ids are rewritten in — and
    // therefore how much work a fail-closed abort has to roll back — the same
    // on every run and in every test, instead of whatever the DISTINCT
    // implementation happens to produce.
    let existing: Vec<String> = {
        let mut stmt = tx.prepare(&format!(
            "SELECT DISTINCT account_id FROM ({union}) ORDER BY account_id"
        ))?;
        let ids = stmt.query_map([], |r| r.get(0))?.collect::<SqlResult<Vec<_>>>()?;
        ids
    };

    // Case 3 above: an empty list beside rows is an unreadable `accounts.json`,
    // not an install with no accounts. Nothing has been written yet, so
    // dropping the transaction unwritten is the whole of "do nothing".
    if accounts.is_empty() && !existing.is_empty() {
        crate::log!(
            "migration 19: {} account id(s) in the cache but the account list is empty \
             — treating that as an unreadable accounts.json and leaving the tables alone",
            existing.len()
        );
        return Ok(());
    }

    // Which ids actually carry user data. An id here that resolves to no
    // account aborts (case 1); an id absent from this set is satellite-only
    // and its rows are deleted (case 2).
    let ids_with_user_data: std::collections::HashSet<String> = {
        let mut stmt = tx.prepare(
            "SELECT account_id FROM notes UNION SELECT account_id FROM folders",
        )?;
        let ids = stmt
    ...
    for old in existing {
        if crate::accounts::is_qualified(&old) {
            continue;
        }
        let acct = accounts
            .iter()
            .find(|a| a.id == old)
            .or_else(|| accounts.iter().find(|a| a.email == old));
        let Some(acct) = acct else {
            if ids_with_user_data.contains(&old) {
                return Err(rusqlite::Error::InvalidParameterName(format!(
                    "{ACCOUNT_ID_MIGRATION_ABORT}: account_id {old:?} matches no account \
                     by id or email but still holds notes or folders — refusing to guess \
                     a backend"
                )));
            }
            // Satellite-only leftovers of an account that is provably gone.
            // Deleting them is what keeps a stale `note_tags` row from wedging
            // the whole database open (case 2 above).
            crate::log!(
                "migration 19: dropping satellite-only rows for departed account {old:?}"
            );
        ...
        let new = crate::accounts::account_id_for(acct.backend_kind, &acct.email);
        for table in ACCOUNT_ID_TABLES {
            tx.execute(
                &format!("UPDATE {table} SET account_id = ?1 WHERE account_id = ?2"),
                params![new, old],
            )?;
        }
        rewrote_any = true;
    ...
    // Re-derive FTS from `notes` rather than UPDATEing it: its content is
    // derived state and `notes` is now authoritative for account_id
    // (gotcha #4). Inside the same transaction, so the index can never be
    // observed disagreeing with the rows it indexes.
    // `purged_any` belongs here as much as `rewrote_any`: a purge leaves the
    // departed account's `notes_fts` rows behind (that table is not in
    // ACCOUNT_ID_TABLES, being derived), and the re-derive below is what
    // removes them — it rebuilds the index from `notes`, which no longer has
    // them.
    if rewrote_any || purged_any {
        tx.execute("DELETE FROM notes_fts", [])?;
        let rows: Vec<(String, String, String, String)> = {
            let mut stmt = tx.prepare("SELECT uuid, account_id, title, body_html FROM notes")?;
            let rows = stmt
...
/// Whether a pull may overwrite the cached pin.
///
/// **A per-backend decision that used to be a hardcoded assumption.** The old
/// comment here read "pin is Jodd-local metadata that Gmail/Apple don't
/// carry, so a remote pull must preserve whatever the local replica has" —
/// correct for every backend that existed when it was written, and wrong for
/// the one that came next. On Gmail, Microsoft and LocalFs the pin is a Jodd
/// sidecar with `pin_dirty` behind it, and accepting a remote value would
/// destroy a pin the user set. On iCloud the pin is Apple's own, carried on a
/// `Note_UserSpecific` record, and refusing it means the pin never arrives.
///
/// Spelled as a required argument rather than a default so a new backend
/// cannot inherit either answer by accident — the same reason
/// `canonical_uuid_for` has no wildcard arm (gotcha #18).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum RemotePin {
    /// The remote knows nothing about pins. Keep the local value.
    LocalWins,
    /// The remote carries the real pin. Accept it.
    RemoteWins,
}

/// The shared head of the upsert. The two variants differ only in whether
/// `pinned` is in the SET clause, so they are spelled once and diverge in one
/// line — a copy would be two places to update the next time a column moves.
macro_rules! upsert_from_remote_sql {
    ($extra:expr) => {
        concat!(
            "INSERT INTO notes (uuid, account_id, id, title, body_html, date,
                ...
                last_remote_modified_at, pinned, meta_msg_id, pin_dirty,
                remote_label)
...
/// The pin policy for a backend. **No wildcard arm** — a fifth backend must
/// state its answer rather than inherit one.
pub fn remote_pin_policy(kind: crate::accounts::BackendKind) -> RemotePin {
    use crate::accounts::BackendKind::*;
    match kind {
        // The pin is a Jodd sidecar on these; the remote carries nothing, and
        // accepting a false from it would silently unpin the user's notes.
        Gmail | Microsoft | LocalFs => RemotePin::LocalWins,
        // Apple's own pin, on a `Note_UserSpecific` record.
        ICloud => RemotePin::RemoteWins,
    }
}
...
impl RemotePin {
    fn upsert_sql(self) -> &'static str {
        match self {
            RemotePin::LocalWins => upsert_from_remote_sql!(""),
            RemotePin::RemoteWins => upsert_from_remote_sql!(",\n                pinned = excluded.pinned"),
        }
    }
}

impl Db {
    /// Test-only / temp-dir path: identical to what `open()` did before
    /// at-rest encryption — plain, unencrypted SQLite, no keychain access.
    /// Test fixtures in this crate and in `jodd-mcp` call this explicitly
    /// instead of `open()` so test runs never touch the real OS keychain
    /// or pay migration cost. See docs/superpowers/specs/2026-08-13-at-rest-encryption-design.md
    /// (Open questions #1: "Test/temp-dir DBs don't need real key management").
    pub fn open_unencrypted(app_data_dir: &PathBuf) -> SqlResult<Self> {
    ...
    /// Open (or create) the cache database, encrypted at rest via SQLCipher.
    /// `app_data_dir` is the Tauri app's user-data directory — `~/Library/
    /// Application Support/jodd` on macOS, `%APPDATA%/jodd` on Windows.
    ///
    /// On a fresh install, generates a new key and creates a fresh encrypted
    /// database. On an existing plaintext install, runs the one-time
    /// migration first. On an existing encrypted install, opens normally —
    /// a stored key that doesn't decrypt the file surfaces as
    /// `DbOpenError::KeyMismatchOrCorrupt`, which the caller (see
    /// `lib.rs`'s `.setup()`) must offer account re-indexing for, not treat
    /// as a generic fatal error.
    pub fn open(app_data_dir: &PathBuf) -> Result<Self, crate::db_crypto::DbOpenError> {
        use crate::db_crypto::{self, DbOpenError};

        std::fs::create_dir_all(app_data_dir).ok();
        let db_path = app_data_dir.join("jodd.sqlite3");
        let backup_path = app_data_dir.join("jodd.sqlite3.plaintext-backup");
        let tmp_path = app_data_dir.join("jodd.sqlite3.encrypting");

        // Resume an interrupted migration BEFORE anything else. Migration
        // renames original→backup then tmp→original; if the process dies
        // between those two renames there is no `jodd.sqlite3` at all, and
        // the naive reading is "fresh install" — which silently creates an
        // empty DB while every note sits in `.plaintext-backup` with nothing
        // pointing at it. A backup with no canonical DB can only mean an
        // interrupted migration, so put it back and let migration re-run
        // (which now also clears any stale `.encrypting` tmp file).
        if !db_path.exists() && backup_path.exists() {
            let _ = std::fs::remove_file(&tmp_path);
            std::fs::rename(&backup_path, &db_path).map_err(|e| {
                DbOpenError::Migration(format!("resume interrupted migration: {e}"))
            })?;
        }

        // A file that exists, is non-empty, and is NOT plaintext SQLite is an
        // already-encrypted database.
        let existing_encrypted_db = db_path
            .metadata()
            .map(|m| m.len() > 0)
            .unwrap_or(false)
            && !db_crypto::is_plaintext_sqlite(&db_path);

        let key_hex = match db_crypto::load_key_hex() {
            Ok(Some(existing)) => existing,
            // A failed keychain READ must never fall through to minting a new
            // key: that would overwrite a good entry and leave the existing
            // encrypted DB permanently undecryptable.
            Err(e) => return Err(DbOpenError::Keychain(e)),
            Ok(None) => {
                if existing_encrypted_db {
                    // Contradiction: an encrypted DB exists but the keychain
                    // says no key was ever stored. Minting a fresh key here
                    // would guarantee the file can never be decrypted again.
                    return Err(DbOpenError::Keychain(format!(
                        "{} exists and appears encrypted, but no DB key could be read \
                         from the keychain — refusing to mint a fresh key over it. \
                         Restore the keychain entry, or move the file aside to start \
                         over with an empty cache.",
                        db_path.display()
                    )));
                }
                let fresh = db_crypto::generate_key_hex();
                db_crypto::save_key_hex(&fresh).map_err(DbOpenError::Keychain)?;
                fresh
            }
        };

        let mut migrated = false;
        if db_path.exists() && db_crypto::is_plaintext_sqlite(&db_path) {
            db_crypto::migrate_plaintext_to_encrypted(&db_path, &key_hex)
                .map_err(DbOpenError::Migration)?;
            migrated = true;
        }

        let conn = db_crypto::open_encrypted(&db_path, &key_hex)?;
        conn.execute_batch(
            "PRAGMA journal_mode = WAL;
             PRAGMA synchronous = NORMAL;
             PRAGMA temp_store = MEMORY;
        ...
        let db = Db { conn: Mutex::new(conn) };
        // `classify_migrate_error`, not `?`: migration #19's fail-closed abort
        // must not reach `.setup()` as a generic `Sqlite` error, whose handler
        // opens an empty temp-dir cache and leaves the user's real one
        // unreachable with no way to tell why.
        db.migrate().map_err(classify_migrate_error)?;
        db.fts_backfill_if_empty()?;
        db.migrate_tags_to_body()?;
        db.edges_backfill_if_empty()?;

        // Retention policy for the plaintext backup (finding #4): keeping a
        // full plaintext copy of every note on disk forever would undo the
        // point of encrypting at rest. Delete it — but only HERE, once the
        // freshly-encrypted DB has proven it opens, decrypts, and survives
        // every app-level migration/backfill above. Deleting right after the
        // sqlcipher_export step instead would throw away the only recoverable
        // copy if the export produced a subtly corrupt database. Best-effort:
        // a failed unlink is a warning, never a failed launch.
        if migrated {
            if let Err(e) = std::fs::remove_file(&backup_path) {
                if e.kind() != std::io::ErrorKind::NotFound {
                    eprintln!(
                        "[db] could not delete plaintext backup {}: {e} — it still \
                         contains unencrypted notes and should be removed manually",
                        backup_path.display()
                    );
                }
            }
    ...
    fn migrate(&self) -> SqlResult<()> {
        ...
        let migrations: &[(i64, Step)] = &[
            ...
            (
                ...
                Step::Sql("ALTER TABLE notes ADD COLUMN pinned INTEGER NOT NULL DEFAULT 0;
            ...
            (
                ...
                Step::Sql("ALTER TABLE notes ADD COLUMN meta_msg_id TEXT;
            ...
            (
                ...
                Step::Sql("CREATE TABLE note_tags (
                    account_id TEXT NOT NULL,
                    uuid TEXT NOT NULL,
                    tag TEXT NOT NULL,
            ...
            (
                ...
                Step::Sql("CREATE TABLE tag_tombstones (
                    account_id TEXT NOT NULL,
                    uuid TEXT NOT NULL,
                    tag TEXT NOT NULL,
            ...
            (
                ...
                Step::Sql("ALTER TABLE notes ADD COLUMN tags_meta_msg_id TEXT;
            ...
            (
                ...
                Step::Sql("UPDATE notes SET tags_dirty = 1
            ...
            (
                ...
                // docs/DIRECTION.md §6.
                Step::Sql("CREATE TABLE attachments (
                    account_id TEXT NOT NULL,
            ...
            (
                ...
                Step::Sql("CREATE VIRTUAL TABLE notes_fts USING fts5(
                    uuid UNINDEXED,
            ...
            (
                ...
                Step::Sql("DROP TABLE IF EXISTS edges;
                CREATE TABLE edges (
                    account_id TEXT NOT NULL,
                    src_uuid   TEXT NOT NULL,
            ...
            (
                ...
                Step::Sql("ALTER TABLE folders ADD COLUMN kind TEXT NOT NULL DEFAULT 'user';"),
            ),
            ...
            (
                16,
                // Microsoft M2 fix (whole-branch review, 2026-08-15, Critical
                // fix 2): the label Jodd last CONFIRMED the remote holds for
                // this note, distinct from `label` (the locally-desired
                // value, mutated immediately by `apply_local_edit`/local
                // moves). On Gmail and LocalFs a label change is an intrinsic
                // part of `save_note_full` itself (Gmail inserts under the
                // new label and trashes the old message; LocalFs writes into
                // the new folder's directory and removes the old file), but
                // Exchange's `PATCH` is a true in-place update that never
                // touches `parentFolderId` — so a label change on that
                // backend needs an EXPLICIT `POST /me/messages/{id}/move`
                // after the PATCH, or the remote note stays in its old
                // folder forever while the local cache believes it moved.
                // `push_one_dirty` (lib.rs) reads this column to decide
                // whether that move is necessary, comparing it against the
                // row's current `label` — a no-op when they already agree,
                // so a move is never issued on an ordinary content-only save.
                //
                // NULL for every existing row and for a freshly-created one
                // (nothing confirmed on the remote yet) — callers must treat
                // NULL as "unknown, do not assume a move is needed", which is
                // the safe default: a stale/missing value only means one
                // rare Apple-side-move-plus-Jodd-edit racing past unnoticed
                // once, and the very next successful pull
                // (`upsert_from_remote`) or push (`mark_pushed`) stamps a
                // real value from ground truth — bounded and self-healing
                // (gotcha #4), not a data-loss risk.
                Step::Sql("ALTER TABLE notes ADD COLUMN remote_label TEXT;"),
            ),
            (
                17,
                // Why a push the worker will never retry needs a column of its
                // own (2026-08-17).
                //
                // `TransportError::Permanent` means retrying THIS operation
                // can never succeed. The worker had no way to act on that: it
                // logged the error and left the row `dirty`, so the same
                // doomed push fired every 5 seconds forever (measured:
                // 5,816 attempts, zero successes, ~80 minutes) while the UI
                // showed "Saved" and the note itself dropped out of the list
                // 30 s later, because no list path returns a note that does
                // not exist remotely.
                //
                // The two obvious cheaper answers are both wrong:
                //
                //   - Clear to `clean`. That is the lie the fix exists to
                //     remove — a note marked synced that was never written
                //     anywhere.
                //   - Drop the row, the way `push_folder` handles its own
                //     Permanent (lib.rs). Acceptable for a folder; for a note
                //     it is silent data loss.
                //
                // So: keep `sync_state = 'dirty'` (the row genuinely does hold
                // unsent local content — that stays true) and hang the reason
                // off it. NULL means "not blocked", which is every existing
                // row and every new one. `list_dirty` skips non-NULL, which
                // stops the retry; `has_pending_pushes` skips it too, so a
                // blocked row cannot wedge a Draining account into never
                // reaching Inactive and never being removable (gotcha #2's
                // wedge, the same one the folder path had to close).
                //
                // Deliberately NOT a sixth `sync_state`: blockage is a
                // qualifier ON dirty, not a peer of it, and the state machine
                // in `apply_local_edit` has five values' worth of transitions
                // that would all have needed a new arm. Same reasoning that
                // put `pin_dirty` in its own column.
                Step::Sql("ALTER TABLE notes ADD COLUMN push_blocked_reason TEXT;"),
            ),
            (
                18,
                // A forwarding address for `rekey_note_uuid`.
                //
                // When a backend assigns the note's identity at CREATE time
                // (Exchange's `internetMessageId`; also Gmail whenever
                // `resolve_uuid_for_save` mints a fresh uuid), the row's
                // PRIMARY KEY changes underneath every holder of the old
                // one. The DB half of that has always been correct and is
                // asserted by `an_uncontested_create_ends_up_clean_with_the_
                // real_identity` ("the placeholder uuid must be gone after
                // rekey") — but nothing recorded where it went, so a caller
                // still holding the old uuid had no way to find the row
                // again. `save_note` read that miss as "brand-new note" and
                // inserted a SECOND row, which the worker then CREATEd
                // remotely: measured 2026-08-18 on kaiwan.h@live.com, one
                // note the user edited three times became three notes in
                // Outlook and in Apple Notes (cache rows carried
                // local_version 3/2/1 — one per save that landed between
                // pushes). See gotcha #16.
                //
                // Kept as its own table rather than a `prev_uuid` column on
                // `notes`: a row can be rekeyed more than once, and the
                // lookup needs to work from a uuid whose row no longer
                // exists — which a column on that very row cannot answer.
                //
                // Chains are collapsed on write (see `rekey_note_uuid`), so
                // resolution is a single indexed lookup and never recursive.
                Step::Sql("CREATE TABLE IF NOT EXISTS note_uuid_aliases (
                    account_id TEXT NOT NULL,
                    old_uuid   TEXT NOT NULL,
                    new_uuid   TEXT NOT NULL,
                    created_at INTEGER NOT NULL,
                    PRIMARY KEY (account_id, old_uuid)
                 );
                 CREATE INDEX IF NOT EXISTS idx_note_uuid_aliases_new
                    ON note_uuid_aliases (account_id, new_uuid);"),
            ),
            (
                19,
                // account_id becomes {backend}:{email}, so two accounts can
                // share an email address on different backends. The first
                // migration that cannot be a SQL string: the backend kind of
                // each account lives in accounts.json, outside the database
                // entirely. See `migrate_account_ids` for the write order and
                // `migrate_account_ids_with` for the fail-closed rule.
                //
                // NO ROLLBACK PATH. A downgrade past this point is unsupported.
                Step::Rust(migrate_account_ids),
            ),
        ];
        for (v, step) in migrations {
    ...
    /// Build exclusions from a positive AI account allowlist; orphaned cache
    /// rows must never become eligible just because accounts.json lost an entry.
    pub fn ai_excluded_accounts(&self, allowed: &[String]) -> SqlResult<Vec<String>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare("SELECT DISTINCT account_id FROM notes")?;
    ...
    pub fn list_notes(&self, account_id: &str) -> SqlResult<Vec<CachedNote>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT uuid, account_id, id, title, body_html, date, x_mail_created_date,
                    ...
                    pinned, meta_msg_id, pin_dirty, push_blocked_reason
    ...
    /// The 17 `notes` columns, in the exact order `row_to_note` reads them.
    ///
    ...
    const NOTE_COLS: &'static str = "n.uuid, n.account_id, n.id, n.title, n.body_html, \
        ...
        n.pinned, n.meta_msg_id, n.pin_dirty, n.push_blocked_reason";
    ...
    /// data, so "descendant" has to be reconstructed by prefix matching.
    /// Literal prefix comparison matches folder_scope::Mode::Subtree, including
    /// case and wildcard characters; shared conformance tests pin the SQL form.
    ///
        ...
        let sql = format!(
            "SELECT {} FROM notes n
             WHERE n.account_id = ?1
               AND n.sync_state != 'deleted_pending'
               AND (n.label = ?2 OR substr(n.label, 1, length(?2) + 1) = (?2 || '/') COLLATE BINARY)
             ORDER BY {}",
            Self::NOTE_COLS,
            Self::NEWEST_FIRST
        );
        ...
        let sql = format!(
            "SELECT {} FROM notes n
             WHERE n.account_id = ?1
               ...
               AND (?2 IS NULL OR n.label = ?2 OR substr(n.label, 1, length(?2) + 1) = (?2 || '/') COLLATE BINARY)
             ORDER BY {}",
            Self::NOTE_COLS,
            Self::NEWEST_FIRST
        );
        ...
        let sql = format!(
            "SELECT COUNT(*) FROM notes n
             WHERE n.sync_state != 'deleted_pending'
               ...
               AND (?2 IS NULL OR n.label = ?2 OR substr(n.label, 1, length(?2) + 1) = (?2 || '/') COLLATE BINARY){excl}"
        );
        let mut binds: Vec<Box<dyn rusqlite::ToSql>> = vec![
            Box::new(account_id.map(str::to_string)),
    ...
    /// `find_citation_owner` with any number of exclusions. URL ingest must
    /// exclude both the note it reads links from and an append target.
    /// Fetches at most `exclude.len() + 1` owners, so the first survivor is
    /// found without a dynamic `NOT IN` list.
    pub fn find_citation_owner_excluding(
        &self,
        account_id: &str,
        url: &str,
        exclude: &[String],
        ...
        let mut stmt = conn.prepare(&sql)?;
        let rows = stmt.query_map(params![account_id, url, (exclude.len() + 1) as i64], row_to_note)?;
        for row in rows {
            let note = row?;
            if !exclude.iter().any(|x| x == &note.uuid) {
                return Ok(Some(note));
            }
        }
        Ok(None)
    ...
    /// Notes carrying Extract's Source block. Backs the "Extracts" view —
    /// fully virtual like `list_orphaned_notes`: provenance is derived from
    /// the body, never from the folder a note happens to be filed in.
    /// See docs/superpowers/specs/2026-09-15-extract-filing-design.md.
    pub fn list_extract_notes(&self, account_id: &str) -> SqlResult<Vec<CachedNote>> {
        let conn = self.conn.lock().unwrap();
        let sql = format!(
            "SELECT {} FROM notes n
             WHERE n.account_id = ?1 AND n.sync_state != 'deleted_pending'
               AND instr(n.body_html, ?2) > 0
             ORDER BY {}",
            Self::NOTE_COLS,
            Self::NEWEST_FIRST
        );
        ...
        let rows = stmt.query_map(
            params![account_id, crate::llm::markdown::SOURCE_MARKER],
            row_to_note,
        )?;
        rows.collect()
    }
    ...
    pub fn upsert_from_remote(&self, n: &CachedNote, pin: RemotePin) -> SqlResult<()> {
        let mut conn = self.conn.lock().unwrap();
        let tx = conn.transaction()?;
        tx.execute(
            pin.upsert_sql(),
            ...
            Some(expected) => conn.execute(
                "UPDATE notes
                 ...
                 WHERE uuid = ?5 AND account_id = ?6 AND local_version = ?7
                   AND (?8 IS NULL OR (body_html = ?8 AND title = ?9 AND label = ?10 AND sync_state != 'deleted_pending'))",
                params![title, body_html, label, now_ms(), uuid, account_id, expected,
                    reviewed.map(|n| n.body_html.as_str()), reviewed.map(|n| n.title.as_str()), reviewed.map(|n| n.label.as_str())],
            )?,
        };
        if rows_changed == 0 {
            return Ok(false);
    ...
    pub fn apply_local_edit(
        ...
        self.apply_local_edit_inner(uuid, account_id, title, body_html, label, None, None)
            .map(|_| ())
    }
    ...
    pub fn apply_local_edit_versioned(
        ...
        self.apply_local_edit_inner(uuid, account_id, title, body_html, label, Some(expected_local_version), None)
    }

    /// Review/apply requires the exact snapshot, including remote edits that
    /// keep local_version unchanged. Compare in the UPDATE, across processes.
    pub fn append_reviewed_snapshot(&self, uuid: &str, account_id: &str, reviewed: &CachedNote, body: &str) -> SqlResult<bool> {
        self.apply_local_edit_inner(uuid, account_id, &reviewed.title, body, &reviewed.label,
            Some(reviewed.local_version), Some(reviewed))
    }
    ...
    ///
    /// `id` and `remote_version` are stamped from two DIFFERENT caller-supplied
    /// values — `new_remote_id` and `new_version` — because they answer two
    /// different questions (`id` addresses the remote object; `remote_version`
    /// is the change-detection token `reconcile_one` compares against the next
    /// fetch's `Note::version`, see that field's doc comment). They happen to
    /// be equal for Gmail (its `id` IS its `version`) but are NOT equal for
    /// LocalFs (`id` = file path, `version` = Date header) or, once Task 6
    /// lands, for Microsoft (`id` = Graph id, `version` =
    /// `lastModifiedDateTime`). Every caller gets both from the same
    /// `SavedNote` the push just produced — see `SavedNote::version`.
    pub fn mark_pushed(
        &self,
        uuid: &str,
        account_id: &str,
        ...
        conn.execute(
            ...
            params![new_remote_id, now, uuid, account_id, pushed_date, pushed_body_html,
                    expected_local_version, new_version],
        )?;
        Ok(())
    }

    /// The label Jodd last CONFIRMED the remote holds for this note — see
    /// migration #16's comment for why this differs from `label` and how it
    /// stays current. `None` covers both "no row" and "row exists but the
    /// column is NULL" (never yet confirmed) — callers must treat both the
    /// same way: unknown, so assume no move is needed. Reads a single column
    /// directly rather than going through the much larger `CachedNote`
    /// (whose every construction site would otherwise need updating for a
    /// value only `push_one_dirty`'s move-dispatch check reads).
    pub fn get_remote_label(&self, uuid: &str, account_id: &str) -> SqlResult<Option<String>> {
        let conn = self.conn.lock().unwrap();
        let row: Option<Option<String>> = conn
            .query_row(
                "SELECT remote_label FROM notes WHERE uuid = ?1 AND account_id = ?2",
                params![uuid, account_id],
                |r| r.get(0),
            )
            .optional()?;
    ...
    /// Stamp `id` alone, with NO `local_version` guard — unlike `mark_pushed`.
    ///
    /// Exists for exactly one caller: `push_one_dirty` (lib.rs), right after a
    /// successful CREATE, before its own call to the (guarded) `mark_pushed`.
    /// A remote object now genuinely exists at `remote_id` regardless of
    /// whether a concurrent local edit landed mid-push and made
    /// `mark_pushed`'s optimistic lock miss — that guard decides whether the
    /// row's CONTENT (body/date/sync_state/remote_version) is safe to mark
    /// clean, not whether Jodd remembers that a remote object exists at all.
    /// Conflating the two was the bug: on a guard miss `id` used to stay
    /// however it was before the push (empty, for a brand-new note), so the
    /// next tick's `existing_remote_id` read `None` again and
    /// `save_note_full` took the CREATE branch a second time. On the
    /// Microsoft backend that produces a second, genuinely distinct
    /// `internetMessageId` — a visible duplicate note Apple's own dedupe
    /// can't collapse (`find_ids_for_uuid` is still unimplemented,
    /// `DedupSummary` is always empty for this backend) — and the poll that
    /// later sees the orphaned first note can also mint a spurious conflict
    /// copy on top, if it lands while the row is still dirty under that
    /// uuid. Gmail is exposed to the same guard-miss shape but survives it
    /// today: every push re-inserts unconditionally, so a stale `id` just
    /// means the *next* push's trash-old step targets the wrong (or no)
    /// message, and pull-side dedupe-by-uuid absorbs the resulting orphan.
    pub fn set_remote_id(&self, uuid: &str, account_id: &str, remote_id: &str) -> SqlResult<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute(
            "UPDATE notes SET id = ?1 WHERE uuid = ?2 AND account_id = ?3",
            params![remote_id, uuid, account_id],
        )?;
        Ok(())
    }
    ...
    /// Move a note to the identity its backend assigned.
    ///
    /// Exchange assigns `internetMessageId` on create; Jodd cannot choose it
    /// the way it chooses Gmail's X-UUID header, but local-first means the
    /// row exists (and is visible in the UI) before the `POST` ever returns.
    /// It is inserted under a locally-minted placeholder uuid first. Without
    /// this rekey, the next pull returns the note keyed by the real
    /// `internetMessageId`, finds no row with that uuid, and inserts a
    /// SECOND row — then `prune_clean` deletes the placeholder, so the
    /// user's new note is silently replaced, losing its pins, tags and
    /// editor attachment. Task 6 calls this from `push_one_dirty` right
    /// after a successful create.
    ///
    /// `note_tags`, `tag_tombstones` and `attachments` are the same set of
    /// uuid-keyed tables `delete()` above cleans up, moved instead of
    /// dropped. They are relocated wholesale (`UPDATE ... SET uuid = new
    /// WHERE uuid = old`) rather than re-derived. `note_tags` IS a pure
    /// function of body_html (`reconcile_tags_from_body_conn` re-derives it
    /// on every body-touching write), so moving the rows here is really just
    /// avoiding one redundant re-derive — the very next body-touching write
    /// (`apply_local_edit_inner`) would reconstruct the same set from the
    /// body regardless of this function. `tag_tombstones` is history the
    /// reconciler never recomputes at all,
    /// and attachment BLOBs obviously cannot be reconstructed from the
    /// body — for both, moving is the only option, not a choice. Gotcha #4
    /// ("derive, don't migrate") governs *content* changes; here the
    /// content is untouched and only the identity key moves, so relocating
    /// the existing rows is the correct tool, not a workaround. `edges` as
    /// a SOURCE (`src_uuid`) is the same story and moves the same way. As a
    /// DESTINATION, `edges` is keyed by `dst_id` (an 8-hex-char uuid slug —
    /// see `parse_link_key`) or by `dst_title`, never by the full uuid, so
    /// no destination row can reference `old_uuid` in the first place —
    /// there is nothing to move there. (`attachments` in particular is
    /// dead weight for the only caller this function has today — Exchange
    /// refuses attachments outright — moved here for symmetry with the
    /// other uuid-keyed tables, not because it can matter yet.)
    ///
    /// FTS is the one piece this function re-derives rather than moves:
    /// `notes_fts` has no rename-friendly UPSERT (`fts_index_conn`'s own doc
    /// comment: "FTS5 has no UPSERT → delete-then-insert"), and reusing that
    /// existing delete-then-insert helper avoids hand-rolling a second FTS
    /// write path that could drift from the first.
    ///
    /// Every moved table is cleared at `new_uuid` before the move, not just
    /// moved from `old_uuid` — `notes` self-heals against a stale row at
    /// `new_uuid` for free (`UPDATE OR REPLACE`'s conflict resolution
    /// deletes it), and so does `notes_fts` (delete-then-insert), but
    /// `note_tags`/`tag_tombstones`/`edges`/`attachments` do not: an
    /// `UPDATE ... WHERE uuid = old_uuid` only ever touches rows that still
    /// exist under `old_uuid`, so a stale row already sitting at `new_uuid`
    /// with no `old_uuid` counterpart would otherwise survive the move
    /// untouched instead of being replaced by it. That collision is real,
    /// not hypothetical: Task 6 adds `new_uuid` to `state.pushing` before
    /// calling this, but the gap between the backend's `POST` returning
    /// (Exchange now knows the note under `internetMessageId`) and that
    /// bookkeeping landing is nonzero. A poll inside that gap reaches
    /// `upsert_from_remote`, which calls `reconcile_tags_from_body_conn` /
    /// `reconcile_edges_from_body_conn` under `new_uuid`. If a local edit
    /// then lands on `old_uuid` before this function runs — removing a tag
    /// or a `[[wikilink]]` — `apply_local_edit_inner` re-derives
    /// `old_uuid`'s rows to the smaller set, and without the explicit
    /// clear-first below, the stale poll-derived row at `new_uuid` would
    /// never be touched: the tag or link the user just deleted would
    /// silently reappear under the note's new identity.
    ///
    /// One transaction: a half-rekeyed row is a note the UI cannot find.
    pub fn rekey_note_uuid(&self, old_uuid: &str, new_uuid: &str, account_id: &str) -> SqlResult<()> {
        ...
        // `OR REPLACE` is defensive, not expected: old_uuid is a placeholder
        // this account_id minted locally and new_uuid is an identity the
        // backend just assigned, so no `notes` row should already exist
        // under new_uuid. If one somehow does, replacing it is safer than a
        // panic on the UNIQUE constraint.
        tx.execute(
            "UPDATE OR REPLACE notes SET uuid = ?1 WHERE uuid = ?2 AND account_id = ?3",
            params![new_uuid, old_uuid, account_id],
        )?;

        // Clear the destination in each of these FIRST: unlike `notes`
        // above, `UPDATE ... WHERE uuid = old_uuid` never touches a row
        // that's already sitting at new_uuid with no old_uuid counterpart
        // (the poll-vs-push race documented above), so without this the
        // move would merge onto stale data instead of replacing it.
        for (table, col) in [
            ("note_tags", "uuid"),
            ("tag_tombstones", "uuid"),
            ("attachments", "note_uuid"),
        ] {
            tx.execute(
                &format!("DELETE FROM {table} WHERE {col} = ?1 AND account_id = ?2"),
                params![new_uuid, account_id],
            )?;
            tx.execute(
                &format!("UPDATE {table} SET {col} = ?1 WHERE {col} = ?2 AND account_id = ?3"),
                params![new_uuid, old_uuid, account_id],
            )?;
        }
        tx.execute(
            "DELETE FROM edges WHERE src_uuid = ?1 AND account_id = ?2",
            params![new_uuid, account_id],
        )?;
        tx.execute(
            "UPDATE edges SET src_uuid = ?1 WHERE src_uuid = ?2 AND account_id = ?3",
            params![new_uuid, old_uuid, account_id],
        )?;

        // Re-index FTS under the new key through the shared helpers. If the
        // notes UPDATE above matched no row (old_uuid didn't exist under
        // this account — not expected, but not this function's job to
        // diagnose), there is nothing to re-index either.
        if let Some((title, body_html)) = tx
            .query_row(
                "SELECT title, body_html FROM notes WHERE uuid = ?1 AND account_id = ?2",
                params![new_uuid, account_id],
                |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)),
            )
            .optional()?
        {
            fts_delete_conn(&tx, old_uuid, account_id)?;
            fts_index_conn(&tx, new_uuid, account_id, &title, &body_html)?;
        }

        // Leave a forwarding address, in the SAME transaction as the move —
        // a rekey whose alias didn't commit is exactly the silent
        // duplicate-minting state migration #18 exists to prevent.
        //
        // Two writes, not one:
        //
        //  1. Re-point aliases that already led to `old_uuid`. A note can be
        //     rekeyed twice (a create pushed, then rekeyed again by a later
        //     create), and without this an alias chain A→B, B→C would need
        //     recursive resolution — which is easy to get wrong and easy to
        //     make cyclic. Collapsing on write keeps `resolve_note_uuid` a
        //     single lookup that cannot loop.
        //  2. Record old→new itself.
        //
        // Order matters: step 1 must run BEFORE step 2, or the row just
        // inserted by step 2 (old→new) would be re-pointed by step 1's
        // `WHERE new_uuid = old_uuid` into the self-alias new→new.
        tx.execute(
            "UPDATE note_uuid_aliases SET new_uuid = ?1 WHERE new_uuid = ?2 AND account_id = ?3",
            params![new_uuid, old_uuid, account_id],
        )?;
        tx.execute(
            "INSERT OR REPLACE INTO note_uuid_aliases (account_id, old_uuid, new_uuid, created_at)
             VALUES (?1, ?2, ?3, ?4)",
            params![account_id, old_uuid, new_uuid, now_ms()],
        )?;
        // A uuid that is now a LIVE key must not also be an alias pointing
        // elsewhere: `new_uuid` could have been the `old_uuid` of an earlier
        // rekey whose row was later re-created under that same identity.
        // Resolution prefers a live row, so this is belt-and-braces — but a
        // stale self-referential entry is confusing to read in a dump.
        tx.execute(
            "DELETE FROM note_uuid_aliases WHERE old_uuid = ?1 AND account_id = ?2",
            params![new_uuid, account_id],
        )?;
    ...
    /// Follow `rekey_note_uuid`'s forwarding address: given a uuid a caller
    /// is holding, return the uuid that note lives under NOW.
    ///
    /// Returns `uuid` unchanged in the ordinary case — a live row exists
    /// under it, so there is nothing to forward. The alias table is only
    /// consulted when the lookup MISSES, which makes this cheap on the hot
    /// path and makes a live row always win over a stale alias.
    ///
    /// Also returns `uuid` unchanged when the alias target no longer exists
    /// (the note was deleted after being rekeyed). Forwarding to a dead row
    /// would be worse than not forwarding: `save_note` would insert its new
    /// row under the REMOTE identity of a note the user deleted, so the next
    /// poll would see a resurrected note rather than a new one. Falling back
    /// to the caller's own uuid keeps that case on the pre-existing path.
    ///
    /// Why this is not `Db::get`'s job: `get` is a plain read used by
    /// pollers, reconcilers and dumps, and silently answering a question
    /// about uuid A with the row for uuid B would make every one of those
    /// callers lie. Forwarding is a WRITE-path decision — "which row does
    /// this edit belong to" — so it stays an explicit call.
    pub fn resolve_note_uuid(&self, uuid: &str, account_id: &str) -> SqlResult<String> {
        let conn = self.conn.lock().unwrap();
        Self::resolve_note_uuid_conn(&conn, uuid, account_id)
    }

    // Mutation callers hold this connection through resolution and UPDATE,
    // so a worker rekey cannot slip between those two steps.
    fn resolve_note_uuid_conn(conn: &rusqlite::Connection, uuid: &str, account_id: &str) -> SqlResult<String> {
        let live: Option<i64> = conn
            .query_row(
                "SELECT 1 FROM notes WHERE uuid = ?1 AND account_id = ?2",
                params![uuid, account_id],
                |r| r.get(0),
            )
            .optional()?;
        ...
        let target: Option<String> = conn
            .query_row(
                "SELECT a.new_uuid FROM note_uuid_aliases a
                 JOIN notes n ON n.uuid = a.new_uuid AND n.account_id = a.account_id
                 WHERE a.old_uuid = ?1 AND a.account_id = ?2",
                params![uuid, account_id],
                |r| r.get(0),
            )
            .optional()?;
        Ok(target.unwrap_or_else(|| uuid.to_string()))
    }
    ...
    pub fn delete_account(&self, account_id: &str) -> SqlResult<(usize, usize)> {
        ...
        // Forwarding addresses for rows that no longer exist point nowhere —
        // `resolve_note_uuid` already ignores them (it JOINs against `notes`),
        // so this is disk hygiene rather than correctness. Left in the same
        // best-effort block as the rest for the same reason: a re-added
        // account should start from a clean slate, not inherit the previous
        // one's identity history.
        let _ = conn.execute(
            "DELETE FROM note_uuid_aliases WHERE account_id = ?1",
            params![account_id],
        );
    ...
    ///
    /// **Excludes rows whose last push was refused permanently**
    /// (`push_blocked_reason IS NOT NULL`). This is the whole retry-stopping
    /// half of the 2026-08-17 fix: `TransportError::Permanent` means retrying
    /// unchanged can never succeed, so re-listing the row every 5 s only
    /// produces log noise and network traffic — 5,816 identical failures in
    /// one measured session. The row is NOT lost: it stays `dirty` (it really
    /// does hold unsent content), it is surfaced to the UI via
    /// `list_push_blocked`, and a local edit or `clear_push_blocks` puts it
    /// straight back in this listing.
    pub fn list_dirty(&self) -> SqlResult<Vec<CachedNote>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT uuid, account_id, id, title, body_html, date, x_mail_created_date,
                    ...
                    last_synced_at, last_local_modified_at, last_remote_modified_at,
                    pinned, meta_msg_id, pin_dirty, push_blocked_reason
             FROM notes WHERE sync_state = 'dirty' AND push_blocked_reason IS NULL
    ...
    pub fn move_notes_batch(
        ...
        let mut conn = self.conn.lock().unwrap();
        let tx = conn.transaction()?;
        let resolved = uuids.iter()
            .map(|u| Self::resolve_note_uuid_conn(&tx, u, account_id))
                ...
                "UPDATE notes
                 SET label = ?1,
                     ...
                     sync_state = CASE sync_state
                         ...
                         ELSE sync_state
                     END,
                     -- A move changes the DESTINATION, which is the most
                     -- common thing a permanent refusal was about (a folder
                     -- with no reachable Exchange id — gotcha #12). Re-arm
                     -- rather than leaving the note blocked on a folder it is
                     -- no longer going to.
                     push_blocked_reason = NULL,
                     last_local_modified_at = ?2
                 WHERE uuid = ?3 AND account_id = ?4",
    ...
    pub fn set_pin(&self, uuid: &str, account_id: &str, pinned: bool) -> SqlResult<()> {
        let conn = self.conn.lock().unwrap();
        let uuid = Self::resolve_note_uuid_conn(&conn, uuid, account_id)?;
        conn.execute(
    ...
    ///
    /// `new_remote_version` is `Some((version, date))` when the sidecar push
    /// also touched the note's own remote object (Microsoft's named MAPI
    /// property — see `backend::RemoteNoteVersion`'s doc comment) and
    /// `None` when it did not (Gmail/LocalFs's separate sidecar object).
    /// Only in the `Some` case are `remote_version`/`date` updated —
    /// otherwise this call must not touch them, or a genuinely unrelated
    /// remote change made in the same window would be silently forgotten.
    /// Stamping them here on the `Some` path is exactly why this exists:
    /// without it, `notes.remote_version` would silently desync from what
    /// the property PATCH actually left on the server, and the next poll's
    /// `reconcile_one` would misread a pin-only push as a remote content
    /// change — spinning up a spurious conflict copy for a Dirty row.
    pub fn mark_pin_pushed(
        ...
        pushed_pinned: bool,
        new_remote_version: Option<(&str, &str)>,
    ) -> SqlResult<usize> {
        let conn = self.conn.lock().unwrap();
        ...
        let touched = match new_remote_version {
            Some((version, date)) => conn.execute(
                "UPDATE notes
                 SET meta_msg_id = ?1,
                     pin_dirty = 0,
                     remote_version = ?5,
                ...
                params![new_meta_msg_id, uuid, account_id, pushed_pinned as i64, version, date],
            )?,
            None => conn.execute(
                "UPDATE notes
                 SET meta_msg_id = ?1,
    ...
    // ─── Tags ──────────────────────────────────────────────────────────────
    //
    // Tags have no sidecar of their own: they round-trip as inline
    // #hashtags in the note body (see `tags_from_body` /
    // `reconcile_tags_from_body_conn`), which re-derives `note_tags` on
    // every local edit and every remote pull. This is just a read helper
    // over that derived table.

    /// Every tag on one note, sorted alphabetically.
    ...
    /// Notes pending trash on the remote. Worker picks these up alongside
    /// dirty rows.
    ///
    /// **Blocked rows are skipped, exactly as `list_dirty` skips them.** A
    /// deletion can fail permanently too — measured 2026-08-27 on iCloud,
    /// where deleting a password-protected note answers HTTP 400 forever:
    /// 491 identical requests over 3.5 hours, one every ~5 seconds, because
    /// this queue kept handing the same row back. That is gotcha #14's
    /// failure on the deletion path, which never got the rule the content
    /// path did.
        ...
        let mut stmt = conn.prepare(
            "SELECT uuid, account_id, id, title, body_html, date, x_mail_created_date,
             ...
             FROM notes WHERE sync_state = 'deleted_pending'
               AND push_blocked_reason IS NULL",
        )?;
        let rows = stmt.query_map([], row_to_note)?;
        rows.collect()
    }
    ...
    ///
    /// **A permanently-blocked note does not count** (`push_blocked_reason IS
    /// NOT NULL`) — in the DELETE queue as well as the dirty one, added
    /// 2026-08-27 when the deletion path learned to block at all. Counting
    /// it would recreate gotcha #2's wedge exactly: the
    /// worker only leaves Draining once this returns false, `remove_account`
    /// refuses a Draining account, and a block the worker will never retry
    /// never clears — so one unsendable note would make the whole account
    /// permanently unremovable. This is the same trade `push_folder` makes
    /// for a folder it drops.
    ///
    /// The consequence must be surfaced, not hidden: "nothing is pending" is
    /// now genuinely true (the worker owes nothing) while unsent content
    /// still exists on the device. `PendingPushes::blocked` carries that
    /// number separately so the UI can say both things, and both come from
    /// the queue rather than from a sync-state guess.
    pub fn has_pending_pushes(&self, account_id: &str) -> SqlResult<bool> {
        let conn = self.conn.lock().unwrap();
        let n: i64 = conn.query_row(
            "SELECT
               (SELECT COUNT(*) FROM notes
                 WHERE account_id = ?1 AND sync_state = 'dirty'
                   AND push_blocked_reason IS NULL)
             + (SELECT COUNT(*) FROM notes
                 WHERE account_id = ?1 AND sync_state = 'deleted_pending'
                   AND push_blocked_reason IS NULL)
    ...
    pub fn count_pending_pushes(&self, account_id: &str) -> SqlResult<PendingPushes> {
        ...
        Ok(PendingPushes {
            notes: one(
                "SELECT COUNT(*) FROM notes WHERE account_id = ?1 AND sync_state = 'dirty'
                   AND push_blocked_reason IS NULL",
            )?,
            blocked: one(
                "SELECT COUNT(*) FROM notes WHERE account_id = ?1
                   AND push_blocked_reason IS NOT NULL",
            )?,
            deletes: one(
    ...
    // ─── Permanently-refused pushes ───────────────────────────────────────

    /// Record that this note's push was refused permanently, and stop the
    /// worker retrying it.
    ///
    /// `sync_state` is left alone on purpose. The row IS still dirty — it
    /// holds local content no remote has — and saying otherwise is the lie
    /// this whole path exists to remove. What changes is only whether
    /// `list_dirty` hands it to the worker again. See migration #17.
    ///
    /// Idempotent, and it deliberately overwrites an existing reason: the
    /// second failure's message is the current one, and a stale reason shown
    /// to the user is worse than none.
    pub fn mark_push_blocked(&self, uuid: &str, account_id: &str, reason: &str) -> SqlResult<()> {
        let conn = self.conn.lock().unwrap();
        conn.execute(
            "UPDATE notes SET push_blocked_reason = ?1 WHERE uuid = ?2 AND account_id = ?3",
            params![reason, uuid, account_id],
        )?;
        Ok(())
    }

    /// Re-arm every blocked note in one account, returning how many were
    /// cleared. Backs the explicit "Try again" action.
    ///
    /// There is no automatic re-arm, by decision. The two permanent refusals
    /// this backend actually produces — an attachment on Exchange, and a
    /// subfolder with no reachable Exchange id (gotcha #12) — have no cheap
    /// signal that says "the cause is gone now", and a timer-based retry
    /// would just be the 5-second loop with extra steps. The user editing the
    /// note (`apply_local_edit` clears the reason) or pressing the button is
    /// the signal. If a future backend gains a real "blocker cleared" event,
    /// that event should call this — not a poll.
    pub fn clear_push_blocks(&self, account_id: &str) -> SqlResult<usize> {
        let conn = self.conn.lock().unwrap();
        let n = conn.execute(
            "UPDATE notes SET push_blocked_reason = NULL
             WHERE account_id = ?1 AND push_blocked_reason IS NOT NULL",
            params![account_id],
        )?;
        Ok(n)
    }

    /// Every note in one account whose push was permanently refused.
    ///
    /// The read side of "the UI must be able to see this". It goes through
    /// SQLite rather than any list path that talks to the backend, because a
    /// blocked note has **no remote object at all** in the case that matters
    /// (a create that never landed) — `list_notes` and `list_notes_in_folder`
    /// both build their result from what the remote returned, so a blocked
    /// note is absent from both by construction. That is precisely how a note
    /// the UI called "Saved" left the list 30 seconds later.
    pub fn list_push_blocked(&self, account_id: &str) -> SqlResult<Vec<CachedNote>> {
        let conn = self.conn.lock().unwrap();
        let mut stmt = conn.prepare(
            "SELECT uuid, account_id, id, title, body_html, date, x_mail_created_date,
                    label, local_version, remote_version, sync_state,
             ...
             FROM notes
             WHERE account_id = ?1 AND push_blocked_reason IS NOT NULL
               AND sync_state != 'deleted_pending'
             ORDER BY last_local_modified_at DESC",
        )?;
        let rows = stmt.query_map(params![account_id], row_to_note)?;
        rows.collect()
...
impl CachedNote {
    ...
    pub fn to_frontend_note(&self) -> crate::backend::gmail::wire::Note {
        crate::backend::gmail::wire::Note {
            ...
            // The `Note` this produces feeds `cache_by_msg_id` (lib.rs), which
            // every vertical's `list_all_notes`/`list_notes_in_folder` receives
            // as a fan-out optimization: an id already in the cache can skip
            // its own remote fetch and reuse the cached fields, `version`
            // included. That is safe today only because Gmail is the sole
            // vertical that actually reads the map, and Gmail's `id` IS its
            // `version` (see `SavedNote::version`'s doc comment), so recycling
            // `remote_version` here can never disagree with a fresh fetch's
            // `Note::version`. Microsoft does NOT read `cache_by_id` today
            // (`_cache_by_id` in `backend/microsoft/mod.rs` is unused — a Graph
            // list response already carries the body, so there is no per-
            // message GET to skip) — but the moment a later task makes it do
            // so, a cache-hit note would report `version = remote_version`
            // unconditionally, `remote_changed` would read `false` by
            // construction regardless of what actually changed on Graph, and
            // the exact silent-overwrite bug this task exists to eliminate
            // would return with no test failing. Same trap, same shape as the
            // one flagged on Microsoft's `save_note_full` stub for Task 6.
            version: self.remote_version.clone().unwrap_or_default(),
            x_mail_created_date: self.x_mail_created_date.clone(),
            label: self.label.clone(),
            account_id: Some(self.account_id.clone()),
            pinned: self.pinned,
            local_version: self.local_version,
            // Jodd-local, cache-only, and the reason this field exists at all:
            // the frontend cannot otherwise distinguish a note that synced
            // from one the worker has given up on. Same shape as `pinned` —
            // never on the wire, populated here from SQLite.
            push_blocked_reason: self.push_blocked_reason.clone(),
    ...
    /// remote_version = the vertical's `Note::version` (Gmail: message id;
    /// Microsoft: lastModifiedDateTime; LocalFs: Date header) — NOT always
    /// the message id, see that field's doc comment — last_synced_at = now,
    /// last_remote_modified_at parsed from the Date header. local_version
    /// stays at 0 (caller hasn't edited locally).
    pub fn from_remote(account_id: &str, n: &crate::backend::gmail::wire::Note) -> Self {
        ...
        CachedNote {
            ...
            label: n.label.clone(),
            local_version: 0,
            // Was `n.id`. See `backend::Note::version` — the id is only a
            // change signal on backends that re-mint it, which is not all of them.
            remote_version: Some(n.version.clone()),
            sync_state: SyncState::Clean,
            last_synced_at: Some(now),
            last_local_modified_at: now,
            last_remote_modified_at: last_remote,
            // Carried through rather than zeroed. Whether it is allowed to
            // OVERWRITE a cached pin is `RemotePin`'s decision, made per
            // backend at the call site — this only stops the value being
            // thrown away before that decision can be made.
            //
            // It used to be a hardcoded `false`, on the same reasoning as
            // upsert_from_remote's ON CONFLICT clause: the pin is a Jodd
            // sidecar and no remote carries one. On the sidecar backends
            // nothing changes — a wire-parsed Note is unpinned there anyway,
            // so `n.pinned` IS false and LocalWins protects the cached value
            // on every update. On iCloud the pin is Apple's own, and zeroing
            // it here made `RemotePin::RemoteWins` faithfully write a false
            // it had already destroyed: the third and last layer this pin had
            // to survive, after the wrong record type and the ON CONFLICT
            // clause. The symptom was a pin visible in a freshly-fetched
            // folder and absent everywhere the list came from the cache.
            //
            // Sidecar pin reconciliation (apply_remote_pin /
            // clear_pins_not_in) is unchanged and still owns the pin on the
            // backends that keep one in a sidecar.
            pinned: n.pinned,
            meta_msg_id: None,
            pin_dirty: false,
...
impl Db {
    ...
    /// A folder row's sync state INCLUDING `deleted_pending`, which
    /// `list_folders` hides. `None` = no row. `llm::filing` needs the
    /// difference: a pending-deletion Inbox must not be reused or recreated.
    pub fn folder_sync_state(&self, account_id: &str, path: &str) -> SqlResult<Option<FolderSyncState>> {
        let conn = self.conn.lock().unwrap();
        conn.query_row(
        ...
        .optional()
        .map(|s| s.map(|s| FolderSyncState::from_str(&s)))
    }

    /// Test-only: put a folder row into an arbitrary sync state. Reaching
    /// `deleted_pending` honestly needs a clean row first (a `dirty_new`
    /// delete drops the row), which needs a remote.
    #[cfg(test)]
    pub(crate) fn force_folder_sync_state(&self, account_id: &str, path: &str, state: FolderSyncState) {
        let conn = self.conn.lock().unwrap();
        conn.execute(
            "UPDATE folders SET sync_state = ?1 WHERE account_id = ?2 AND path = ?3",
            params![state.as_str(), account_id, path],
        )
        .unwrap();
    }
    ...
    /// `kind` is re-derived from the new leaf (gotcha #4).
    pub fn rename_folder(
        ...
        conn.execute(
            ...
            params![new_path, now_ms(), account_id, old_path, derive_workflow_kind(new_path)],
        )?;
        Ok(())
    }
    ...
    /// update all notes' label field to match; `kind` of the renamed folder
    /// is re-derived from its new leaf (gotcha #4). Used for move/rename ops
    ...
    pub fn rename_subtree(
        ...
        // Re-derive kind for the renamed folder itself: only the top row's leaf
        // changes. Descendants keep their own leaf names, so their derived kind
        // is unchanged.
        tx.execute(
            "UPDATE folders SET kind = ?1 WHERE account_id = ?2 AND path = ?3",
            params![derive_workflow_kind(new_path), account_id, new_path],
        )?;
...
mod tests_workflow_folder {
    ...
    /// Gotcha #4: kind is DERIVED from the leaf. A device that pulls the
    /// renamed label derives `user`; if the renaming device kept
    /// `system_workflow`, the same folder would sit in different Sidebar
    /// groups on different devices. Reachable since extract filing made
    /// `__Extracts__` renamable. This test exercises the production path
    /// (rename_subtree), not the direct rename_folder API.
    #[test]
    fn renaming_a_folder_re_derives_its_kind_from_the_new_leaf() {
        let db = temp_db();
        let acct = "test@example.com";
        db.ensure_workflow_folder(acct, "__Extracts__").unwrap();
        db.rename_subtree(acct, "Notes/__Extracts__", "Notes/Extracts").unwrap();
        db.create_folder_local_new(acct, "Notes/Plain").unwrap();
        db.rename_subtree(acct, "Notes/Plain", "Notes/Still Plain").unwrap();

        let kinds: std::collections::HashMap<String, String> =
            db.list_folders(acct).unwrap().into_iter().map(|f| (f.path, f.kind)).collect();
        assert_eq!(kinds.get("Notes/Extracts").map(String::as_str), Some("user"));
        assert_eq!(kinds.get("Notes/Still Plain").map(String::as_str), Some("user"));
    }
}

#[cfg(test)]
mod tests_open_encryption {
    use super::*;
    use tempfile::tempdir;

    #[test]
    fn open_generates_a_key_and_encrypts_a_fresh_install() {
        crate::secrets::init().expect("credential store must initialize");
        let dir = tempdir().unwrap();
        let path = dir.path().to_path_buf();

        let _db = Db::open(&path).expect("fresh install must open cleanly");
        let db_file = path.join("jodd.sqlite3");
        assert!(!crate::db_crypto::is_plaintext_sqlite(&db_file), "fresh DB must be encrypted");
        assert!(
            crate::db_crypto::load_key_hex().expect("key read must not error").is_some(),
            "a key must have been persisted"
        );
        crate::db_crypto::delete_test_key();
    }

    #[test]
    fn open_migrates_an_existing_plaintext_db() {
        crate::secrets::init().expect("credential store must initialize");
        let dir = tempdir().unwrap();
        let path = dir.path().to_path_buf();
        let db_file = path.join("jodd.sqlite3");
        // Create a plaintext DB the old way, bypassing Db::open entirely.
        Connection::open(&db_file).unwrap().execute_batch("PRAGMA journal_mode=WAL;").unwrap();
        assert!(crate::db_crypto::is_plaintext_sqlite(&db_file));

        let _db = Db::open(&path).expect("migration must succeed");
        assert!(!crate::db_crypto::is_plaintext_sqlite(&db_file), "must be encrypted after open()");
        // The plaintext backup is deleted once the encrypted DB has proven it
        // opens and passes its own migrations — keeping it would leave a full
        // unencrypted copy of every note on disk forever.
        assert!(
            !path.join("jodd.sqlite3.plaintext-backup").exists(),
            "plaintext backup must not survive a fully successful migration"
        );
        crate::db_crypto::delete_test_key();
    }

    /// Finding #1: a keychain read that FAILS (as opposed to "no entry") must
    /// never lead to minting a fresh key over an existing encrypted DB.
    #[test]
    fn open_refuses_to_mint_a_fresh_key_over_an_existing_encrypted_db() {
        crate::secrets::init().expect("credential store must initialize");
        let dir = tempdir().unwrap();
        let path = dir.path().to_path_buf();
        let db_file = path.join("jodd.sqlite3");

        // An encrypted DB written with a key that is NOT in the keychain,
        // standing in for "the key could not be read".
        let orphan_key = crate::db_crypto::generate_key_hex();
        {
            let conn = Connection::open(&db_file).unwrap();
            conn.execute_batch(&format!(
                "PRAGMA key = \"x'{orphan_key}'\"; CREATE TABLE t (x INTEGER);"
            ))
            .unwrap();
        }
        crate::db_crypto::delete_test_key();
        ...
        match Db::open(&path) {
            Err(crate::db_crypto::DbOpenError::Keychain(msg)) => {
                assert!(msg.contains("appears encrypted"), "unexpected message: {msg}");
            }
            Err(other) => panic!("expected DbOpenError::Keychain, got {other}"),
            Ok(_) => panic!("expected DbOpenError::Keychain, but open() succeeded"),
        }
        // And the file is untouched — no fresh key was written over it.
        assert!(crate::db_crypto::open_encrypted(&db_file, &orphan_key).is_ok());
    }

    /// Finding #5: dying between migration's two renames leaves only a
    /// `.plaintext-backup`. That must be resumed, not read as a fresh install.
    #[test]
    fn open_resumes_a_migration_interrupted_between_the_two_renames() {
        crate::secrets::init().expect("credential store must initialize");
        let dir = tempdir().unwrap();
        let path = dir.path().to_path_buf();
        let backup = path.join("jodd.sqlite3.plaintext-backup");
        {
            let conn = Connection::open(&backup).unwrap();
            conn.execute_batch("CREATE TABLE marker (x INTEGER); INSERT INTO marker VALUES (5);")
                .unwrap();
        }
        // A stale partial tmp file from the crashed attempt, too (finding #6).
        {
            let conn = Connection::open(path.join("jodd.sqlite3.encrypting")).unwrap();
            conn.execute_batch("CREATE TABLE marker (x INTEGER);").unwrap();
        }

        let db = Db::open(&path).expect("interrupted migration must resume");
        let value: i64 = db
            .conn
            .lock()
            .unwrap()
            .query_row("SELECT x FROM marker", [], |r| r.get(0))
            .expect("the user's data must survive, not be replaced by an empty DB");
        assert_eq!(value, 5);
        crate::db_crypto::delete_test_key();
    }

    #[test]
    fn open_unencrypted_never_touches_the_keychain_or_encrypts() {
        let dir = tempdir().unwrap();
        let path = dir.path().to_path_buf();
        let _db = Db::open_unencrypted(&path).expect("must open cleanly");
        let db_file = path.join("jodd.sqlite3");
        assert!(crate::db_crypto::is_plaintext_sqlite(&db_file), "open_unencrypted must stay plaintext");
    }
}
...
mod tests_migrate {
    ...
    // Characterization guard for the Step refactor (task 2 of the account-
    // identity plan): proves migrate() still applies every shipped migration
    // exactly once, both before AND after `migrations` moves from
    // `&[(i64, &str)]` to `&[(i64, Step)]`. It must stay green across that
    // refactor — a red run here would mean the refactor changed behaviour.
    #[test]
    fn a_rust_migration_step_runs_and_is_recorded() {
        let dir = tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        let conn = db.conn.lock().unwrap();
        // Every shipped migration must be recorded exactly once after open().
        let n: i64 = conn
            .query_row("SELECT COUNT(*) FROM migrations", [], |r| r.get(0))
            .unwrap();
        assert!(n >= 18, "expected the shipped migrations to be recorded, got {n}");
        // Named explicitly, because `n >= 18` pins no particular version: the
        // only `Step::Rust` in the slice is migration 19, so without this the
        // registration line is compile-verified and nothing more — deleting
        // the `(19, Step::Rust(..))` entry would leave this test green.
        let has_19: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM migrations WHERE version = 19",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(has_19, 1, "migration 19 must be registered and recorded");
        let dupes: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM (SELECT version FROM migrations GROUP BY version HAVING COUNT(*) > 1)",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(dupes, 0, "a version was recorded twice");
    }
}

/// Strip HTML tags to plain text for the FTS index (so searches match note
/// content, not tag/attribute names). Decodes character references and
/// collapses runs of whitespace.
///
/// The three steps are ordered deliberately:
///
/// 1. **strip tags first** — decoding first would turn a body's literal
///    `&lt;b&gt;` into `<b>`, which this loop would then eat along with the
///    text after it: silent content loss, no error;
/// 2. **decode second** — before this was wired in, `&nbsp;` reached the FTS
///    index verbatim, so searching `nbsp` matched notes containing no such
///    word while a phrase spanning the entity did not match at all;
/// 3. **collapse whitespace last** — `&nbsp;` decodes to U+00A0, which
///    `split_whitespace` treats as whitespace, so this is the step that
///    actually normalises Apple's hard spaces into ordinary ones.
...
/// Longest character reference we will look at, in bytes after the `&`.
/// The widest thing we decode is a padded hex ref like `#x0010FFFF` (10), so
/// this is generous; its real job is to stop a stray `&` in prose from
/// scanning to a distant `;` ("Tom & Jerry; friends").
const MAX_CHAR_REF_LEN: usize = 12;

/// Resolve one character reference body (the text between `&` and `;`) to the
/// character it denotes, or `None` if it isn't one we decode.
///
/// The named set is deliberately small — the entities a contenteditable's
/// `innerHTML` serializer and Apple Notes actually emit. `&nbsp;` maps to a
/// real U+00A0 rather than to a plain space: that is what the entity *means*,
/// and every caller either collapses whitespace afterwards
/// (`strip_html_to_text`) or wants the run to terminate there (`extract_urls`),
/// both of which treat U+00A0 as whitespace already.
fn decode_char_ref(body: &str) -> Option<char> {
    if let Some(num) = body.strip_prefix('#') {
        ...
        // char::from_u32 already rejects surrogates and anything above
        // U+10FFFF, so malformed refs need no explicit range guard here.
        return char::from_u32(cp).filter(|c| *c != '\0');
    }
    match body {
        "nbsp" => Some('\u{A0}'),
        "amp" => Some('&'),
...
/// Decode the HTML/XML character references that show up in real note bodies
/// (a contenteditable's `innerHTML` serialization escapes `&` to `&amp;`,
/// `<` to `&lt;`, etc. on save; Apple emits `&nbsp;` for consecutive spaces)
/// so a URL extracted from stored `body_html` compares equal, byte-for-byte,
/// to the same URL scanned from raw, unescaped text (e.g. Extract's pasted
/// source) — and so text derived for the FTS index, Ask Jodd's context and
/// autolink carries the characters the note actually reads as.
///
/// **Exactly one level of decoding, never two.** A genuinely double-escaped
/// `&amp;lt;` (the literal text "&lt;") must decode to `&lt;` and stop. The
/// original implementation got this from a `.replace()` chain that ran
/// `&amp;` last; the single forward pass below gets it structurally, because
/// output is never rescanned. Anything unrecognised — unknown name, malformed
/// or out-of-range number, bare `&` in prose — is copied through verbatim
/// rather than dropped.
pub(crate) fn decode_html_entities(s: &str) -> std::borrow::Cow<'_, str> {
    if !s.contains('&') {
    ...
    while i < bytes.len() {
        if bytes[i] == b'&' {
            // Byte-level scan for the terminator is safe: ASCII ';' can never
            // appear inside a multi-byte UTF-8 sequence (continuation bytes
            // are all >= 0x80), so this can't split a character boundary.
            let end = bytes[i + 1..]
                .iter()
                .take(MAX_CHAR_REF_LEN)
                .position(|&b| b == b';');
            if let Some(rel) = end {
                if rel > 0 {
                    if let Some(c) = decode_char_ref(&s[i + 1..i + 1 + rel]) {
                        out.push(c);
                        i += rel + 2; // '&' + body + ';'
                        continue;
                    }
        ...
        // Not a reference we decode — copy one whole character through.
...
mod tests {
    use super::{
        decode_html_entities, extract_hashtags, extract_urls, extract_wikilinks, note_slug,
        parse_link_key, rewrite_hashtag_in_body, rewrite_wikilink_slug_in_body, slugify,
        strip_html_to_text,
    };

    #[test]
    fn strip_html_to_text_decodes_nbsp_and_collapses_it() {
        // The reported defect (2026-08-14): Apple emits &nbsp; routinely for
        // consecutive spaces, and every consumer of this helper — FTS, Ask
        // Jodd's context, autolink — saw the six literal characters.
        assert_eq!(
            strip_html_to_text("<div>2be-throwaway in&nbsp; JODD-PROBE-B</div>"),
            "2be-throwaway in JODD-PROBE-B"
        );
    }

    #[test]
    fn strip_html_to_text_decodes_amp_without_re_decoding_its_payload() {
        assert_eq!(strip_html_to_text("<p>Tom &amp; Jerry</p>"), "Tom & Jerry");
        // Single level, deliberately: `&amp;nbsp;` is the author writing the
        // literal text "&nbsp;", not a hard space.
        assert_eq!(
            strip_html_to_text("<p>literal &amp;nbsp; entity</p>"),
            "literal &nbsp; entity"
        );
    }

    #[test]
    fn strip_html_to_text_strips_tags_before_decoding() {
        // If decoding ran first, `&lt;b&gt;` would become `<b>` and the tag
        // stripper would then delete it along with the text after it.
        assert_eq!(
            strip_html_to_text("<div>see &lt;b&gt;bold&lt;/b&gt; here</div>"),
            "see <b>bold</b> here"
        );
    }
    ...
    fn decode_html_entities_handles_numeric_references() {
        ...
        assert_eq!(decode_html_entities("&#x1F600; and &#X2014;"), "😀 and —");
    }

    #[test]
    fn decode_html_entities_leaves_malformed_and_unknown_refs_alone() {
        // char::from_u32 rejects surrogates and >U+10FFFF, so these fall
        // through to being copied verbatim rather than panicking.
        assert_eq!(
            decode_html_entities("bad &#99999999; &#xFFFFFFFF; &#xD800; refs"),
            "bad &#99999999; &#xFFFFFFFF; &#xD800; refs"
        );
        assert_eq!(decode_html_entities("&notarealentity; stays"), "&notarealentity; stays");
        // A bare ampersand in prose is not a reference.
        assert_eq!(decode_html_entities("Tom & Jerry; friends"), "Tom & Jerry; friends");
    }

    #[test]
    fn decode_html_entities_preserves_the_one_level_rule_for_all_names() {
        // Pins the property the original `.replace()` chain got by ordering
        // `&amp;` last: exactly one level of decoding, never two.
    ...
    #[test]
    fn extract_urls_terminates_at_a_decoded_nbsp() {
        // Before entity decoding reached strip_html_to_text, `&nbsp;` had no
        // whitespace in it, so a URL followed by one swallowed the entity and
        // the word after it into a single bogus citation.
        assert_eq!(
            extract_urls(&strip_html_to_text("<div>see https://example.com/page&nbsp;then stop</div>")),
...
mod tests_citations {
    ...
    /// URL ingest excludes the source note AND an append target: a
    /// link-collection note always "cites" its own links.
    #[test]
    fn find_citation_owner_excluding_skips_every_excluded_uuid() {
        let db = crate::test_support::temp_db();
        let acct = "gmail:x@example.com";
        let (a, b) = ("AAAAAAAA-0000-0000-0000-00000000000A", "BBBBBBBB-0000-0000-0000-00000000000B");
        crate::test_support::note(acct, a).body("see https://shared.example/v").insert(&db);
        crate::test_support::note(acct, b).body("also https://shared.example/v").insert(&db);
        let url = "https://shared.example/v";
        assert!(db.find_citation_owner_excluding(acct, url, &[]).unwrap().is_some());
        assert_eq!(db.find_citation_owner_excluding(acct, url, &[a.to_string()]).unwrap().map(|n| n.uuid).as_deref(), Some(b));
        assert_eq!(db.find_citation_owner_excluding(acct, url, &[a.to_string(), b.to_string()]).unwrap(), None);
    }
}
...
mod tests_smart_folders {
    ...
    /// The Extracts view is fully virtual, like Orphaned and Stale: it is
    /// derived from the Source block Extract writes, not from a folder, so an
    /// extract filed into any folder still shows up here.
    #[test]
    fn list_extract_notes_follows_the_source_marker_not_the_folder() {
        let db = temp_db();
        let acct = "test@example.com";
        let marker = crate::llm::markdown::SOURCE_MARKER;
        let filed = "11111111-0000-0000-0000-000000000001";
        let plain = "11111111-0000-0000-0000-000000000002";
        let deleted = "11111111-0000-0000-0000-000000000003";

        let mut filed_note = mk_note(
            acct,
            filed,
            "Filed extract",
            &format!("<h2>Point</h2><details>{marker}<pre>src</pre></details>"),
        );
        filed_note.label = "Notes/Trading".to_string();
        db.insert_local_new(&filed_note).unwrap();
        db.insert_local_new(&mk_note(acct, plain, "Plain note", "<div>no source</div>")).unwrap();
        db.insert_local_new(&mk_note(
            acct,
            deleted,
            "Deleted extract",
            &format!("<details>{marker}<pre>src</pre></details>"),
        ))
        .unwrap();
        {
            let conn = db.conn.lock().unwrap();
            conn.execute(
                "UPDATE notes SET sync_state = 'deleted_pending' WHERE uuid = ?1",
                params![deleted],
            )
            .unwrap();
        }
        ...
        let uuids: Vec<String> = db
            .list_extract_notes(acct)
            .unwrap()
            .into_iter()
            .map(|n| n.uuid)
            .collect();
        assert_eq!(uuids, vec![filed.to_string()]);
    }
}
...
mod tests_rekey {
    ...
    /// The core guarantee: everything keyed by the old uuid — the row
    /// itself, its derived tags, its outgoing links, its tag tombstones,
    /// its attachments, and its FTS entry — follows it to the new uuid,
    /// and nothing is left behind under the old one. This is what makes a
    /// Microsoft-vertical create converge onto the real `internetMessageId`
    /// instead of leaving a placeholder row for `prune_clean` to quietly
    /// delete out from under the user.
    ///
    /// Deliberately exercises all five uuid-keyed tables `rekey_note_uuid`
    /// touches (notes, note_tags, tag_tombstones, attachments, edges) plus
    /// FTS: dropping any one of the move statements from the implementation
    /// should fail this test, not just shrink its coverage silently.
    #[test]
    fn rekey_moves_the_row_and_rederives_everything_keyed_by_uuid() {
        let db = temp_db();
        // Explicit non-colliding body: the builder's own default filler text
        // ("body of {uuid}") would otherwise also match the FTS "body"
        // search below and make the hit-count assertion ambiguous.
        note("acct", "target-uuid").title("Target").body("nothing here").insert(&db);
        note("acct", "temp-uuid")
            .title("Title")
            .body("body #work [[Target]]")
            .insert(&db);
        db.upsert_attachment("acct", "temp-uuid", &attachment("cid-1")).unwrap();
        {
            let conn = db.conn.lock().unwrap();
            conn.execute(
                "INSERT INTO tag_tombstones (account_id, uuid, tag, deleted_at) VALUES (?1, ?2, ?3, ?4)",
                params!["acct", "temp-uuid", "ghost", 12_345_i64],
            )
            .unwrap();
        }

        db.rekey_note_uuid("temp-uuid", "<real@exchange>", "acct").unwrap();

        assert!(db.get("temp-uuid", "acct").unwrap().is_none(), "old row must be gone");
        let moved = db.get("<real@exchange>", "acct").unwrap().expect("row under the new uuid");
        assert_eq!(moved.title, "Title");
        assert_eq!(moved.body_html, "<div>Title</div><div>body #work [[Target]]</div>");
        assert_eq!(moved.sync_state, SyncState::Clean);

        // Tags are derived from the body's #work hashtag on insert
        // (reconcile_tags_from_body_conn) — the move must carry that
        // derived state along, not orphan it under the old key.
        assert!(db.list_tags_for("acct", "temp-uuid").unwrap().is_empty(), "old tags must be gone");
        assert_eq!(db.list_tags_for("acct", "<real@exchange>").unwrap(), vec!["work".to_string()]);

        // edges: the [[Target]] wikilink is a `mentions` edge keyed by
        // src_uuid — it must resolve under the new uuid and not the old.
        assert!(
            db.outgoing_links("acct", "temp-uuid").unwrap().is_empty(),
            "old outgoing links must be gone"
        );
        let links = db.outgoing_links("acct", "<real@exchange>").unwrap();
        assert_eq!(links.len(), 1);
        assert_eq!(links[0].uuid, "target-uuid");

        // attachments: BLOBs, not derivable from the body — must move as
        // rows, not be re-created.
        assert!(
            db.list_attachments("acct", "temp-uuid").unwrap().is_empty(),
            "old attachments must be gone"
        );
        let atts = db.list_attachments("acct", "<real@exchange>").unwrap();
        assert_eq!(atts.len(), 1);
        assert_eq!(atts[0].content_id, "cid-1");

        // tag_tombstones: history, not derivable from anything — must move
        // as rows too.
        let conn = db.conn.lock().unwrap();
        let old_tombstones: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM tag_tombstones WHERE account_id = ?1 AND uuid = ?2",
                params!["acct", "temp-uuid"],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(old_tombstones, 0, "old tombstones must be gone");
        let new_tombstones: Vec<String> = conn
            .prepare("SELECT tag FROM tag_tombstones WHERE account_id = ?1 AND uuid = ?2")
            .unwrap()
            .query_map(params!["acct", "<real@exchange>"], |r| r.get(0))
            .unwrap()
            .collect::<Result<_, _>>()
            .unwrap();
        assert_eq!(new_tombstones, vec!["ghost".to_string()]);
        drop(conn);

        // FTS must find it under the new uuid and not the old.
        let hits = db.search_notes(Some("acct"), None, "body", &[]).unwrap();
        assert_eq!(hits.len(), 1);
        assert_eq!(hits[0].uuid, "<real@exchange>");
    }

    #[test]
    fn rekey_is_a_noop_when_the_uuid_is_unchanged() {
        let db = temp_db();
        note("acct", "same").title("T").insert(&db);
        db.rekey_note_uuid("same", "same", "acct").unwrap();
        assert!(db.get("same", "acct").unwrap().is_some(), "row must survive");
    }

    /// Fix-round finding: `note_tags`/`tag_tombstones`/`edges`/`attachments`
    /// move via `UPDATE ... WHERE uuid = old_uuid`, which never touches a
    /// row that's already sitting at `new_uuid` with no `old_uuid`
    /// counterpart. That collision is real, not contrived — see the "poll
    /// landing in the gap" paragraph on `rekey_note_uuid`'s doc comment: a
    /// poll inside the gap between the backend's create returning and this
    /// function being called reaches `upsert_from_remote`, which derives
    /// tags/edges directly under `new_uuid`. This test plants exactly that
    /// outcome — rows at `new_uuid` with nothing under `old_uuid` to have
    /// produced them — and asserts the rekey REPLACES them instead of
    /// merging alongside them. Before the destination-clear fix, every
    /// assertion below fails: the stale rows survive next to the real ones.
    #[test]
    fn rekey_clears_stale_rows_already_at_the_destination_instead_of_merging() {
        let db = temp_db();
        note("acct", "target-uuid").title("Target").insert(&db);
        note("acct", "stale-target-uuid").title("StaleTarget").insert(&db);
        note("acct", "temp-uuid")
            .title("Title")
            .body("body #work [[Target]]")
            .insert(&db);

        // Plant poll-derived leftovers at new_uuid with NO old_uuid row
        // behind them — exactly what a poll landing mid-race would leave.
        db.upsert_attachment("acct", "<real@exchange>", &attachment("stale-cid")).unwrap();
        {
            let conn = db.conn.lock().unwrap();
            conn.execute(
                "INSERT INTO note_tags (account_id, uuid, tag) VALUES (?1, ?2, ?3)",
                params!["acct", "<real@exchange>", "stale-tag"],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO tag_tombstones (account_id, uuid, tag, deleted_at) VALUES (?1, ?2, ?3, ?4)",
                params!["acct", "<real@exchange>", "stale-ghost", 99_999_i64],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO edges (account_id, src_uuid, dst_id, dst_title, rel)
                 VALUES (?1, ?2, '', ?3, 'mentions')",
                params!["acct", "<real@exchange>", "staletarget"],
            )
            .unwrap();
        }
        ...
        assert_eq!(
            db.list_tags_for("acct", "<real@exchange>").unwrap(),
            vec!["work".to_string()],
            "stale-tag must be replaced, not merged alongside the real tag"
        );

        assert!(
            db.list_attachments("acct", "<real@exchange>").unwrap().is_empty(),
            "stale-cid must be cleared — temp-uuid never had an attachment to move"
        );
        ...
        assert_eq!(
            links.len(),
            1,
            "the stale edge to StaleTarget must be cleared, not merged with the real Target link"
        );
        assert_eq!(links[0].uuid, "target-uuid");

        let conn = db.conn.lock().unwrap();
        let stale_tombstones: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM tag_tombstones
                 WHERE account_id = ?1 AND uuid = ?2 AND tag = 'stale-ghost'",
                params!["acct", "<real@exchange>"],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(
            stale_tombstones, 0,
            "stale-ghost tombstone must be cleared — temp-uuid never had one to move"
        );
    }
}
...
mod drain_complete_tests {
    ...
    fn has_pending_pushes_sees_every_queue() {
        ...
        db.mark_pushed("u1", acct, "mid-1", "mid-1", DATE, "<div>b</div>", 2).unwrap();
        ...
        db.mark_pin_pushed("u1", acct, Some("meta-1"), true, None).unwrap();
    ...
    // ─── Permanently-refused pushes (2026-08-17) ─────────────────────────

    /// The retry-stopping half. A note the backend refused permanently must
    /// leave the worker's queue — it was re-listed every 5 s for 80 minutes
    /// in the measured case — while still being, truthfully, dirty.
    #[test]
    fn a_blocked_note_leaves_the_worker_queue_but_stays_dirty() {
        let db = temp_db();
        let acct = "a@example.com";
        note(acct, "u1").insert(&db);
        db.apply_local_edit("u1", acct, "t", "<div>b</div>", "Notes").unwrap();
        assert_eq!(db.list_dirty().unwrap().len(), 1, "precondition: queued");

        db.mark_push_blocked("u1", acct, "permanent: no Exchange folder id").unwrap();

        assert!(db.list_dirty().unwrap().is_empty(), "the worker must stop retrying it");
        let row = db.get("u1", acct).unwrap().unwrap();
        assert_eq!(row.sync_state, SyncState::Dirty, "it still holds unsent content");
        assert_eq!(
            row.push_blocked_reason.as_deref(),
            Some("permanent: no Exchange folder id"),
            "and the reason is on the row, not only in a log line"
        );
    }

    /// Gotcha #2's wedge, closed. The worker only leaves Draining once
    /// `has_pending_pushes` is false, and `remove_account` refuses a Draining
    /// account — so a row the worker will never retry must not count, or one
    /// unsendable note makes the account permanently unremovable.
    #[test]
    fn a_blocked_note_cannot_wedge_an_account_into_draining_forever() {
        let db = temp_db();
        let acct = "a@example.com";
        note(acct, "u1").insert(&db);
        db.apply_local_edit("u1", acct, "t", "<div>b</div>", "Notes").unwrap();
        assert!(db.has_pending_pushes(acct).unwrap(), "precondition: pending");

        db.mark_push_blocked("u1", acct, "permanent: nope").unwrap();

        assert!(
            !db.has_pending_pushes(acct).unwrap(),
            "the worker owes nothing it can act on, so the drain must be allowed to finish"
        );
    }

    /// "Nothing pending" and "nothing unsent" are now different statements,
    /// and the UI has to be able to say both. A blocked note in the `notes`
    /// count would show an "N left" that never decrements.
    #[test]
    fn blocked_notes_are_counted_apart_from_queued_ones() {
        let db = temp_db();
        let acct = "a@example.com";
        for u in ["u1", "u2"] {
            note(acct, u).insert(&db);
            db.apply_local_edit(u, acct, "t", "<div>b</div>", "Notes").unwrap();
        }
        db.mark_push_blocked("u2", acct, "permanent: nope").unwrap();

        let c = db.count_pending_pushes(acct).unwrap();
        assert_eq!(c.notes, 1, "only u1 is still queued");
        assert_eq!(c.blocked, 1, "u2 is unsent but unqueued — its own number");
    }

    /// The user's own edit is one of the two re-arm paths (the other is the
    /// explicit retry). Without this, a note blocked once stays blocked even
    /// after the user changes exactly the thing that caused the refusal.
    #[test]
    fn editing_a_blocked_note_puts_it_back_in_the_queue() {
        let db = temp_db();
        let acct = "a@example.com";
        note(acct, "u1").insert(&db);
        db.mark_push_blocked("u1", acct, "permanent: nope").unwrap();

        db.apply_local_edit("u1", acct, "t2", "<div>b2</div>", "Notes").unwrap();

        assert!(db.get("u1", acct).unwrap().unwrap().push_blocked_reason.is_none());
        assert_eq!(db.list_dirty().unwrap().len(), 1, "back in the worker's queue");
    }

    /// A move changes the destination, which is what most permanent refusals
    /// on this backend are about (a folder with no reachable Exchange id).
    #[test]
    fn moving_a_blocked_note_puts_it_back_in_the_queue() {
        let db = temp_db();
        let acct = "a@example.com";
        note(acct, "u1").insert(&db);
        db.mark_push_blocked("u1", acct, "permanent: no Exchange folder id").unwrap();

        db.move_notes_batch(acct, &["u1".to_string()], "Notes/Elsewhere").unwrap();

        assert!(db.get("u1", acct).unwrap().unwrap().push_blocked_reason.is_none());
        assert_eq!(db.list_dirty().unwrap().len(), 1);
    }

    /// The explicit "Try again" path. Scoped to one account: another
    /// account's blocked notes are not the user's business here.
    #[test]
    fn clearing_blocks_re_arms_only_the_named_account() {
        let db = temp_db();
        note("a@example.com", "u1").insert(&db);
        note("b@example.com", "u2").insert(&db);
        db.mark_push_blocked("u1", "a@example.com", "permanent: nope").unwrap();
        db.mark_push_blocked("u2", "b@example.com", "permanent: nope").unwrap();

        assert_eq!(db.clear_push_blocks("a@example.com").unwrap(), 1);
        assert!(db.get("u1", "a@example.com").unwrap().unwrap().push_blocked_reason.is_none());
        assert!(db.get("u2", "b@example.com").unwrap().unwrap().push_blocked_reason.is_some());
        assert_eq!(db.clear_push_blocks("a@example.com").unwrap(), 0, "idempotent");
    }

    /// The read side the UI depends on, and the one filter it needs: a note
    /// the user has since deleted is not something to nag them about.
    #[test]
    fn listing_blocked_notes_skips_ones_the_user_has_deleted() {
        let db = temp_db();
        let acct = "a@example.com";
        note(acct, "u1").insert(&db);
        note(acct, "u2").insert(&db);
        db.mark_push_blocked("u1", acct, "permanent: nope").unwrap();
        db.mark_push_blocked("u2", acct, "permanent: nope").unwrap();
        db.mark_deleted("u2", acct).unwrap();

        let blocked = db.list_push_blocked(acct).unwrap();
        assert_eq!(blocked.len(), 1);
        assert_eq!(blocked[0].uuid, "u1");
    }

    /// The reason has to survive the hop to the frontend, or the editor can
    /// only say "something went wrong" — which is barely better than "Saved".
    #[test]
    fn the_block_reason_reaches_the_frontend_note() {
        let db = temp_db();
        let acct = "a@example.com";
        note(acct, "u1").insert(&db);
        db.mark_push_blocked("u1", acct, "permanent: attachments are impossible").unwrap();

        let n = db.get("u1", acct).unwrap().unwrap().to_frontend_note();
        assert_eq!(
            n.push_blocked_reason.as_deref(),
            Some("permanent: attachments are impossible")
        );
    }
    ...
    fn count_and_has_pending_agree_when_only_a_folder_is_dirty() {
        ...
        assert!(
            c.notes + c.deletes + c.pins + c.folders > 0,
...
mod cross_process_safety_tests {
    ...
    fn mark_pushed_with_stale_version_is_a_noop() {
        ...
        // Worker's push completes and calls mark_pushed with the local_version
        // it READ (unrelated to the new_version arg below, which is the
        // vertical's Note::version — Gmail-style tests use the same value as
        // the id since Gmail's id IS its version).
        db.mark_pushed("u1", "acc", "msg-1", "msg-1", "Mon, 11 Aug 2026 10:00:00 +0700", "<p>v1</p>", 1).unwrap();
        ...
        // With the CURRENT local_version it applies normally.
        db.mark_pushed("u1", "acc", "msg-2", "msg-2", "Mon, 11 Aug 2026 10:00:05 +0700", "<p>v1</p><p>appended</p>", row.local_version).unwrap();
        let row = db.get("u1", "acc").unwrap().unwrap();
        assert_eq!(row.sync_state, SyncState::Clean);
        assert_eq!(row.id, "msg-2");
    }

    /// `set_remote_id` exists precisely because `mark_pushed`'s guard above
    /// must NOT be the only way `id` gets stamped — see its doc comment for
    /// the duplicate-create bug that gap caused on the Microsoft backend.
    /// Same stale-local_version setup as `mark_pushed_with_stale_version_is_
    /// a_noop`, but this asserts the opposite: `id` moves anyway.
    #[test]
    fn set_remote_id_ignores_the_local_version_guard() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        let n = test_note("u4", "acc", "Title", "<p>v1</p>", "Notes");
        db.insert_local_new(&n).unwrap();

        // A concurrent edit lands mid-push, bumping local_version past what
        // the in-flight push read (still 1).
        db.apply_local_edit("u4", "acc", "Title", "<p>v1</p><p>appended</p>", "Notes").unwrap();

        db.set_remote_id("u4", "acc", "GRAPH-ID").unwrap();
        let row = db.get("u4", "acc").unwrap().unwrap();
        assert_eq!(row.id, "GRAPH-ID", "id must be stamped regardless of local_version");
        // Nothing else moved: content/sync_state are mark_pushed's job, not this one's.
        assert_eq!(row.sync_state, SyncState::Dirty, "set_remote_id must not touch sync_state");
        assert_eq!(row.body_html, "<p>v1</p><p>appended</p>", "set_remote_id must not touch body_html");
    }

    /// `rekey_note_uuid` changes a note's PRIMARY KEY. Every caller still
    /// holding the old one — the open editor, above all — needs a way back
    /// to the row, or its next write looks like a brand-new note. That is
    /// the duplicate-on-edit defect (gotcha #16).
    #[test]
    fn a_rekey_leaves_a_forwarding_address_from_the_old_uuid() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        db.insert_local_new(&test_note("local-uuid", "acc", "T", "<p>a</p>", "Notes")).unwrap();

        db.rekey_note_uuid("local-uuid", "<msg1@exchange>", "acc").unwrap();

        assert_eq!(db.resolve_note_uuid("local-uuid", "acc").unwrap(), "<msg1@exchange>");
    }

    /// Resolution is scoped per account, like every other cache read: the
    /// same uuid can legitimately exist in two mailboxes (the PK is
    /// `(uuid, account_id)`), so a forwarding address from one account must
    /// never redirect the other's edit.
    #[test]
    fn a_forwarding_address_never_crosses_accounts() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        db.insert_local_new(&test_note("shared-uuid", "acc-a", "T", "<p>a</p>", "Notes")).unwrap();
        db.insert_local_new(&test_note("shared-uuid", "acc-b", "T", "<p>b</p>", "Notes")).unwrap();

        db.rekey_note_uuid("shared-uuid", "<msg1@exchange>", "acc-a").unwrap();

        assert_eq!(db.resolve_note_uuid("shared-uuid", "acc-a").unwrap(), "<msg1@exchange>");
        assert_eq!(db.resolve_note_uuid("shared-uuid", "acc-b").unwrap(), "shared-uuid",
            "account B's row is untouched and must resolve to itself");
    }

    /// A live row always wins over an alias. Without this ordering a uuid
    /// that was rekeyed away and later re-created (an Apple-side note
    /// arriving under the same identity) would have its edits silently
    /// redirected onto the OTHER note.
    #[test]
    fn a_live_row_wins_over_a_stale_forwarding_address() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        db.insert_local_new(&test_note("u", "acc", "T", "<p>a</p>", "Notes")).unwrap();
        db.rekey_note_uuid("u", "<msg1@exchange>", "acc").unwrap();

        // Something re-creates a row under the old uuid.
        db.insert_local_new(&test_note("u", "acc", "Other", "<p>other</p>", "Notes")).unwrap();

        assert_eq!(db.resolve_note_uuid("u", "acc").unwrap(), "u",
            "the alias must be ignored while a real row exists under the uuid as asked for");
    }

    /// Collapse-on-write: A→B then B→C must leave A pointing straight at C.
    /// Resolution stays a single lookup that cannot recurse or cycle.
    #[test]
    fn repeated_rekeys_collapse_the_chain_rather_than_nesting_it() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        db.insert_local_new(&test_note("A", "acc", "T", "<p>a</p>", "Notes")).unwrap();

        db.rekey_note_uuid("A", "B", "acc").unwrap();
        db.rekey_note_uuid("B", "C", "acc").unwrap();

        assert_eq!(db.resolve_note_uuid("A", "acc").unwrap(), "C");
        assert_eq!(db.resolve_note_uuid("B", "acc").unwrap(), "C");
    }

    /// An unknown uuid with no alias is returned unchanged — that is the
    /// brand-new-note path, and forwarding must not invent a target for it.
    #[test]
    fn an_unknown_uuid_resolves_to_itself() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        assert_eq!(db.resolve_note_uuid("never-seen", "acc").unwrap(), "never-seen");
    }

    /// A move is a write, so it follows forwarding addresses too. An editor
    /// (or a folder-suggestion chip) still holding the pre-rekey uuid calls
    /// `move_notes_batch` with it; without resolution the UPDATE matches no
    /// row, returns 0, the frontend reports success and SQLite is unchanged
    /// (gotcha #16).
    #[test]
    fn a_move_by_the_pre_rekey_uuid_moves_the_live_row() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        db.insert_local_new(&test_note("local-uuid", "acc", "T", "<p>a</p>", "Notes/Inbox")).unwrap();
        db.rekey_note_uuid("local-uuid", "<msg1@exchange>", "acc").unwrap();

        let moved = db
            .move_notes_batch("acc", &["local-uuid".to_string()], "Notes/Trading")
            .unwrap();

        assert_eq!(moved, 1, "the old uuid must reach the live row");
        assert_eq!(db.get("<msg1@exchange>", "acc").unwrap().unwrap().label, "Notes/Trading");
    }

    /// `mark_pushed`'s two id-shaped parameters answer different questions
    /// (`id` addresses the remote object, `remote_version` is the
    /// change-detection token `remote_changed` compares — see the function's
    /// doc comment) and are bound to two DIFFERENT SQL placeholders (`?1` and
    /// `?8`). Every real call site today happens to pass equal strings for
    /// both (Gmail: `id IS its version`; the other tests above reuse
    /// "msg-1"/"msg-1" etc.), so nothing at THIS layer would fail if `?8`
    /// silently reverted to reusing `?1` — only the end-to-end LocalFs
    /// regression test in `lib.rs` (`localfs_push_reconcile_tests`) would
    /// eventually catch it, several layers away from the SQL that's actually
    /// wrong. Pin the binding here too, with values that would collide if the
    /// two columns aliased each other.
    #[test]
    fn mark_pushed_stamps_id_and_remote_version_from_distinct_arguments() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        let n = test_note("u3", "acc", "Title", "<p>v1</p>", "Notes");
        db.insert_local_new(&n).unwrap();

        db.mark_pushed(
            "u3", "acc",
            "msg-2",  // new_remote_id -> id, binds ?1
            "ver-2",  // new_version -> remote_version, binds ?8
            "Mon, 11 Aug 2026 10:00:00 +0700",
            "<p>v1</p>",
            n.local_version,
        )
        .unwrap();

        let row = db.get("u3", "acc").unwrap().unwrap();
        assert_eq!(row.id, "msg-2", "id must come from new_remote_id (?1)");
        assert_eq!(
            row.remote_version,
            Some("ver-2".to_string()),
            "remote_version must come from new_version (?8), not be aliased to new_remote_id"
        );
    }

    /// Microsoft M2 fix 2: `mark_pushed` must stamp `remote_label` from the
    /// row's own CURRENT `label` so `push_one_dirty`'s next move-dispatch
    /// check compares against ground truth. A brand-new row has never been
    /// confirmed on the remote, so `get_remote_label` starts `None` — not
    /// `Some("Notes")`, which would let a note filed straight into a
    /// non-default label look like "already confirmed there" without ever
    /// having been pushed.
    #[test]
    fn mark_pushed_stamps_remote_label_from_the_rows_own_label() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        let n = test_note("u5", "acc", "Title", "<p>v1</p>", "Notes/Ideas");
        db.insert_local_new(&n).unwrap();
        assert_eq!(db.get_remote_label("u5", "acc").unwrap(), None, "never pushed — nothing confirmed yet");

        db.mark_pushed("u5", "acc", "msg-1", "ver-1", "Mon, 11 Aug 2026 10:00:00 +0700", "<p>v1</p>", n.local_version).unwrap();
        assert_eq!(
            db.get_remote_label("u5", "acc").unwrap(),
            Some("Notes/Ideas".to_string()),
            "a successful push confirms the remote now holds the row's current label"
        );
    }

    /// A `mark_pushed` call whose optimistic lock misses (the row moved on
    /// since the caller read it — see `mark_pushed_with_stale_version_is_a_
    /// noop` above) must not stamp `remote_label` either: the UPDATE is a
    /// complete no-op on a guard miss, and `remote_label` is one more column
    /// in that same guarded UPDATE, not a separate write.
    #[test]
    fn a_guard_miss_leaves_remote_label_untouched() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        let n = test_note("u6", "acc", "Title", "<p>v1</p>", "Notes");
        db.insert_local_new(&n).unwrap();
        db.mark_pushed("u6", "acc", "msg-1", "ver-1", "Mon, 11 Aug 2026 10:00:00 +0700", "<p>v1</p>", n.local_version).unwrap();
        assert_eq!(db.get_remote_label("u6", "acc").unwrap(), Some("Notes".to_string()));

        // A concurrent local move lands, bumping local_version past what a
        // second, stale push attempt would supply.
        db.apply_local_edit("u6", "acc", "Title", "<p>v1</p>", "Notes/Moved").unwrap();
        db.mark_pushed("u6", "acc", "msg-2", "ver-2", "Mon, 11 Aug 2026 10:00:05 +0700", "<p>v1</p>", n.local_version).unwrap();

        assert_eq!(
            db.get_remote_label("u6", "acc").unwrap(),
            Some("Notes".to_string()),
            "a guard-missed push must not claim the remote confirmed a label it never actually moved to"
        );
    }

    /// The pull-side half of Microsoft M2 fix 2: a poll is ground truth for
    /// where the remote actually has the note filed, so `upsert_from_remote`
    /// must stamp `remote_label` too — both on the initial insert and on a
    /// later pull that reports the note under a DIFFERENT label (e.g. an
    /// Apple-side move `push_one_dirty`'s own move-dispatch never saw).
    /// A deletion refused permanently must leave every queue that hands the
    /// worker work, and every count that gates account removal.
    ///
    /// The measurement behind this: deleting a password-protected iCloud
    /// note answers HTTP 400 forever, and before 2026-08-27 this queue kept
    /// serving the same row — 491 identical requests in 3.5 hours. The
    /// content path has had gotcha #14's rule since 2026-08-17; the deletion
    /// path had none of it.
    #[test]
    fn a_permanently_blocked_deletion_leaves_the_worker_queue_and_the_pending_counts() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        let acct = "icloud:a@b.com";
        let mut n = test_note("locked-1", acct, "Locked", "<p>b</p>", "Notes");
        n.sync_state = SyncState::Clean;
        db.upsert_from_remote(&n, RemotePin::LocalWins).unwrap();
        db.mark_deleted("locked-1", acct).unwrap();

        // Before the block: the worker is handed the row and the account
        // still owes the backend something.
        assert_eq!(db.list_deleted_pending().unwrap().len(), 1);
        assert!(db.has_pending_pushes(acct).unwrap());
        assert_eq!(db.count_pending_pushes(acct).unwrap().deletes, 1);

        db.mark_push_blocked("locked-1", acct, "permanent: password-protected").unwrap();

        // After: the loop stops, and Draining can finish (gotcha #2's wedge).
        assert!(
            db.list_deleted_pending().unwrap().is_empty(),
            "a blocked deletion must not be handed back — this is the 491-retry storm"
        );
        assert!(
            !db.has_pending_pushes(acct).unwrap(),
            "a deletion the worker will never retry must not keep an account Draining forever"
        );
        let counts = db.count_pending_pushes(acct).unwrap();
        assert_eq!(counts.deletes, 0, "the queue count must match what the worker will attempt");
        assert_eq!(counts.blocked, 1, "and the number must still be visible, just separately");

        // The row itself is untouched: the note IS still on the server.
        let row = db.get("locked-1", acct).unwrap().expect("the row must not be dropped");
        assert_eq!(row.sync_state, SyncState::DeletedPending);
        assert!(row.push_blocked_reason.is_some());
    }

    #[test]
    fn a_remote_pin_is_accepted_only_where_the_remote_actually_carries_one() {
        // The two halves of RemotePin, against the same cached row. Getting
        // either backwards is silent: LocalWins on iCloud means the pin never
        // arrives, RemoteWins on Gmail means a pull unpins notes the user
        // pinned by hand and nothing reports it.
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        let acct = "icloud:a@b.com";
        let mut n = test_note("u1", acct, "T", "<p>b</p>", "Notes");
        n.sync_state = SyncState::Clean;
        n.pinned = false;
        db.upsert_from_remote(&n, RemotePin::LocalWins).unwrap();

        // Pin it locally, the way a user would on a backend that allows it.
        db.set_pin("u1", acct, true).unwrap();
        assert!(db.get("u1", acct).unwrap().unwrap().pinned);

        // A pull carrying pinned=false must NOT unpin under LocalWins.
        let mut unpinned = n.clone();
        unpinned.pinned = false;
        db.upsert_from_remote(&unpinned, RemotePin::LocalWins).unwrap();
        assert!(
            db.get("u1", acct).unwrap().unwrap().pinned,
            "LocalWins must not let a remote that knows nothing about pins clear one"
        );

        // Under RemoteWins the same pull is authoritative.
        db.upsert_from_remote(&unpinned, RemotePin::RemoteWins).unwrap();
        assert!(
            !db.get("u1", acct).unwrap().unwrap().pinned,
            "RemoteWins must accept the remote's answer, including false"
        );

        // And it can set one, which is the case that was broken: the row
        // already exists, so the INSERT branch never runs.
        let mut pinned = n.clone();
        pinned.pinned = true;
        db.upsert_from_remote(&pinned, RemotePin::RemoteWins).unwrap();
        assert!(
            db.get("u1", acct).unwrap().unwrap().pinned,
            "an existing row takes the ON CONFLICT path — the whole bug"
        );
    }

    #[test]
    fn from_remote_carries_the_pin_so_remote_wins_has_something_to_write() {
        // The layer that made RemoteWins look broken: the pin was zeroed on
        // the way in, so the ON CONFLICT clause faithfully wrote a false that
        // had already been destroyed. Visible as a pin that showed in a
        // freshly-fetched folder and vanished everywhere the list came from
        // the cache.
        let mut wire = crate::backend::Note {
            id: "r1".into(),
            uuid: "u1".into(),
            title: "T".into(),
            body_html: "<p>b</p>".into(),
            date: "2026-08-23T00:00:00Z".into(),
            version: "v1".into(),
            label: "Notes".into(),
            x_mail_created_date: None,
            account_id: None,
        ...
        assert!(
            CachedNote::from_remote("icloud:a@b.com", &wire).pinned,
            "a backend that carries a real pin must reach the upsert with it"
        );

        wire.pinned = false;
        assert!(
            !CachedNote::from_remote("gmail:a@b.com", &wire).pinned,
            "and a wire-parsed note from a sidecar backend is unpinned, as before"
        );
    }

    #[test]
    fn every_backend_states_its_pin_policy() {
        use crate::accounts::BackendKind::*;
        // Pin is a Jodd sidecar on the email backends; Apple's own on iCloud.
        assert_eq!(remote_pin_policy(Gmail), RemotePin::LocalWins);
        assert_eq!(remote_pin_policy(Microsoft), RemotePin::LocalWins);
        assert_eq!(remote_pin_policy(LocalFs), RemotePin::LocalWins);
        assert_eq!(remote_pin_policy(ICloud), RemotePin::RemoteWins);
    }

    #[test]
    fn upsert_from_remote_stamps_remote_label_from_the_incoming_label() {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        let mut n = test_note("u7", "acc", "Title", "<p>v1</p>", "Notes/Ideas");
        n.sync_state = SyncState::Clean;
        db.upsert_from_remote(&n, RemotePin::LocalWins).unwrap();
        assert_eq!(
            db.get_remote_label("u7", "acc").unwrap(),
            Some("Notes/Ideas".to_string()),
            "a pull is ground truth for what the remote currently holds"
        );

        let mut moved = n.clone();
        moved.label = "Notes/Elsewhere".to_string();
        db.upsert_from_remote(&moved, RemotePin::LocalWins).unwrap();
        assert_eq!(
            db.get_remote_label("u7", "acc").unwrap(),
            Some("Notes/Elsewhere".to_string()),
            "a later pull reporting a different label must update remote_label too, not just label"
        );
    }
...
#[cfg(test)]
mod tests_sqlcipher_backend {
    use rusqlite::Connection;

    #[test]
    fn sqlite_backend_supports_sqlcipher_pragmas() {
        let conn = Connection::open_in_memory().unwrap();
        let version: String = conn
            .query_row("PRAGMA cipher_version;", [], |r| r.get(0))
            .expect("PRAGMA cipher_version must return a value on a SQLCipher-enabled build");
        assert!(!version.is_empty(), "cipher_version was empty");
    }
}

/// Entity decoding, observed through the FTS index rather than through
/// `strip_html_to_text` directly — the layer the user actually experiences.
///
/// Apple Notes emits `&nbsp;` routinely for consecutive spaces, so before the
/// deriver decoded references the index carried the markup verbatim: searching
/// `nbsp` matched notes containing no such word, and a phrase spanning the
/// entity silently fell through to the OR-of-terms fallback and matched for
/// the wrong reason.
#[cfg(test)]
mod tests_entity_decoding_through_fts {
    use super::*;
    use tempfile::tempdir;
    ...
    const ACCT: &str = "probe@example.com";

    fn mk(uuid: &str, title: &str, body_html: &str) -> CachedNote {
        CachedNote {
            uuid: uuid.to_string(),
            account_id: ACCT.to_string(),
            id: String::new(),
            ...
            last_synced_at: None,
            last_local_modified_at: 1,
            last_remote_modified_at: Some(1_780_251_590_000),
            pinned: false,
            meta_msg_id: None,
            pin_dirty: false,
        push_blocked_reason: None,
    ...
    fn hits(db: &Db, q: &str) -> Vec<String> {
        db.search_notes(Some(ACCT), None, q, &[])
            .unwrap()
            .into_iter()
            .map(|n| n.uuid)
            .collect()
    ...
    #[test]
    fn entity_markup_is_not_searchable_as_content() {
        let db = temp_db();
        db.insert_local_new(&mk(
            "AAAAAAAA-1111-2222-3333-444444444444",
            "probe",
            "<div>2be-throwaway in&nbsp; JODD-PROBE-B and R&amp;D budget</div>",
        ))
        .unwrap();

        // The defect: these matched, because the index held the raw entities.
        assert!(hits(&db, "nbsp").is_empty(), "entity name must not be indexed as content");
        assert!(hits(&db, "amp").is_empty(), "entity name must not be indexed as content");

        // ...while the real words remain findable, so decoding didn't break indexing.
        assert_eq!(hits(&db, "throwaway").len(), 1);
        assert_eq!(hits(&db, "budget").len(), 1);
    }

    #[test]
    fn a_phrase_spanning_an_nbsp_matches_as_a_phrase_not_via_the_or_fallback() {
        let db = temp_db();
        let spanning = "AAAAAAAA-1111-2222-3333-444444444444";
        let decoy = "BBBBBBBB-1111-2222-3333-444444444444";
        // The phrase "alpha beta" only exists in `spanning`, and only once the
        // &nbsp; between the words is decoded.
        db.insert_local_new(&mk(spanning, "spanning", "<div>alpha&nbsp;beta gamma</div>")).unwrap();
        // Holds BOTH tokens but never the phrase. If the strict phrase match
        // misses and the multi-word OR-of-terms fallback fires instead, this
        // note comes back too — which is exactly what happened before the fix,
        // making the search look like it worked when it hadn't.
        db.insert_local_new(&mk(decoy, "decoy", "<div>beta something alpha</div>")).unwrap();

        assert_eq!(
            hits(&db, "alpha beta"),
            vec![spanning.to_string()],
            "phrase must match only the note that truly contains it"
        );
    }

    #[test]
    fn upsert_from_remote_reindexes_fts_so_a_deriver_change_self_heals() {
        // Pins the rollout claim: rows indexed by an older deriver need no
        // migration, because a full-sweep poll re-upserts every clean note and
        // upsert_from_remote re-derives the FTS row unconditionally.
        let db = temp_db();
        let uuid = "AAAAAAAA-1111-2222-3333-444444444444";
        let n = mk(uuid, "probe", "<div>quarterly budget</div>");
        db.insert_local_new(&n).unwrap();
        assert_eq!(hits(&db, "budget").len(), 1);

        // Stand in for a row derived by the old code: drop it from the index.
        db.conn
            .lock()
            .unwrap()
            .execute("DELETE FROM notes_fts WHERE account_id = ?1", params![ACCT])
            .unwrap();
        assert!(hits(&db, "budget").is_empty(), "index row must really be gone");

        // A poll re-upserting the SAME, unchanged note rebuilds it.
        db.upsert_from_remote(&n, RemotePin::LocalWins).unwrap();
        assert_eq!(hits(&db, "budget").len(), 1, "upsert_from_remote must re-derive the index");
    }
}

/// Migration #19 — `account_id` becomes `{backend}:{email}`.
///
/// These exercise `migrate_account_ids_with`, NEVER the `migrate_account_ids`
/// wrapper. The wrapper reaches `accounts::load_accounts()` /
/// `save_accounts()`, which resolve through `paths::config_base()` — a
/// process-global `OnceLock` that "cannot be reset between tests"
/// (`paths.rs`). A test calling the wrapper would therefore read, and
/// REWRITE, the developer's real `~/Library/Application Support/jodd/
/// accounts.json`. The split exists so the migration's whole database
/// behaviour is testable without that.
#[cfg(test)]
mod migration_19_tests {
    use super::*;
    use crate::accounts::{Account, AccountStatus, BackendKind, LlmConfig};
    use crate::test_support::note;

    fn acct(id: &str, email: &str, backend_kind: BackendKind) -> Account {
        Account {
            id: id.to_string(),
    ...
    /// A database carrying one row in EVERY `account_id` table for each id.
    ///
    /// Seeded AFTER `open_unencrypted` has already run migration #19 against
    /// an empty database, which is exactly how a real upgrade differs: there
    /// the rows predate the migration. The tests then call the migration
    /// explicitly, so the ordering costs nothing.
    fn seeded(ids: &[&str]) -> (Db, tempfile::TempDir) {
        let dir = tempfile::tempdir().unwrap();
        let db = Db::open_unencrypted(&dir.path().to_path_buf()).unwrap();
        for (i, id) in ids.iter().enumerate() {
            let uuid = format!("UUID-{i}");
            // notes + notes_fts + note_tags + edges, all through the real path.
            note(id, &uuid)
                .title("t")
                .body("body #alpha [[Other]]")
                .label("Notes/A")
                .insert(&db);
            let conn = db.conn.lock().unwrap();
            conn.execute(
                "INSERT INTO folders (account_id, path, label_id, sync_state,
                 ...
                 VALUES (?1, 'Notes/A', 'Label_1', 'clean', 0, 0)",
                params![id],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO tag_tombstones (account_id, uuid, tag, deleted_at)
                 VALUES (?1, ?2, 'gone', 0)",
                params![id, uuid],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO attachments (account_id, note_uuid, content_id, mime_type, data)
                 VALUES (?1, ?2, 'cid1', 'image/png', x'00')",
                params![id, uuid],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO note_uuid_aliases (account_id, old_uuid, new_uuid, created_at)
                 VALUES (?1, 'OLD', ?2, 0)",
                params![id, uuid],
            )
            .unwrap();
    ...
    fn distinct_account_ids(db: &Db, table: &str) -> Vec<String> {
        let conn = db.conn.lock().unwrap();
        let mut stmt = conn
            .prepare(&format!(
    ...
    const LOCALFS_ID: &str = "localfs:1E2D3C4B-0000-0000-0000-000000000000";

    #[test]
    fn migration_19_qualifies_bare_ids_and_leaves_localfs_alone() {
        let (db, _dir) = seeded(&["a@b.com", LOCALFS_ID]);
        let accounts = vec![
            acct("a@b.com", "a@b.com", BackendKind::Gmail),
            acct(LOCALFS_ID, "vault", BackendKind::LocalFs),
        ];
        {
            let conn = db.conn.lock().unwrap();
            migrate_account_ids_with(&conn, &accounts).unwrap();
        }
        // Every table, not just `notes` — a rewrite that stops short strands
        // the rest under an id nothing will ever query again.
        for table in ACCOUNT_ID_TABLES {
            assert_eq!(
                distinct_account_ids(&db, table),
                vec!["gmail:a@b.com".to_string(), LOCALFS_ID.to_string()],
                "table {table} was not rewritten"
            );
        }
    }

    #[test]
    fn migration_19_is_idempotent() {
        let (db, _dir) = seeded(&["a@b.com"]);
        let accounts = vec![acct("a@b.com", "a@b.com", BackendKind::Gmail)];
        let conn = db.conn.lock().unwrap();
        migrate_account_ids_with(&conn, &accounts).unwrap();
        migrate_account_ids_with(&conn, &accounts).unwrap();
        let id: String = conn
            .query_row("SELECT account_id FROM notes LIMIT 1", [], |r| r.get(0))
            .unwrap();
        assert_eq!(id, "gmail:a@b.com", "second run must not double-prefix");
    }

    #[test]
    fn migration_19_resolves_by_email_when_the_account_list_was_already_rewritten() {
        // The half-finished state: accounts.json holds gmail:a@b.com, the
        // database still holds the bare a@b.com. Resolution is by email, so
        // this completes rather than aborting.
        let (db, _dir) = seeded(&["a@b.com"]);
        let accounts = vec![acct("gmail:a@b.com", "a@b.com", BackendKind::Gmail)];
        let conn = db.conn.lock().unwrap();
        migrate_account_ids_with(&conn, &accounts).unwrap();
        let id: String = conn
            .query_row("SELECT account_id FROM notes LIMIT 1", [], |r| r.get(0))
    ...
    /// The orphan is seeded ALONGSIDE a resolvable id, and sorts after it, so
    /// `a@b.com` is rewritten in every table before the orphan trips the
    /// abort. A single-id fixture would not test this at all: the abort would
    /// fire before any `UPDATE` ran, and "untouched" would be satisfied by a
    /// transaction that never wrote anything — the assertion would hold even
    /// if the transaction's drop behaviour were `Commit`. The state that
    /// destroys data is a *partial* rewrite, which is what this pins.
    #[test]
    fn migration_19_aborts_and_rolls_back_a_partial_rewrite() {
        let (db, _dir) = seeded(&["a@b.com", "orphan@nowhere.com"]);
        let accounts = vec![acct("a@b.com", "a@b.com", BackendKind::Gmail)];
        let conn = db.conn.lock().unwrap();
        let err = migrate_account_ids_with(&conn, &accounts).unwrap_err();
        assert!(
            format!("{err}").contains("orphan@nowhere.com"),
        ...
        for table in ACCOUNT_ID_TABLES {
            assert_eq!(
                distinct_account_ids(&db, table),
                vec!["a@b.com".to_string(), "orphan@nowhere.com".to_string()],
                "abort must roll back the rewrite already applied to {table}"
            );
        }
    }

    #[test]
    fn migration_19_carries_fts_rows_to_the_new_id() {
        let (db, _dir) = seeded(&["a@b.com"]);
        let accounts = vec![acct("a@b.com", "a@b.com", BackendKind::Gmail)];
        {
            let conn = db.conn.lock().unwrap();
        ...
        // Content, not just the id column. Counting rows by account_id says
        // nothing about what was re-derived INTO them: transposing the title
        // and body_html arguments at the `fts_index_conn` call site would
        // leave the counts identical and every search silently indexing the
        // wrong column.
        let hits = db
            .search_notes(Some("gmail:a@b.com"), None, "alpha", &[])
            .unwrap();
        assert_eq!(
            hits.iter().map(|n| n.uuid.as_str()).collect::<Vec<_>>(),
            vec!["UUID-0"],
            "the re-derived index must still find the note by its body text"
        );

        let conn = db.conn.lock().unwrap();
        let stale: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM notes_fts WHERE account_id = 'a@b.com'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(stale, 0, "no FTS row may keep the old account_id");
        let n: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM notes_fts WHERE account_id = 'gmail:a@b.com'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert!(n > 0, "FTS rows must carry the new account_id");

        // `search_notes` MATCHes every column at once, so it alone cannot tell
        // a transposition from a correct index. These two column-scoped
        // queries can: body text must be in `body`, the title in `title`.
        let in_body: i64 = conn
            .query_row(
                "SELECT COUNT(*) FROM notes_fts WHERE notes_fts MATCH 'body:\"alpha\"'",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert_eq!(in_body, 1, "body text must be indexed in the body column");
        let (fts_title, fts_body): (String, String) = conn
            .query_row(
                "SELECT title, body FROM notes_fts WHERE account_id = 'gmail:a@b.com'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .unwrap();
        assert_eq!(fts_title, "t", "title column must hold the title");
        assert!(
            fts_body.contains("alpha"),
            "body column must hold the body text, got {fts_body:?}"
        );
    }

    /// The `false` branches of the guard that keeps `cargo test` — and every
    /// temp-dir database — away from the developer's real `accounts.json`.
    /// Untested, a refactor of `paths::data_base()` or of `Db::open`'s
    /// directory handling could flip it with the whole suite still green.
    ///
    /// `live_base` is the REAL `paths::data_base()` here, exactly as the
    /// production call site passes it: the point of this test is that a
    /// tempdir connection is rejected on the DIRECTORY comparison and not by
    /// an early `None`/empty return, so it cannot pass for the wrong reason.
    #[test]
    fn the_live_cache_guard_rejects_temp_and_in_memory_connections() {
        let (db, dir) = seeded(&["a@b.com"]);
        let conn = db.conn.lock().unwrap();
        let path = conn.path().expect("an on-disk connection must expose a path");
        assert!(!path.is_empty(), "an on-disk connection must expose a path");
        // Canonicalised: on macOS the tempdir is handed out as `/var/...`
        // while SQLite reports the resolved `/private/var/...`.
        assert!(
            std::path::Path::new(path)
                .canonicalize()
                .unwrap()
                .starts_with(dir.path().canonicalize().unwrap()),
            "fixture sanity: the connection must live in the tempdir"
        );
        assert!(
            !connection_is_the_live_user_cache(&conn, crate::paths::data_base()),
            "a tempdir database must never be treated as the live user cache"
        );

        let mem = rusqlite::Connection::open_in_memory().unwrap();
        assert!(
            !connection_is_the_live_user_cache(&mem, crate::paths::data_base()),
            "an in-memory database must never be treated as the live user cache"
        );
    }

    /// The **`true`** branch — the one that rewrites `accounts.json`.
    ///
    /// Reachable only because `live_base` is a parameter. With the lookup
    /// inlined, the sole way to make this function answer `true` was to make
    /// the connection the developer's real cache, and an earlier attempt to
    /// test it did exactly that and rewrote a real `accounts.json`. Here the
    /// "live" directory is a tempdir, so the assertion costs nothing.
    #[test]
    fn the_live_cache_guard_accepts_a_connection_in_the_live_directory() {
        let base = tempfile::tempdir().unwrap();
        // `<base>/jodd` is the shape `.setup()` builds: data_base() joined with
        // "jodd" (the guard appends the leaf itself, so `base` is passed bare).
        let db = Db::open_unencrypted(&base.path().join("jodd")).unwrap();
        let conn = db.conn.lock().unwrap();
        assert!(
            connection_is_the_live_user_cache(&conn, Some(base.path().to_path_buf())),
            "a connection inside <live_base>/jodd IS the live user cache"
        );
    }

    /// The false-negative direction the canonicalisation exists for.
    ///
    /// A false negative is the expensive one: the tables get qualified and
    /// `accounts.json` is left bare, which is the split state migration #19
    /// exists to close. macOS reaches the same shape for real — `/var` is a
    /// symlink to `/private/var`, and SQLite reports the resolved path while
    /// `paths::data_base()` does not — so this is a live case, not a
    /// contrived one.
    #[cfg(unix)]
    #[test]
    fn the_live_cache_guard_sees_through_a_symlinked_data_directory() {
        let tmp = tempfile::tempdir().unwrap();
        let real = tmp.path().join("real");
        std::fs::create_dir_all(real.join("jodd")).unwrap();
        let link = tmp.path().join("link");
        std::os::unix::fs::symlink(&real, &link).unwrap();

        // Connection opened through the REAL path, `live_base` given as the
        // symlink. Plain path equality says "different"; canonicalising both
        // sides says "same", which is the answer that keeps the account list
        // and the tables together.
        let db = Db::open_unencrypted(&real.join("jodd")).unwrap();
        let conn = db.conn.lock().unwrap();
        assert!(
            connection_is_the_live_user_cache(&conn, Some(link.clone())),
            "a symlinked data directory must still resolve to the live user cache"
        );
    }

    /// Case 2 of the fail-closed rule: an id that survives ONLY in satellite
    /// tables belongs to an account that is provably gone (`remove_account`
    /// shortens `accounts.json` before wiping the database, and
    /// `delete_account`'s satellite deletes are best-effort). It has no note
    /// to misfile, so it must be deleted rather than abort the whole open.
    #[test]
    fn migration_19_deletes_a_satellite_only_orphan_instead_of_aborting() {
        let (db, _dir) = seeded(&["a@b.com"]);
        const GONE: &str = "gone@nowhere.com";
        {
            let conn = db.conn.lock().unwrap();
            // Exactly the leftovers `delete_account` used to miss — no row in
            // `notes` or `folders` anywhere.
            conn.execute(
                "INSERT INTO note_tags (account_id, uuid, tag) VALUES (?1, 'U-GONE', 'stale')",
                params![GONE],
            )
            .unwrap();
            conn.execute(
                "INSERT INTO edges (account_id, src_uuid, dst_id, dst_title, rel) \
                 VALUES (?1, 'U-GONE', '', 'stale', 'tagged')",
                params![GONE],
            )
            .unwrap();
        }
        ...
        {
            let conn = db.conn.lock().unwrap();
            migrate_account_ids_with(&conn, &accounts)
                .expect("a satellite-only orphan must not abort the migration");
        }
        for table in ACCOUNT_ID_TABLES {
            assert!(
                !distinct_account_ids(&db, table).iter().any(|id| id == GONE),
                "table {table} still holds rows for the departed account"
            );
        }
        // …and the resolvable account was migrated as normal.
        assert_eq!(
            distinct_account_ids(&db, "notes"),
            vec!["gmail:a@b.com".to_string()]
        );
    }

    /// Case 1 — the one that still aborts — must be distinguishable at the
    /// caller. Without the marker it arrives as `DbOpenError::Sqlite`, whose
    /// `.setup()` handler opens an empty temp-dir cache: the user's intact
    /// database is simply not shown, every launch, with one log line.
    #[test]
    fn an_orphan_that_still_holds_notes_aborts_with_a_classifiable_error() {
        let (db, _dir) = seeded(&["a@b.com", "orphan@nowhere.com"]);
        let accounts = vec![acct("a@b.com", "a@b.com", BackendKind::Gmail)];
        let conn = db.conn.lock().unwrap();
        let err = migrate_account_ids_with(&conn, &accounts).unwrap_err();
        assert!(
            matches!(
                classify_migrate_error(err),
                crate::db_crypto::DbOpenError::AccountIdMigration(_)
            ),
            "the abort must reach the caller as AccountIdMigration, not as a \
             generic Sqlite error the temp-dir fallback swallows"
        );
    }

    /// An ordinary SQLite failure keeps its existing meaning — the classifier
    /// must not turn every migration error into the new variant, or the
    /// temp-dir fallback stops working for the case it was written for.
    #[test]
    fn an_ordinary_sqlite_error_is_not_reclassified() {
        let err = rusqlite::Error::InvalidParameterName("some other problem".to_string());
        assert!(matches!(
            classify_migrate_error(err),
            crate::db_crypto::DbOpenError::Sqlite(_)
        ));
    }

    /// Case 3: `load_accounts` swallows every error and returns an empty
    /// `Vec`, so an unreadable or truncated `accounts.json` is indistinguishable
    /// from "this user has no accounts". Treating it as orphans would either
    /// delete an intact cache or lock the user out of it on every launch.
    #[test]
    fn an_empty_account_list_leaves_a_non_empty_database_untouched() {
        let (db, _dir) = seeded(&["a@b.com", "c@d.com"]);
        {
            let conn = db.conn.lock().unwrap();
            migrate_account_ids_with(&conn, &[])
                .expect("an unreadable account list must not abort the open");
        }
        for table in ACCOUNT_ID_TABLES {
            assert_eq!(
                distinct_account_ids(&db, table),
                vec!["a@b.com".to_string(), "c@d.com".to_string()],
                "table {table} must be untouched when the account list could not be read"
            );
        }
    }
}
