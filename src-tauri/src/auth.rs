...
// for token exchange. The secret is embedded in the binary and explicitly
// documented as not actually secret for Desktop clients; PKCE provides the
// additional per-flow protection via the per-flow verifier.
...
pub async fn wait_for_callback() -> Result<CallbackResult, String> {
